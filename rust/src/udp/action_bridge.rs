...
use super::server::{SceneObject, AvatarMovementState};
...
impl ActionBridge {
    ...
    pub async fn say(
        ...
        for addr in &recipients {
            self.send_chat_from_simulator(
                message, speaker_name, speaker_id, position,
                1, 1, 1, *addr,
            ).await?;
        }

        info!("[ACTION_BRIDGE] {} says: '{}' to {} viewers", speaker_name, message, recipients.len());
        Ok(())
    }
    ...
    pub async fn rez_prim(
        ...
        if let Some(db) = &self.db_connection {
            ...
            let now = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_secs() as i32;
            ...
            tokio::spawn(async move {
                ...
                info!("[ACTION_BRIDGE] Persisted prim {} '{}' to DB", obj_uuid, obj_name);
            });
        }

        self.add_to_ai_inventory(obj_uuid, name, owner_id).await;

        info!("[ACTION_BRIDGE] Rezzed {:?} '{}' local_id={} uuid={} at {:?}", shape, name, local_id, obj_uuid, position);
        Ok(local_id)
    }

    pub async fn rez_box(
        &self, owner_id: Uuid, position: [f32; 3], scale: [f32; 3], name: &str,
    ) -> Result<u32> {
        self.rez_prim(PrimShape::Box, owner_id, position, scale, name).await
    }

    pub async fn rez_cylinder(
        &self, owner_id: Uuid, position: [f32; 3], scale: [f32; 3], name: &str,
    ) -> Result<u32> {
        self.rez_prim(PrimShape::Cylinder, owner_id, position, scale, name).await
    }

    pub async fn rez_sphere(
        &self, owner_id: Uuid, position: [f32; 3], scale: [f32; 3], name: &str,
    ) -> Result<u32> {
        self.rez_prim(PrimShape::Sphere, owner_id, position, scale, name).await
    }

    pub async fn rez_torus(
        &self, owner_id: Uuid, position: [f32; 3], scale: [f32; 3], name: &str,
    ) -> Result<u32> {
        self.rez_prim(PrimShape::Torus, owner_id, position, scale, name).await
    }

    pub async fn rez_tube(
        &self, owner_id: Uuid, position: [f32; 3], scale: [f32; 3], name: &str,
    ) -> Result<u32> {
        self.rez_prim(PrimShape::Tube, owner_id, position, scale, name).await
    }

    pub async fn rez_ring(
        &self, owner_id: Uuid, position: [f32; 3], scale: [f32; 3], name: &str,
    ) -> Result<u32> {
        self.rez_prim(PrimShape::Ring, owner_id, position, scale, name).await
    }

    pub async fn rez_prism(
        &self, owner_id: Uuid, position: [f32; 3], scale: [f32; 3], name: &str,
    ) -> Result<u32> {
        self.rez_prim(PrimShape::Prism, owner_id, position, scale, name).await
    }

    pub async fn set_object_position(&self, local_id: u32, position: [f32; 3]) -> Result<()> {
        {
            let mut objects = self.scene_objects.write();
            let obj = objects.get_mut(&local_id)
    ...
    pub fn find_objects_near(&self, center: [f32; 3], radius: f32, name_filter: Option<&str>) -> Vec<(u32, String, [f32; 3], bool)> {
        let objects = self.scene_objects.read();
        let r2 = radius * radius;
        let mut results: Vec<_> = objects.values()
            .filter(|o| {
            ...
            .map(|o| {
                let is_linkset = !o.scene_group_id.is_nil() && objects.values().any(|c| c.scene_group_id == o.scene_group_id && c.local_id != o.local_id);
                (o.local_id, o.name.clone(), o.position, is_linkset)
            })
            .collect();
        results.sort_by(|a, b| {
            let da = (a.2[0]-center[0]).powi(2) + (a.2[1]-center[1]).powi(2) + (a.2[2]-center[2]).powi(2);
            let db = (b.2[0]-center[0]).powi(2) + (b.2[1]-center[1]).powi(2) + (b.2[2]-center[2]).powi(2);
    ...
    pub fn scan_linkset(&self, root_local_id: u32) -> Result<Vec<(i32, u32, String, [f32; 3])>> {
        let objects = self.scene_objects.read();
        let root = objects.get(&root_local_id)
            .ok_or_else(|| anyhow!("Object {} not found", root_local_id))?;
        let group_id = root.scene_group_id;
        let mut result = vec![(root.link_number, root.local_id, root.name.clone(), root.scale)];
        if !group_id.is_nil() {
            let mut children: Vec<_> = objects.values()
    ...
    pub async fn set_object_color(&self, local_id: u32, color: [f32; 4]) -> Result<()> {
        {
            ...
            let obj = objects.get_mut(&local_id)
                .ok_or_else(|| anyhow!("Object {} not found", local_id))?;
            obj.texture_entry = crate::udp::messages::object_update::build_colored_prim_texture_entry(color);
        }
        self.broadcast_object_update(local_id).await
    }
    ...
    pub async fn set_object_texture(&self, local_id: u32, texture_uuid: Uuid) -> Result<()> {
        {
            ...
            let obj = objects.get_mut(&local_id)
                .ok_or_else(|| anyhow!("Object {} not found", local_id))?;
            obj.texture_entry = crate::udp::messages::object_update::build_textured_prim_texture_entry(texture_uuid);
        }
        self.broadcast_object_update(local_id).await
    }
    ...
    pub async fn link_objects(&self, root_id: u32, child_ids: &[u32]) -> Result<()> {
        ...
        {
            let mut objects = self.scene_objects.write();
            let root_pos = objects.get(&root_id).map(|r| r.position).unwrap_or([0.0; 3]);
        ...
        if let Some(db) = &self.db_connection {
            ...
            tokio::spawn(async move {
                if let DatabaseConnection::PostgreSQL(pool) = db.as_ref() {
                    ...
                    info!("[ACTION_BRIDGE] Persisted linkset {} ({} prims) to DB", updates[0].1, updates.len());
                }
            });
        }
        ...
        info!("[ACTION_BRIDGE] Linked {} children to root {}", child_ids.len(), root_id);
        Ok(())
    }
    ...
    pub async fn add_to_linkset(&self, root_id: u32, new_ids: &[u32]) -> Result<Vec<(u32, i32)>> {
        let (group_id, max_link, db_updates, assignments) = {
            ...
            if gid.is_nil() {
                return Err(anyhow!("Root object {} has no linkset (scene_group_id is nil)", root_id));
            }
            let max_ln = objects.values()
        ...
        if let Some(db) = &self.db_connection {
            ...
            tokio::spawn(async move {
                if let DatabaseConnection::PostgreSQL(pool) = db.as_ref() {
                    ...
                    info!("[ACTION_BRIDGE] add_to_linkset: persisted {} new prims to linkset {}", updates.len(), updates[0].1);
                }
            });
        }
        ...
        info!("[ACTION_BRIDGE] add_to_linkset: added {} prims to root {} (links {}..{})",
            assignments.len(), root_id, max_link + 1, max_link + assignments.len() as i32);
    ...
    pub async fn delete_object(&self, local_id: u32) -> Result<()> {
        ...
        let clients: Vec<SocketAddr> = {
            let states = self.avatar_states.read();
            states.values().filter(|s| !s.is_npc).map(|s| s.client_addr).collect()
        };
        ...
        for addr in clients {
            let _ = self.send_message(KILL_OBJECT_ID, &data, addr, true, false).await;
        }
    ...
    pub async fn rez_mesh(
        ...
        self.rez_mesh_with_te(owner_id, geometry, position, scale, name, None).await
    }
    ...
    pub async fn rez_mesh_with_te(
        ...
        if let Some(db) = &self.db_connection {
            if let Some(pool) = db.postgres_pool() {
                ...
                .execute(pool).await;
                info!("[ACTION_BRIDGE] Mesh asset persisted: {} ({} bytes)", asset_id, mesh_data.len());
            }
        }
        ...
        let num_faces = geometry.faces.len().min(crate::mesh::encoder::MAX_MESH_FACES);
        match num_faces {
            1 => {
        ...
        info!("[REZ_MESH] faces={}, profile_curve={}, path_curve={}, te_len={}, asset={}", num_faces, obj.profile_curve, obj.path_curve, obj.texture_entry.len(), asset_id);
        ...
        if let Some(db) = &self.db_connection {
            ...
            tokio::spawn(async move {
                ...
                info!("[ACTION_BRIDGE] Persisted mesh prim {} '{}' to DB (pc={} prc={} pb={} ph={})", obj_uuid, obj_name, pc, prc, prof_begin, prof_hollow);
            });
        }

        self.add_to_ai_inventory(obj.uuid, name, owner_id).await;

        info!("[ACTION_BRIDGE] Rezzed mesh '{}' local_id={} asset={} at {:?}", name, local_id, asset_id, position);
        Ok(local_id)
    }
    ...
    pub async fn import_mesh(
        ...
        self.rez_mesh(owner_id, geometry, position, [1.0, 1.0, 1.0], name).await
    }
    ...
    pub async fn rez_mesh_multi_lod(
        ...
        if let Some(db) = &self.db_connection {
            if let Some(pool) = db.postgres_pool() {
                ...
                .bind(asset_id).bind(name).bind(&agent_str).bind(&mesh_data)
                .execute(pool).await;
                info!("[ACTION_BRIDGE] Multi-LOD mesh asset persisted: {} ({} bytes)", asset_id, mesh_data.len());
            }
        }
        ...
        match num_faces {
            1 => { obj.profile_curve = 48; obj.path_curve = 32; obj.path_scale_y = 150; }
            2 => { obj.profile_curve = 48; obj.path_curve = 32; obj.profile_hollow = 27500; obj.path_scale_y = 150; }
            3 => { obj.profile_curve = 48; obj.path_curve = 16; }
            4 => { obj.profile_curve = 48; obj.path_curve = 16; obj.profile_hollow = 27500; }
            5 => { obj.profile_curve = 51; obj.path_curve = 16; }
            6 => { obj.profile_curve = 49; obj.path_curve = 16; }
            7 => { obj.profile_curve = 49; obj.path_curve = 16; obj.profile_hollow = 27500; }
            _ => { obj.profile_curve = 49; obj.path_curve = 16; obj.profile_begin = 9375; }
        }
        info!("[REZ_MESH_MULTI_LOD] faces={}, profile_curve={}, path_curve={}, asset={}", num_faces, obj.profile_curve, obj.path_curve, asset_id);
        ...
        info!("[ACTION_BRIDGE] Rezzed multi-LOD mesh '{}' local_id={} asset={} at {:?}", name, local_id, asset_id, position);
        Ok(local_id)
    }
    ...
    pub async fn blender_generate(
        ...
        use crate::mesh::blender_worker::{BlenderWorker, ExportFormat, is_clothing_template};

        let worker = BlenderWorker::new()?;
        let script = BlenderWorker::get_template(template, params)?;

        if is_clothing_template(template) {
            let dae_path = worker.generate_with_format(&script, ExportFormat::Dae).await?;
            let geometry = crate::mesh::glc_bridge::import_dae_with_skin(&dae_path.to_string_lossy())?;
            info!("[ACTION_BRIDGE] Clothing '{}': {} faces, skin={}",
                name, geometry.faces.len(), geometry.skin_info.is_some());
            self.rez_mesh(owner_id, geometry, position, [1.0, 1.0, 1.0], name).await
        } else {
            ...
            self.import_mesh(owner_id, &obj_path.to_string_lossy(), name, position).await
        }
    }
    ...
    pub async fn blender_generate_snapshot(
        ...
        use crate::mesh::blender_worker::{BlenderWorker, ExportFormat, ruth2_base_dir};
        ...
        let mut live_texture_entries: std::collections::HashMap<Uuid, Vec<u8>> = std::collections::HashMap::new();
        {
            let objects = self.scene_objects.read();
            for obj in objects.values() {
                if obj.owner_id == owner_id && obj.attachment_point > 0 && !obj.texture_entry.is_empty() {
        ...
        let avatar_name = Self::get_avatar_name_pg(&owner_id, pool).await.unwrap_or_else(|| "Avatar".to_string());

        let snapshot = collector.collect_snapshot_with_live_te(
            &owner_id, &avatar_name, &appearance, pool, None, &live_texture_entries,
        ).await?;

        let mut pieces_json = Vec::new();
        let mut piece_texture_map: std::collections::HashMap<String, Vec<Option<Uuid>>> = std::collections::HashMap::new();
        for piece in &snapshot.pieces {
            let dae_path = tmp_dir.join(format!("{}.dae", piece.name));
            let tex_files: Vec<Option<String>> = piece.texture_paths.iter()
            ...
            info!("[SNAPSHOT] Wrote piece DAE: {} ({} faces, {} tex_uuids)", piece.name, piece.geometry.faces.len(), piece.texture_uuids.len());

            piece_texture_map.insert(piece.name.clone(), piece.texture_uuids.clone());

            let textures: Vec<serde_json::Value> = tex_files.iter()
                .map(|t| match t {
        ...
        let baked_head_uuid = if baked.head.is_some() { baked.head_uuid } else { None };
        let baked_upper_uuid = if baked.upper.is_some() { baked.upper_uuid } else { None };
        let baked_lower_uuid = if baked.lower.is_some() { baked.lower_uuid } else { None };
        let baked_eyes_uuid = if baked.eyes.is_some() { baked.eyes_uuid } else { None };
        ...
        let pose_json_path = Self::resolve_pose_json_pg(anim_name, &tmp_dir, pool).await
            .unwrap_or_default();
        ...
        info!("[SNAPSHOT] Wrote manifest: {} ({} pieces)", manifest_path.display(), pieces_json.len());

        let mut params = std::collections::HashMap::new();
        params.insert("MANIFEST_PATH".to_string(), manifest_path.to_string_lossy().to_string());
        ...
        let parts_data: Vec<serde_json::Value> = serde_json::from_str(
            &std::fs::read_to_string(&parts_manifest_path)?
        )?;
        ...
        for (idx, part) in parts_data.iter().enumerate() {
            ...
            let offset = part["offset"].as_array().map(|a| {
                [
            ...
            if let Some(bake_uuid) = body_tex_map.get(part_name).and_then(|u| *u) {
                ...
                has_texture = true;
                info!("[SNAPSHOT] Body part {} → baked texture {}", part_name, bake_uuid);
                ...
                if has_texture {
                    info!("[SNAPSHOT] Attachment {} → {} face textures", part_name, face_uuids.iter().filter(|u| u.is_some()).count());
                }
            }
            let te_bytes = if has_texture { Some(ted.to_bytes()) } else { None };
            ...
            match crate::mesh::glc_bridge::import_file(part_path) {
                Ok(mut geometry) => {
                    ...
                    if geometry.faces.len() < orig_faces {
                        info!("[SNAPSHOT] Filtered {}/{} faces (removed physics hulls)", geometry.faces.len(), orig_faces);
                    }
                    ...
                    match self.rez_mesh_with_te(owner_id, geometry, part_pos, scale, &piece_name, te_bytes.clone()).await {
                        Ok(local_id) => {
        ...
        info!("[SNAPSHOT] Rezzed {}/{} parts", rezzed_ids.len(), parts_data.len());
        ...
        if rezzed_ids.len() > 1 {
            ...
            if let Err(e) = self.link_objects(root_id, child_ids).await {
                ...
                info!("[SNAPSHOT] Linked {} parts into linkset (root={})", rezzed_ids.len(), root_id);
            }
        }

        let local_id = rezzed_ids[0];

        info!("[SNAPSHOT] Keeping temp dir for debugging: {}", tmp_dir.display());
    ...
    async fn load_appearance_for_snapshot(
        ...
        use crate::region::avatar::appearance::{Appearance, TextureEntry, Attachment};
        ...
        for (name, value) in &ap_rows {
            ...
            if parts.len() >= 2 {
                if let (Ok(item_id), Ok(asset_id)) = (parts[0].parse::<Uuid>(), parts[1].parse::<Uuid>()) {
                    appearance.attachments.push(Attachment { item_id, asset_id, point });
                }
            }
        }
            ...
            for obj in objects.values() {
                if obj.owner_id == *agent_id && obj.attachment_point > 0 {
                    if let Some(mesh_uuid) = crate::mesh::extract_mesh_asset_uuid(&obj.extra_params) {
                        ...
                        let already_has = appearance.attachments.iter().any(|a| a.asset_id == mesh_uuid && a.point == point);
                        if !already_has {
                            info!("[SNAPSHOT] Found live mesh attachment: '{}' point={} mesh={}",
    ...
    async fn resolve_pose_json_pg(anim_name: &str, tmp_dir: &std::path::Path, pool: &sqlx::PgPool) -> Option<String> {
        let instance_dir = std::env::var("OPENSIM_INSTANCE_DIR").unwrap_or_else(|_| ".".to_string());
        let candidates = vec![
            format!("{}/../../assets/poses/{}.pose.json", instance_dir, anim_name),
            format!("{}/../../opensim-next/assets/poses/{}.pose.json", instance_dir, anim_name),
        ...
        let row: Option<(Vec<u8>,)> = sqlx::query_as(
            "SELECT data FROM assets WHERE name = $1 AND assettype = 20"
        )
        .bind(anim_name)
        .fetch_optional(pool)
        .await
    ...
    pub async fn create_tshirt(
        ...
        use crate::mesh::texture_compositor::{
            TShirtTextureConfig, LogoPlacement, PlacementSide, BodyRegion, compose_tshirt_texture,
        };
        ...
        .execute(pool)
        .await?;
        info!("[TSHIRT] Texture asset {} uploaded ({} bytes J2K)", texture_asset_id, j2k_data.len());
        ...
        params.insert("TEXTURE_PATH".to_string(), texture_png_path.to_string_lossy().to_string());

        let script = BlenderWorker::get_template("shirt", &params)?;
        let dae_path = worker.generate_with_format(&script, ExportFormat::Dae).await?;
        let geometry = crate::mesh::glc_bridge::import_dae_with_skin(&dae_path.to_string_lossy())?;
        info!("[TSHIRT] Mesh generated: {} faces, skin={}", geometry.faces.len(), geometry.skin_info.is_some());

        let npc_pos = self.session_manager.get_avatar_position(owner_id)
            .unwrap_or([128.0, 128.0, 25.0]);
        let rez_pos = [npc_pos[0] + 1.0, npc_pos[1], npc_pos[2]];

        let local_id = self.rez_mesh(owner_id, geometry, rez_pos, [1.0, 1.0, 1.0], name).await?;

        {
            let mut objects = self.scene_objects.write();
            if let Some(obj) = objects.get_mut(&local_id) {
                let te = crate::udp::messages::object_update::build_textured_prim_texture_entry(texture_asset_id);
    ...
    pub fn send_body_alpha_for_garment(&self, garment_type: &str) {
        ...
        if let Some(ref ye) = self.yengine {
            ...
            if !zones.is_empty() {
                info!("[ALPHA] Sent {} alpha hide commands for garment type '{}'", zones.len(), garment_type);
            }
        }
    }

    pub async fn export_oar(&self, region_id: Uuid, filename: &str, object_uuids: Option<Vec<Uuid>>) -> Result<String> {
        use crate::archives::oar::writer::{OarWriter, OarSaveOptions};
        ...
        let msg = format!("Saved {} objects, {} assets to {} ({} bytes)",
            result.stats.objects_saved, result.stats.assets_saved,
            safe_filename, result.stats.archive_size_bytes);
        info!("[ACTION_BRIDGE] OAR export: {}", msg);
        Ok(msg)
    }

    pub async fn insert_script(&self, owner_id: Uuid, local_id: u32, script_name: &str, script_source: &str) -> Result<()> {
        let object_uuid = {
            let objects = self.scene_objects.read();
            objects.get(&local_id).map(|o| o.uuid)
        };
        let object_id = object_uuid.ok_or_else(|| anyhow!("Object {} not found for script insertion", local_id))?;
        ...
        if let Some(db) = &self.db_connection {
            if let Some(pool) = db.postgres_pool() {
                ...
                info!("[ACTION_BRIDGE] Script '{}' persisted: item={} asset={} in prim={}", script_name, new_item_id, asset_id, object_id);
            }
        }
        ...
        let task_inv = if let Some(db) = &self.db_connection {
            if let Some(pool) = db.postgres_pool() {
                ...
                .await
                .unwrap_or_default();
                rows.into_iter().map(|(name, asset_id, inv_type, asset_type, perms)| {
                    crate::scripting::executor::TaskInventoryEntry {
                        name, asset_id, inv_type, asset_type, permissions: perms as u32,
                    }
                }).collect::<Vec<_>>()
            } else { Vec::new() }
        } else { Vec::new() };
        ...
        if let Some(yengine) = &self.yengine {
            ...
            match engine.rez_script(script_id, script_source, 1) {
                    ...
                    let obj_ctx = {
                        ...
                        objects.values().find(|o| o.uuid == object_id).map(|o| {
                            ...
                            crate::scripting::executor::ObjectContext {
                                object_id: o.uuid, owner_id: o.owner_id,
                                object_name: o.name.clone(),
                                position: crate::scripting::LSLVector::new(o.position[0], o.position[1], o.position[2]),
                                rotation: crate::scripting::LSLRotation::new(o.rotation[0], o.rotation[1], o.rotation[2], o.rotation[3]),
                                scale: crate::scripting::LSLVector::new(o.scale[0], o.scale[1], o.scale[2]),
                ...
                Err(e) => info!("[ACTION_BRIDGE] Script '{}' compile failed: {}", script_name, e),
            }
        }
    ...
    pub async fn update_script(&self, owner_id: Uuid, local_id: u32, script_name: &str, script_source: &str) -> Result<()> {
        let object_uuid = {
            let objects = self.scene_objects.read();
            objects.get(&local_id).map(|o| o.uuid)
        };
        let object_id = object_uuid.ok_or_else(|| anyhow!("Object {} not found for script update", local_id))?;
        ...
        if let Some(db) = &self.db_connection {
            if let Some(pool) = db.postgres_pool() {
                ...
                .bind(object_id).bind(script_name)
                ...
                if let Some((item_id, old_asset_id)) = existing {
                    ...
                    let _ = sqlx::query(
                        "UPDATE primitems SET assetid = $1 WHERE itemid = $2"
                    )
                    .bind(new_asset_id).bind(item_id)
                    .execute(pool).await;
                    ...
                    if let Some(yengine) = &self.yengine {
                        ...
                        match engine.rez_script(script_id, script_source, 1) {
                            Ok(()) => {
                                ...
                                info!("[ACTION_BRIDGE] Script '{}' updated and restarted: {}", script_name, script_id);
                            }
                            Err(e) => info!("[ACTION_BRIDGE] Script '{}' recompile failed: {}", script_name, e),
                        }
                    }
                    info!("[ACTION_BRIDGE] Script '{}' asset updated: {} -> {}", script_name, old_asset_id, new_asset_id);
                } else {
                    info!("[ACTION_BRIDGE] Script '{}' not found in prim {}, inserting new", script_name, object_id);
                    return self.insert_script(owner_id, local_id, script_name, script_source).await;
                }
            }
        }
        Ok(())
        ...
        if let Some(db) = &self.db_connection {
            if let Some(pool) = db.postgres_pool() {
                ...
                if !root_items.is_empty() {
                    task_inv_map.insert(root_obj.uuid, serialize_task_inv_xml_standalone(&root_items, root_obj.uuid));
                }
                for child in &children {
                    let child_items = query_task_inventory_for_prim(pool, child.uuid).await;
                    if !child_items.is_empty() {
                        task_inv_map.insert(child.uuid, serialize_task_inv_xml_standalone(&child_items, child.uuid));
                    }
                }
                let asset_id = Uuid::new_v4();
                info!("[GIVE_OBJECT] '{}' extra_params={} bytes{}, sculpt_type={}, children={}",
                      obj_name, root_obj.extra_params.len(),
                      if root_obj.extra_params.len() >= 24 { format!(" (mesh_uuid={})", Uuid::from_slice(&root_obj.extra_params[7..23]).unwrap_or(Uuid::nil())) } else { String::new() },
                      if root_obj.extra_params.len() >= 24 { root_obj.extra_params[23] } else { 0 },
                      children.len());
                let xml = serialize_linkset_to_xml_standalone(&root_obj, &children, &task_inv_map, self.region_handle);
                ...
                .bind(target_agent_id).fetch_optional(pool).await?;
                ...
                info!("[ACTION_BRIDGE] Object '{}' given to {}: item={} asset={}", obj_name, target_agent_id, item_id, asset_id);
                ...
                let viewer_addr = {
                    let states = self.avatar_states.read();
                    states.get(&target_agent_id).filter(|s| !s.is_npc).map(|s| s.client_addr)
                };
                ...
                if let Some(dest) = viewer_addr {
                    ...
                    let _ = self.send_message(BULK_UPDATE_INVENTORY_ID, &body, dest, true, true).await;
                    info!("[ACTION_BRIDGE] Sent BulkUpdateInventory UDP for item {} to agent {} at {}", item_id, target_agent_id, dest);
                }

                let position = {
                    let states = self.avatar_states.read();
                    states.get(&target_agent_id)
                        .map(|s| s.position)
                        .unwrap_or([128.0, 128.0, 25.0])
                };
                let _ = self.say(
    ...
    pub async fn package_object_into_prim(&self, source_local_id: u32, container_local_id: u32) -> Result<String> {
        let (source_root, children, source_name, owner_id) = {
            let objects = self.scene_objects.read();
            let src = objects.get(&source_local_id)
        ...
        let container_uuid = {
            let objects = self.scene_objects.read();
            objects.get(&container_local_id)
                .ok_or_else(|| anyhow!("Container object {} not found", container_local_id))?
                .uuid
        };

        let db = self.db_connection.as_ref().ok_or_else(|| anyhow!("No DB for package_object"))?;
        let pool = db.postgres_pool().ok_or_else(|| anyhow!("No PG pool for package_object"))?;

        let mut task_inv_map = std::collections::HashMap::new();
        let root_items = query_task_inventory_for_prim(pool, source_root.uuid).await;
        if !root_items.is_empty() {
            task_inv_map.insert(source_root.uuid, serialize_task_inv_xml_standalone(&root_items, source_root.uuid));
        }
        for child in &children {
            let child_items = query_task_inventory_for_prim(pool, child.uuid).await;
            if !child_items.is_empty() {
        ...
        let asset_id = Uuid::new_v4();
        let xml = serialize_linkset_to_xml_standalone(&source_root, &children, &task_inv_map, self.region_handle);
        let xml_bytes = xml.as_bytes().to_vec();
        ...
        for dest in &recipients {
            let _ = self.send_message(KILL_OBJECT_ID, &kill_data, *dest, true, false).await;
        }
        ...
        tokio::spawn(async move {
            if let Some(pool) = db_clone.postgres_pool() {
                for uuid in std::iter::once(source_uuid).chain(child_uuids) {
                    let _ = sqlx::query("DELETE FROM primitems WHERE primid = $1::uuid").bind(uuid).execute(pool).await;
                    let _ = sqlx::query("DELETE FROM primshapes WHERE uuid = $1::uuid").bind(uuid).execute(pool).await;
                    let _ = sqlx::query("DELETE FROM prims WHERE uuid = $1::uuid").bind(uuid).execute(pool).await;
                }
            }
        });

        info!("[ACTION_BRIDGE] Packaged '{}' (local_id={}) into container {} as task inv item {}", source_name, source_local_id, container_local_id, item_id);
        Ok(format!("Packaged '{}' into container", source_name))
    }

    async fn add_to_ai_inventory(&self, obj_uuid: Uuid, obj_name: &str, owner_id: Uuid) {
        let Some(db) = &self.db_connection else { return; };
        let Some(pool) = db.postgres_pool() else { return; };
        ...
        tokio::spawn(async move {
            ...
            .bind(&owner_str).fetch_optional(&pool).await.unwrap_or(None);
            ...
            let ai_folder_id = if let Some((fid,)) = ai_folder {
                ...
                .execute(&pool).await;
                info!("[ACTION_BRIDGE] Created 'AI Generated' folder {} for {}", new_folder_id, owner_str);
                new_folder_id
            };
            ...
            let xml = format!(
                ...
                obj_uuid, xml_escape(&obj_name), owner_id, owner_id
            );
            let xml_bytes = xml.as_bytes().to_vec();
            ...
            info!("[ACTION_BRIDGE] Added '{}' to AI Generated inventory: item={} asset={}", obj_name, item_id, asset_id);
        });
    }
    ...
    async fn broadcast_object_update(&self, local_id: u32) -> Result<()> {
        ...
        let mut prim_data = object_update::AvatarObjectData::create_prim_from_scene_object(&obj_clone, region_handle);
        ...
        for addr in &clients {
            ...
            info!("[ACTION_BRIDGE] ObjectUpdate packet: {} bytes, seq={}", buffer.len(), sequence);

            match self.socket.send_to(&buffer, *addr).await {
                Ok(n) => info!("[ACTION_BRIDGE] Sent ObjectUpdate local_id={} to {} ({} bytes, seq={})", local_id, addr, n, sequence),
                Err(e) => info!("[ACTION_BRIDGE] FAILED ObjectUpdate local_id={} to {}: {}", local_id, addr, e),
            }
        }
    ...
    async fn send_chat_from_simulator(
        &self, message: &str, from_name: &str, source_id: Uuid,
        from_pos: [f32; 3], chat_type: u8, source_type: u8, audible: u8,
        dest: SocketAddr,
    ) -> Result<()> {
        ...
        self.send_message(CHAT_FROM_SIMULATOR_ID, &data, dest, true, false).await
    }
    ...
    async fn send_message(
        ...
        let mut flags: u8 = 0;
        if reliable { flags |= 0x40; }
        if zero_encoded { flags |= 0x80; }
    ...
    pub async fn setup_cinematography(
        ...
        if lights.is_empty() {
            let preset = lighting_preset.as_deref()
        ...
        info!("[CINEMATOGRAPHY] Setting up scene '{}': {} lights, {} waypoints, shot='{}'",
            scene_name, lights.len(), camera_waypoints.len(), shot_type);

        for light in &lights {
            let light_params = {
                let mut p = std::collections::HashMap::new();
                p.insert("COLOR".to_string(), format!("{},{},{}", light.color[0], light.color[1], light.color[2]));
            ...
            match self.rez_sphere(owner_id, light.position, [0.3, 0.3, 0.3], &light.name).await {
                Ok(local_id) => {
                    created_ids.push((light.name.clone(), local_id));
                    if let Err(e) = self.set_object_color(local_id, [light.color[0], light.color[1], light.color[2], 0.1]).await {
                        info!("[CINEMATOGRAPHY] Failed to set light alpha: {}", e);
                    }
                    if let Some(source) = crate::ai::script_templates::apply_template("cinema_light", &light_params) {
                        if let Err(e) = self.insert_script(owner_id, local_id, "cinema_light", &source).await {
                            info!("[CINEMATOGRAPHY] Failed to insert light script: {}", e);
                        }
                    }
                    info!("[CINEMATOGRAPHY] Light '{}' placed at {:?} (intensity={}, radius={})",
                        light.name, light.position, light.intensity, light.radius);
                }
                Err(e) => info!("[CINEMATOGRAPHY] Failed to rez light '{}': {}", light.name, e),
            }
        }
        ...
        let drone_pos = if !camera_waypoints.is_empty() {
            ...
            [subject_position[0] - 10.0, subject_position[1], subject_position[2] + 5.0]
        };

        let drone_name = format!("{} Camera", scene_name);
        match self.rez_sphere(owner_id, drone_pos, [0.5, 0.5, 0.5], &drone_name).await {
            Ok(drone_id) => {
                ...
                for (i, wp) in camera_waypoints.iter().enumerate() {
                    ...
                    wp_data.push_str(&format!("{}|{}|{}|{}|{}|{}|{}",
                        ...
                        wp.focus[0], wp.focus[1], wp.focus[2],
                        wp.dwell));
                }

                let mut drone_params = std::collections::HashMap::new();
                drone_params.insert("SPEED".to_string(), format!("{}", speed));
                if let Some(source) = crate::ai::script_templates::apply_template("drone_camera", &drone_params) {
                    if let Err(e) = self.insert_script(owner_id, drone_id, "drone_camera", &source).await {
                ...
                info!("[CINEMATOGRAPHY] Drone camera placed at {:?} with {} waypoints", drone_pos, camera_waypoints.len());
            }
        ...
        if created_ids.len() > 1 {
            ...
            let child_ids: Vec<u32> = created_ids.iter()
        ...
        info!("[CINEMATOGRAPHY] Scene '{}' complete: {} objects created", scene_name, created_ids.len());
        Ok(created_ids)
    }
    ...
    pub async fn generate_terrain(
        ...
        use crate::region::terrain_generator::{self, TerrainParams, preset_description};
        use crate::region::terrain_sender::TerrainSender;
        ...
        terrain_sender.store_and_cache_heightmap(region_uuid, heightmap.clone()).await?;

        let client_addrs: Vec<SocketAddr> = {
            let states = self.avatar_states.read();
            states.values().filter(|s| !s.is_npc).map(|s| s.client_addr).collect()
        };
        terrain_sender.broadcast_full_terrain(region_uuid, &client_addrs).await?;
        ...
        let r32_path = if let (Some(gs), Some(gx), Some(gy)) = (grid_size, grid_x, grid_y) {
            format!("{}/{}_{}_grid{}x{}_tile{}_{}.r32", terrains_dir, preset, actual_seed, gs, gs, gx, gy)
        } else {
        ...
        match self.rez_terrain_preview(npc_id, &heightmap, preview_pos, preset).await {
    ...
    pub async fn preview_terrain(
        ...
        let preview_pos = [position[0] + 2.0, position[1], position[2] + 1.0];
        let preview_local_id = match self.rez_terrain_preview(npc_id, &heightmap, preview_pos, preset).await {
            Ok(lid) => { info!("[TERRAIN] Preview model rezzed: local_id={}", lid); lid }
            Err(e) => { warn!("[TERRAIN] Failed to rez preview model: {}", e); 0 }
        };
...

        {
            ...
            pending.insert(preview_id.clone(), PendingTerrain {
                heightmap,
                region_uuid,
                preset: preset.to_string(),
                seed: actual_seed,
        ...
        let hmax = {
            let pending = self.pending_terrains.read();
            let pt = pending.get(&preview_id).unwrap();
            pt.heightmap.iter().cloned().fold(f32::NEG_INFINITY, f32::max)
        };
        let desc = preset_description(preset);
        let msg = format!(
    ...
    pub async fn apply_pending_terrain(
        &self,
        npc_id: Uuid,
        preview_id: &str,
    ) -> Result<String> {
        ...
        let terrain_sender = TerrainSender::new(db.clone(), self.socket.clone());
        terrain_sender.store_and_cache_heightmap(pending_terrain.region_uuid, pending_terrain.heightmap.clone()).await?;

        let client_addrs: Vec<SocketAddr> = {
            let states = self.avatar_states.read();
            states.values().filter(|s| !s.is_npc).map(|s| s.client_addr).collect()
        };
        terrain_sender.broadcast_full_terrain(pending_terrain.region_uuid, &client_addrs).await?;

        save_heightmap_r32(&pending_terrain.heightmap, 256, &pending_terrain.r32_path)?;
        info!("[TERRAIN] Backed up terrain to {}", pending_terrain.r32_path);

        if pending_terrain.preview_local_id > 0 {
            self.scene_objects.write().remove(&pending_terrain.preview_local_id);
        ...
        let hmin = pending_terrain.heightmap.iter().cloned().fold(f32::INFINITY, f32::min);
        let hmax = pending_terrain.heightmap.iter().cloned().fold(f32::NEG_INFINITY, f32::max);
        let msg = format!(
    ...
    pub async fn reject_pending_terrain(
        &self,
        npc_id: Uuid,
        preview_id: &str,
    ) -> Result<String> {
    ...
    pub async fn rez_terrain_preview(
        ...
        dynamic_img.write_to(&mut png_data, image::ImageFormat::Png)
        ...
        if let Some(db) = &self.db_connection {
            if let Some(pool) = db.postgres_pool() {
                ...
                .bind(&j2k_data)
                .execute(pool).await;
                info!("[TERRAIN_PREVIEW] Sculpt asset stored: {} ({} bytes)", sculpt_asset_id, j2k_data.len());
            }
        }
    ...
    pub async fn load_terrain_image(
        ...
        let preview_pos = [position[0] + 2.0, position[1], position[2] + 1.0];
        let _ = self.rez_terrain_preview(npc_id, &heightmap, preview_pos, basename).await;
...
pub fn load_heightmap_r32(path: &str) -> Result<Vec<f32>> {
    ...
    if data.len() % 4 != 0 {
        return Err(anyhow!("Invalid .r32 file: size {} not multiple of 4", data.len()));
    }
    let count = data.len() / 4;
    let side = (count as f64).sqrt() as usize;
    if side * side != count {
        return Err(anyhow!("Invalid .r32 file: {} values is not a square grid", count));
    }
    let mut heightmap = Vec::with_capacity(count);
    for i in 0..count {
        let offset = i * 4;
        let h = f32::from_le_bytes([data[offset], data[offset+1], data[offset+2], data[offset+3]]);
        heightmap.push(h);
    }
    info!("[TERRAIN] Loaded {0}x{0} .r32 heightmap from {1}", side, path);
    Ok(heightmap)
}
...
pub fn load_heightmap_from_image(
    ...
    let target_size = if w >= 512 && h >= 512 { 512u32 }
        else { 256u32 };

    let resized = if w != target_size || h != target_size {
        image::imageops::resize(&gray, target_size, target_size, image::imageops::FilterType::Lanczos3)
    } else {
        gray
    };
...
fn serialize_part_xml_standalone(
    obj: &SceneObject, group_pos: [f32; 3], offset_pos: [f32; 3],
    parent_id: u32, task_inv_xml: &str, region_handle: u64,
) -> String {
    use base64::Engine;
    ...
    let creation_date = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_secs();
    format!(
        ...
        uuid = obj.uuid, local_id = obj.local_id,
        name = xml_escape(&obj.name), desc = xml_escape(&obj.description),
        material = obj.material, region_handle = region_handle,
        gpx = group_pos[0], gpy = group_pos[1], gpz = group_pos[2],
        opx = offset_pos[0], opy = offset_pos[1], opz = offset_pos[2],
        rx = obj.rotation[0], ry = obj.rotation[1], rz = obj.rotation[2], rw = obj.rotation[3],
        sx = obj.scale[0], sy = obj.scale[1], sz = obj.scale[2],
        link_num = obj.link_number, parent_id = parent_id,
        pcode = obj.pcode, profile_curve = obj.profile_curve, path_curve = obj.path_curve,
        path_begin = obj.path_begin, path_end = obj.path_end,
        path_scale_x = obj.path_scale_x, path_scale_y = obj.path_scale_y,
        path_shear_x = obj.path_shear_x, path_shear_y = obj.path_shear_y,
        path_twist = obj.path_twist, path_twist_begin = obj.path_twist_begin,
        path_radius_offset = obj.path_radius_offset,
        path_taper_x = obj.path_taper_x, path_taper_y = obj.path_taper_y,
        path_revolutions = obj.path_revolutions, path_skew = obj.path_skew,
        profile_begin = obj.profile_begin, profile_end = obj.profile_end,
        profile_hollow = obj.profile_hollow, tex = tex_hex, extra = extra_b64,
        sculpt_texture = sculpt_texture, sculpt_type = sculpt_type,
        owner = obj.owner_id, group_id = obj.group_id, flags = obj.flags,
        base_mask = obj.owner_mask, owner_mask = obj.owner_mask, next_owner_mask = obj.owner_mask,
...
fn serialize_task_inv_xml_standalone(items: &[(Uuid, Uuid, i32, i32, String, String, i32, String, String, String, i32, i32, i32, i32, i32, i32)], prim_id: Uuid) -> String {
    if items.is_empty() {
        return String::new();
    }
    let mut xml = String::from("<TaskInventory>");
    for (item_id, asset_id, asset_type, inv_type, name, desc,
         creation_date, creator_id, owner_id, group_id,
         base_perm, cur_perm, grp_perm, every_perm, next_perm, flags) in items
    {
        xml.push_str(&format!(
            "<TaskInventoryItem>\
            ...
            asset_id = asset_id, base_perm = base_perm, creation_date = creation_date,
            creator_id = creator_id, desc = xml_escape(desc), every_perm = every_perm,
            flags = flags, group_id = group_id, grp_perm = grp_perm,
            inv_type = inv_type, item_id = item_id, owner_id = owner_id,
            name = xml_escape(name), next_perm = next_perm, cur_perm = cur_perm,
            prim_id = prim_id, asset_type = asset_type,
        ));
    }
...
pub fn serialize_linkset_to_xml_standalone(
    root: &SceneObject, children: &[SceneObject],
    task_inv_map: &std::collections::HashMap<Uuid, String>, region_handle: u64,
) -> String {
    ...
    xml.push_str(&serialize_part_xml_standalone(root, root_pos, [0.0, 0.0, 0.0], 0, root_task_inv, region_handle));
    ...
    if children.is_empty() {
        ...
        for child in children {
            ...
            xml.push_str("<Part>");
            xml.push_str(&serialize_part_xml_standalone(child, root_pos, child.position, root.local_id, child_task_inv, region_handle));
            xml.push_str("</Part>");
        }
        xml.push_str("</OtherParts>");
    }
...
pub async fn query_task_inventory_for_prim(pool: &sqlx::PgPool, prim_uuid: Uuid) -> Vec<(Uuid, Uuid, i32, i32, String, String, i32, String, String, String, i32, i32, i32, i32, i32, i32)> {
