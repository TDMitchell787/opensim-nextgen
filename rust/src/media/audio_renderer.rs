...
pub fn render_ambient_audio(preset: &str, duration_secs: f32, output_path: &Path) -> Result<()> {
    ...
    write_wav(output_path, &samples, sample_rate, channels, bits_per_sample)?;

    info!("[MEDIA] Audio rendered: {} ({:.1}s, preset='{}')",
        output_path.display(), duration_secs, preset);
    Ok(())
}
...
fn generate_ambient_samples(preset: &str, total_samples: usize, sample_rate: u32) -> Vec<i16> {
    ...
    match preset {
        ...
        "wind" => {
            for i in 0..total_samples {
                ...
                let whistle = (t * 800.0 * std::f64::consts::TAU).sin()
                    * (t * 0.07 * std::f64::consts::TAU).sin().abs() * 0.05;
        ...
        "forest" => {
            for i in 0..total_samples {
                ...
                let bird1 = if (t * 0.3).fract() < 0.02 {
                    (t * 2200.0 * std::f64::consts::TAU).sin() * 0.15
                ...
                let crickets = (t * 4500.0 * std::f64::consts::TAU).sin()
                    * (t * 7.0 * std::f64::consts::TAU).sin().abs() * 0.04;
        ...
        "urban" | "city" => {
            for i in 0..total_samples {
                let t = i as f64 / sr;
                let traffic = pseudo_noise(i) * 0.15
                    * (1.0 + (t * 0.02 * std::f64::consts::TAU).sin() * 0.3);
