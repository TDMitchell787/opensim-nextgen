...
use super::{ZitiNetworkManager, ZitiProtocol, ZitiService, ZitiServiceType};
...
impl OpenSimZitiIntegration {
    ...
    pub async fn new(config: OpenSimZitiConfig) -> Result<Self> {
        let ziti_manager = Arc::new(RwLock::new(ZitiNetworkManager::new(
            config.ziti_config.clone(),
        )?));
    ...
    pub async fn integrate_session_manager(
        &mut self,
    ...
    pub async fn integrate_region_manager(
        &mut self,
    ...
    pub async fn integrate_asset_manager(
        &mut self,
    ...
    pub async fn create_secure_connection(
        ...
        let connection_id = ziti_manager
            .connect_to_service(target_service, &identity_id)
            .await?;

        tracing::info!(
            "Created secure connection for {:?} to service {}: {}",
            component_type,
            target_service,
            connection_id
        );
    ...
    async fn setup_default_services(&mut self) -> Result<()> {
        ...
        tracing::info!(
            "Set up {} default OpenSim zero trust services",
            services.len()
        );
        Ok(())
    }
    ...
    async fn create_region_service(&self) -> Result<OpenSimZitiService> {
        Ok(OpenSimZitiService {
            ...
            access_rules: vec![OpenSimAccessRule {
    ...
    async fn apply_security_policies(
        ...
        match component_type {
            ...
            _ => {
                ...
                tracing::debug!(
                    "Applying default security policies for {:?}",
                    component_type
                );
            }
        }
...
impl Default for OpenSimZitiConfig {
    fn default() -> Self {
        ...
        default_security_levels.insert(
            OpenSimComponentType::UserAuthentication,
            OpenSimSecurityLevel::System,
        );
        default_security_levels.insert(
            OpenSimComponentType::RegionCommunication,
            OpenSimSecurityLevel::System,
        ...
        default_security_levels.insert(
            OpenSimComponentType::AssetTransfer,
            OpenSimSecurityLevel::User,
        );
        default_security_levels.insert(
            OpenSimComponentType::Administration,
            OpenSimSecurityLevel::Administrator,
        );
        default_security_levels.insert(
            OpenSimComponentType::WebSocketService,
            OpenSimSecurityLevel::User,
        );
