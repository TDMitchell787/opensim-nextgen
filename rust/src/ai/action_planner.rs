...
use crate::mesh::encoder::{generate_box_mesh, generate_cylinder_mesh, generate_sphere_mesh, generate_torus_mesh};
use crate::media::luxor::{LuxorDirector, ScreenSize, OutputSettings, OutputFormat, SnapshotRequest, VideoRequest};
use crate::media::luxor::camera::{CameraRig, CameraPreset};
...
impl ActionPlanner {
    ...
    fn remap_id(llm_id: u32, created_ids: &[(String, u32)]) -> u32 {
        if llm_id >= 1 && (llm_id as usize) <= created_ids.len() {
            ...
            info!("[ACTION_PLANNER] Remapped LLM id {} → real local_id {} (\"{}\")",
                llm_id, real_id, created_ids[(llm_id as usize) - 1].0);
            real_id
        } else {
            llm_id
        }
    ...
    pub async fn execute_response(&self, npc_id: Uuid, response: &NPCResponse) -> Result<(Vec<(String, u32)>, Vec<String>)> {
        self.execute_response_with_session(npc_id, response, None).await
    }

    pub async fn execute_response_with_session(&self, npc_id: Uuid, response: &NPCResponse, session_object_uuids: Option<Vec<Uuid>>) -> Result<(Vec<(String, u32)>, Vec<String>)> {
            ...
            match action {
                NPCAction::RezBox { position, scale, name } => {
                    match self.bridge.rez_box(npc_id, *position, *scale, name).await {
                        Ok(local_id) => { created_ids.push((name.clone(), local_id)); }
                        Err(e) => { let msg = format!("Failed to create '{}': {}", name, e); info!("[ACTION_PLANNER] {}", msg); failures.push(msg); }
                    }
                }
                NPCAction::RezCylinder { position, scale, name } => {
                    match self.bridge.rez_cylinder(npc_id, *position, *scale, name).await {
                        Ok(local_id) => { created_ids.push((name.clone(), local_id)); }
                        Err(e) => { let msg = format!("Failed to create '{}': {}", name, e); info!("[ACTION_PLANNER] {}", msg); failures.push(msg); }
                    }
                }
                NPCAction::RezSphere { position, scale, name } => {
                    match self.bridge.rez_sphere(npc_id, *position, *scale, name).await {
                        Ok(local_id) => { created_ids.push((name.clone(), local_id)); }
                        Err(e) => { let msg = format!("Failed to create '{}': {}", name, e); info!("[ACTION_PLANNER] {}", msg); failures.push(msg); }
                    }
                }
                NPCAction::RezTorus { position, scale, name } => {
                    match self.bridge.rez_torus(npc_id, *position, *scale, name).await {
                        Ok(local_id) => { created_ids.push((name.clone(), local_id)); }
                        Err(e) => { let msg = format!("Failed to create '{}': {}", name, e); info!("[ACTION_PLANNER] {}", msg); failures.push(msg); }
                    }
                }
                NPCAction::RezTube { position, scale, name } => {
                    match self.bridge.rez_tube(npc_id, *position, *scale, name).await {
                        Ok(local_id) => { created_ids.push((name.clone(), local_id)); }
                        Err(e) => { let msg = format!("Failed to create '{}': {}", name, e); info!("[ACTION_PLANNER] {}", msg); failures.push(msg); }
                    }
                }
                NPCAction::RezRing { position, scale, name } => {
                    match self.bridge.rez_ring(npc_id, *position, *scale, name).await {
                        Ok(local_id) => { created_ids.push((name.clone(), local_id)); }
                        Err(e) => { let msg = format!("Failed to create '{}': {}", name, e); info!("[ACTION_PLANNER] {}", msg); failures.push(msg); }
                    }
                }
                NPCAction::RezPrism { position, scale, name } => {
                ...
                NPCAction::SetTexture { local_id, texture_uuid } => {
                    let real_id = Self::remap_id(*local_id, &created_ids);
                ...
                NPCAction::LinkObjects { root_id, child_ids } => {
                    ...
                    let real_children: Vec<u32> = child_ids.iter()
                ...
                NPCAction::InsertScript { local_id, script_name, script_source } => {
                    let real_id = Self::remap_id(*local_id, &created_ids);
                    if let Err(e) = self.bridge.insert_script(npc_id, real_id, script_name, script_source).await {
                ...
                NPCAction::InsertTemplateScript { local_id, template_name, params } => {
                    let real_id = Self::remap_id(*local_id, &created_ids);
                    if let Some(source) = crate::ai::script_templates::apply_template(template_name, params) {
                        if let Err(e) = self.bridge.insert_script(npc_id, real_id, template_name, &source).await {
                            let msg = format!("Failed to insert template '{}': {}", template_name, e);
                            info!("[ACTION_PLANNER] {}", msg);
                            failures.push(msg);
                        }
                    } else {
                ...
                NPCAction::UpdateScript { local_id, script_name, script_source } => {
                    let real_id = Self::remap_id(*local_id, &created_ids);
                    if let Err(e) = self.bridge.update_script(npc_id, real_id, script_name, script_source).await {
                ...
                NPCAction::GiveObject { local_id, target_agent_id } => {
                    ...
                    match self.bridge.give_object(real_id, *target_agent_id).await {
                        ...
                        Err(e) => { let msg = format!("Failed to give object: {}", e); info!("[ACTION_PLANNER] {}", msg); failures.push(msg); }
                    }
                }
                NPCAction::ExportOar { region_id, filename } => {
                    let actual_region = if *region_id == Uuid::nil() { self.bridge.default_region_uuid } else { *region_id };
                    let export_uuids = if !created_ids.is_empty() {
                        let scene_objs = self.bridge.scene_objects_ref();
                        let uuids: Vec<Uuid> = created_ids.iter()
                            .filter_map(|(_, lid)| scene_objs.read().get(lid).map(|o| o.uuid))
                            .collect();
                        if uuids.is_empty() { session_object_uuids.clone() } else { Some(uuids) }
                    } else {
                        session_object_uuids.clone()
                    };
                    match self.bridge.export_oar(actual_region, filename, export_uuids).await {
                        Ok(msg) => info!("[ACTION_PLANNER] OAR export: {}", msg),
                        Err(e) => { let msg = format!("Failed to export OAR: {}", e); info!("[ACTION_PLANNER] {}", msg); failures.push(msg); }
                    }
                }
                NPCAction::RezMesh { geometry_type, position, scale, name } => {
                    let gt = geometry_type.to_lowercase();
                    let blender_template = match gt.as_str() {
                        "table" | "chair" | "shelf" | "arch" | "staircase" |
                        "stone" | "stone_ring" | "boulder" | "column" => Some(gt.as_str().to_string()),
                        "path" | "walkway" | "cobblestone" | "cobblestone_path" | "serpentine_path" => Some("path".to_string()),
                        _ => None,
                    };
                    if let Some(template) = blender_template {
                        ...
                        match self.bridge.blender_generate(npc_id, &template, &params, name, *position).await {
                            Ok(local_id) => { created_ids.push((name.clone(), local_id)); }
                            Err(e) => { let msg = format!("Failed to generate '{}' (auto-template '{}'): {}", name, template, e); info!("[ACTION_PLANNER] {}", msg); failures.push(msg); }
                        }
                    } else {
                        ...
                        match self.bridge.rez_mesh(npc_id, geometry, *position, *scale, name).await {
                            Ok(local_id) => { created_ids.push((name.clone(), local_id)); }
                            Err(e) => { let msg = format!("Failed to create mesh '{}': {}", name, e); info!("[ACTION_PLANNER] {}", msg); failures.push(msg); }
                        }
                    }
                }
                NPCAction::ImportMesh { file_path, name, position } => {
                    match self.bridge.import_mesh(npc_id, file_path, name, *position).await {
                        Ok(local_id) => { created_ids.push((name.clone(), local_id)); }
                        Err(e) => { let msg = format!("Failed to import mesh '{}': {}", name, e); info!("[ACTION_PLANNER] {}", msg); failures.push(msg); }
                    }
                }
                NPCAction::BlenderGenerate { template, params, name, position } => {
                    match self.bridge.blender_generate(npc_id, template, params, name, *position).await {
                        Ok(local_id) => { created_ids.push((name.clone(), local_id)); }
                        Err(e) => { let msg = format!("Failed to generate '{}' (template '{}'): {}", name, template, e); info!("[ACTION_PLANNER] {}", msg); failures.push(msg); }
                    }
                }
                NPCAction::CreateTShirt { target_agent_id, logo_path, shirt_color, front_offset_inches, back_offset_inches, sleeve_length, fit, collar, name } => {
                    match self.bridge.create_tshirt(
                        npc_id, *target_agent_id, logo_path, *shirt_color,
                        *front_offset_inches, *back_offset_inches, *sleeve_length,
                        fit, collar, name,
                    ).await {
                        Ok(msg) => info!("[ACTION_PLANNER] Create T-shirt: {}", msg),
                        Err(e) => { let msg = format!("Failed to create T-shirt '{}': {}", name, e); info!("[ACTION_PLANNER] {}", msg); failures.push(msg); }
                    }
                }
                NPCAction::PackageObjectIntoPrim { source_local_id, container_local_id } => {
                    let real_src = Self::remap_id(*source_local_id, &created_ids);
                    let real_dst = Self::remap_id(*container_local_id, &created_ids);
                    match self.bridge.package_object_into_prim(real_src, real_dst).await {
                        Ok(msg) => info!("[ACTION_PLANNER] Package object: {}", msg),
                        Err(e) => { let msg = format!("Failed to package object: {}", e); info!("[ACTION_PLANNER] {}", msg); failures.push(msg); }
                    }
                }
                NPCAction::GiveToRequester { local_id } => {
                    ...
                    match self.bridge.give_object(real_id, npc_id).await {
                        ...
                        Err(e) => { let msg = format!("Failed to give to requester: {}", e); info!("[ACTION_PLANNER] {}", msg); failures.push(msg); }
                    }
                }
                NPCAction::DroneCinematography { scene_name, shot_type, camera_waypoints, lights, lighting_preset, subject_position, speed } => {
                    match self.bridge.setup_cinematography(
                        npc_id, scene_name, shot_type,
                        camera_waypoints.clone(), lights.clone(),
                        lighting_preset.clone(),
                        *subject_position, *speed,
                    ).await {
                        Ok(ids) => {
                            for (name, lid) in &ids {
                                created_ids.push((name.clone(), *lid));
                            }
                            info!("[ACTION_PLANNER] Cinematography scene '{}' set up: {} objects", scene_name, ids.len());
                        }
                        Err(e) => {
                            let msg = format!("Failed to set up cinematography '{}': {}", scene_name, e);
                            info!("[ACTION_PLANNER] {}", msg);
                            failures.push(msg);
                        }
                    }
                    ...
                    let scene = capture_scene(self.bridge.scene_objects_ref(), region_id, terrain_hm);
                    info!("[LUXOR] Drone render: {} prims, terrain={} for '{}'",
                        scene.prims.len(), scene.terrain.is_some(), scene_name);
                    ...
                    let video_job = match shot_type.to_lowercase().as_str() {
                        ...
                        "dolly" | "push" | "pull" | "flyby" => {
                            let start = [subject_position[0] - 15.0, subject_position[1], subject_position[2] + 5.0];
                            let end = [subject_position[0] + 15.0, subject_position[1], subject_position[2] + 5.0];
                    ...
                    let dist = 12.0;
                    let lighting_rig = lighting_preset.as_ref()
                ...
                NPCAction::TerrainGenerate { preset, seed, scale, roughness, water_level, region_id, grid_size, grid_x, grid_y } => {
                    match self.bridge.generate_terrain(
                        npc_id, preset, *seed, *scale, *roughness, *water_level,
                        region_id.as_deref(), *grid_size, *grid_x, *grid_y,
                    ).await {
                ...
                NPCAction::TerrainLoadImage { file_path, height_min, height_max } => {
                    match self.bridge.load_terrain_image(npc_id, file_path, *height_min, *height_max).await {
                        Ok(msg) => info!("[ACTION_PLANNER] {}", msg),
                        Err(e) => {
                ...
                NPCAction::TerrainPreview { preset, seed, scale, roughness, water_level, region_id, grid_size, grid_x, grid_y } => {
                    match self.bridge.preview_terrain(
                        npc_id, preset, *seed, *scale, *roughness, *water_level,
                        region_id.as_deref(), *grid_size, *grid_x, *grid_y,
                    ).await {
                        Ok(msg) => info!("[ACTION_PLANNER] {}", msg),
                ...
                NPCAction::BuildVehicle { recipe, position, tuning } => {
                    let builder = crate::ai::vehicle_builder::VehicleBuilder::new(&self.bridge);
                    match builder.build_vehicle(npc_id, recipe, *position, tuning, npc_id).await {
                        Ok(result) => {
                ...
                NPCAction::ModifyVehicle { root_id, tuning } => {
                    ...
                    for recipe_name in &recipes {
                        if let Some(recipe) = crate::ai::vehicle_recipes::get_recipe(recipe_name) {
                            if let Some(source) = scripts.get_script_with_tuning(recipe.root_script, tuning) {
                                match self.bridge.update_script(npc_id, *root_id, recipe.root_script, &source).await {
                                    Ok(_) => {
                    ...
                    if !found {
                        ...
                        for recipe_name in &recipes {
                            if let Some(recipe) = crate::ai::vehicle_recipes::get_recipe(recipe_name) {
                                let tuning_str: std::collections::HashMap<String, String> = tuning.iter()
                                    .map(|(k, v)| (k.clone(), format!("{}", v)))
                                    .collect();
                                let template_name = recipe.root_script.trim_end_matches(".lsl");
                                if let Some(source) = crate::ai::script_templates::apply_template(template_name, &tuning_str) {
                                    if self.bridge.update_script(npc_id, *root_id, recipe.root_script, &source).await.is_ok() {
                    ...
                    if !found {
                        let msg = format!("Could not modify vehicle at local_id {}: no matching script found", root_id);
                        info!("[ACTION_PLANNER] {}", msg);
                        failures.push(msg);
                    }
                }
                NPCAction::FindNearbyObject { name, radius } => {
                    ...
                    let center = self.bridge.get_avatar_position(npc_id)
                        .unwrap_or([128.0, 128.0, 25.0]);
                    let results = self.bridge.find_objects_near(center, search_radius, name.as_deref());
                    ...
                    if results.is_empty() {
                        let msg = format!("No objects found near [{:.0},{:.0},{:.0}] within {}m{}",
                            center[0], center[1], center[2], search_radius,
                            name.as_ref().map(|n| format!(" matching '{}'", n)).unwrap_or_default());
                        info!("[ACTION_PLANNER] {}", msg);
                        failures.push(msg);
                    }
                }
                NPCAction::ScanLinkset { root_id } => {
                    ...
                    match self.bridge.scan_linkset(real_id) {
                        Ok(links) => {
                            info!("[ACTION_PLANNER] ScanLinkset root_id={} → {} prims:", real_id, links.len());
                ...
                NPCAction::ScriptLinkset { root_id, script_map, root_script } => {
                    let real_id = Self::remap_id(*root_id, &created_ids);
                    let scripts = crate::ai::vehicle_scripts::VehicleScriptLibrary::global();
                    match self.bridge.scan_linkset(real_id) {
                        Ok(links) => {
                            info!("[ACTION_PLANNER] ScriptLinkset: {} prims, {} script mappings",
                                links.len(), script_map.len());
                                ...
                                for (match_name, script_name) in script_map.iter() {
                                    if name_lower.contains(&match_name.to_lowercase()) {
                                        if let Some(source) = scripts.get_script(script_name) {
                                            ...
                                            let msg = format!("Script '{}' not found in library for prim '{}'", script_name, prim_name);
                                            info!("[ACTION_PLANNER] {}", msg);
                                            failures.push(msg);
                                        }
                                    }
                        ...
                        Err(e) => {
                            let msg = format!("Failed to scan linkset for scripting root_id={}: {}", real_id, e);
                            info!("[ACTION_PLANNER] {}", msg);
                            failures.push(msg);
                        }
                    }
                }
                NPCAction::AddToLinkset { root_id, new_prim_ids } => {
                    let real_root = Self::remap_id(*root_id, &created_ids);
                    let real_ids: Vec<u32> = new_prim_ids.iter()
                        .map(|id| Self::remap_id(*id, &created_ids))
                        .collect();
                    match self.bridge.add_to_linkset(real_root, &real_ids).await {
                        Ok(assignments) => {
                            for (local_id, link_num) in &assignments {
                                info!("[ACTION_PLANNER] add_to_linkset: prim {} → link {} in root {}",
                ...
                NPCAction::LuxorSnapshot { preset, size, quality, effects, lighting, subject_position, name } => {
                    let region_id = self.bridge.default_region_uuid;
                    let terrain_hm = self.bridge.get_terrain_heightmap(region_id).await;
                    let scene = capture_scene(self.bridge.scene_objects_ref(), region_id, terrain_hm);
                    info!("[LUXOR] Scene captured: {} prims, terrain={} for snapshot '{}'",
                        scene.prims.len(), scene.terrain.is_some(), name);

                    let mut camera = CameraRig::default();
                    camera.position = [subject_position[0] - 5.0, subject_position[1] - 5.0, subject_position[2] + 3.0];
                    ...
                    let dist = 10.0;
                    let lighting_rig = lighting.as_ref()
                        .and_then(|l| LightingPreset::from_name(l))
                        .unwrap_or(LightingPreset::Studio3Point)
                        .build_rig(*subject_position, dist);

                    let render_quality = quality.as_ref()
                        .and_then(|q| RenderQuality::from_name(q))
                        .unwrap_or(RenderQuality::Standard);
                    let screen_size = size.as_ref()
                        .and_then(|s| ScreenSize::from_name(s))
                        .unwrap_or(ScreenSize::FullHD);
                    let post_effects: Vec<PostEffect> = effects.iter()
                ...
                NPCAction::LuxorVideo { shot_type, duration, fps, size, quality, effects, lighting, subject_position, name } => {
                    let region_id = self.bridge.default_region_uuid;
                    let terrain_hm = self.bridge.get_terrain_heightmap(region_id).await;
                    let scene = capture_scene(self.bridge.scene_objects_ref(), region_id, terrain_hm);
                    info!("[LUXOR] Scene captured: {} prims, terrain={} for video '{}'",
                        scene.prims.len(), scene.terrain.is_some(), name);
                    ...
                    let video_job = match shot_type.to_lowercase().as_str() {
                        ...
                        "dolly" | "push" | "pull" => {
                            let start = [subject_position[0] - 10.0, subject_position[1], subject_position[2] + 2.0];
                            let end = [subject_position[0] + 10.0, subject_position[1], subject_position[2] + 2.0];
                ...
                NPCAction::ImportFloorplan { image_path, position, wall_height, scale } => {
                    info!("[ACTION_PLANNER] ImportFloorplan: {} at {:?} (wall_h={:?}, scale={:?})",
                        image_path, position, wall_height, scale);
                    match self.execute_floorplan_build(npc_id, image_path, position, wall_height, scale, &mut created_ids).await {
                ...
                NPCAction::ImportElevation { image_path, position } => {
                    info!("[ACTION_PLANNER] ImportElevation: {} at {:?}", image_path, position);
                }
                NPCAction::ImportBlueprint { floorplan_path, elevation_path, position, wall_height, scale } => {
                    info!("[ACTION_PLANNER] ImportBlueprint: floor={}, elev={:?} at {:?}",
                        floorplan_path, elevation_path, position);
                    match self.execute_floorplan_build(npc_id, floorplan_path, position, wall_height, scale, &mut created_ids).await {
                        Ok(count) => info!("[ACTION_PLANNER] Blueprint floor plan built: {} prims", count),
                        Err(e) => {
        ...
        if !created_ids.is_empty() {
            info!("[ACTION_PLANNER] NPC created {} objects: {:?}", created_ids.len(), created_ids);
        }
        if !failures.is_empty() {
            info!("[ACTION_PLANNER] {} action(s) failed: {:?}", failures.len(), failures);
        }
    ...
    async fn execute_floorplan_build(
        ...
        use crate::ai::image_to_build::{analyze_floorplan, floorplan_to_actions, BuildConfig, detect_media_type};

        let llm = self.llm_client.as_ref()
            .ok_or_else(|| anyhow::anyhow!("No LLM client available for vision analysis — set provider in llm.ini"))?;

        let image_data = tokio::fs::read(image_path).await
            .map_err(|e| anyhow::anyhow!("Cannot read floor plan image '{}': {}", image_path, e))?;

        let media_type = detect_media_type(image_path);
        let plan = analyze_floorplan(llm, &image_data, media_type).await
        ...
        for action in &build_actions {
            match action {
                NPCAction::RezBox { position: pos, scale: sc, name } => {
                    match self.bridge.rez_box(npc_id, *pos, *sc, name).await {
                        Ok(local_id) => { created_ids.push((name.clone(), local_id)); }
                        Err(e) => { info!("[ACTION_PLANNER] Failed to rez {}: {}", name, e); }
                    }
                }
                NPCAction::RezPrism { position: pos, scale: sc, name } => {
        ...
        if floorplan_ids.len() > 1 {
            ...
            if let Err(e) = self.bridge.link_objects(root_id, &child_ids).await {
                ...
                info!("[ACTION_PLANNER] Linked floorplan: root={} + {} children", root_id, child_ids.len());
            }
        }
