...
use crate::session::{SessionManager, LoginSession};
use crate::xmlrpc::{XmlRpcParser, XmlRpcResponseGenerator, LoginRequest, LoginResponse};
...
use crate::config::login::{LoginConfigManager, LoginConfig};
use crate::inventory::LoginInventoryService;
use crate::capabilities::{CapabilitiesManager, CapabilitiesConfig};
...
impl LoginService {
    pub fn new(
        ...
        info!("Initializing Login Service with config summary: {:?}", summary);
        ...
        Self {
            ...
            capabilities_manager: Arc::new(std::sync::Mutex::new(CapabilitiesManager::new(capabilities_config))),
            caps_manager: None,
            udp_server: None,
    ...
    pub fn with_defaults(
        ...
        Self::new(config_manager, session_manager, user_database, avatar_service)
    }
    ...
    pub fn from_config_file(
        ...
        Ok(Self::new(config_manager, session_manager, user_database, avatar_service))
    }
    ...
    pub fn set_uas_service(&mut self, uas: Arc<dyn crate::services::traits::UserAgentServiceTrait>) {
        self.uas_service = Some(uas);
    }

    pub fn set_gatekeeper_service(&mut self, gk: Arc<dyn crate::services::traits::GatekeeperServiceTrait>) {
        self.gatekeeper_service = Some(gk);
    }

    pub async fn start_udp_server(&mut self, db_connection: Option<Arc<DatabaseConnection>>) -> Result<()> {
        let config = self.config_manager.config();
        info!("Starting UDP server on port {}", config.server.udp_port);

        let stage_tracker = self.stage_tracker.clone()
    ...
    pub async fn handle_login_request(&self, request_body: String) -> Result<Response<Body>> {
        ...
        debug!("🔍 XMLRPC TRACE: Request body length: {} bytes", request_body.len());
        ...
        info!("Login attempt: first='{}' last='{}' (bytes: {:02x?} {:02x?})",
              login_request.first, login_request.last,
              login_request.first.as_bytes(), login_request.last.as_bytes());
        ...
        match self.authenticate_user(&login_request).await {
            Ok(session) => {
                info!("Successful login for {} {} (circuit: {})",
                      session.first_name, session.last_name, session.circuit_code);

                debug!("[Phase 68] Login IDs: agent={} session={}", session.agent_id, session.session_id);

                let config = self.config_manager.config();

                // Load or create avatar appearance
                // Matches: OpenSim LLLoginService.cs:588-592
                debug!("🔍 TRACE: Loading avatar appearance for agent_id: {}", session.agent_id);
                match self.avatar_service.get_or_create_default_appearance(session.agent_id).await {
                    Ok(avatar) => {
                        info!("[AVATAR SERVICE]: Avatar appearance loaded for {} (Serial: {})",
                              session.agent_id, avatar.data.get("Serial").unwrap_or(&"unknown".to_string()));
                ...
                if let Some(ref db_pool) = self.db_pool {
                    match crate::database::default_inventory::ensure_user_has_inventory(db_pool, session.agent_id).await {
                        Ok(true) => info!("[INVENTORY] Created missing default inventory for agent {}", session.agent_id),
                        Ok(false) => {},
                        Err(e) => warn!("[INVENTORY] ensure_user_has_inventory failed for {}: {}", session.agent_id, e),
                    }
                }
                let inventory = if let Some(ref db_pool) = self.db_pool {
                    info!("[INVENTORY] Querying PostgreSQL for inventory folders for agent {}", session.agent_id);
                    match self.query_inventory_from_database(db_pool, session.agent_id).await {
                        Ok(inv) => {
                            info!("[INVENTORY] Loaded {} skeleton folders from DATABASE for agent {}", inv.folder_count(), session.agent_id);
                            for folder in &inv.inventory_skeleton {
                                debug!("[INVENTORY]   folder: {} type={} id={} parent={}", folder.name, folder.type_default, folder.folder_id, folder.parent_id);
                            }
                ...
                info!("[INVENTORY] Inventory skeleton ready with {} folders", inventory.inventory_skeleton.len());
                ...
                let seed_capability_url = {
                    ...
                    match caps_manager.create_agent_capabilities(session.agent_id, session.session_id) {
                        Ok(seed_url) => {
                            debug!("🔍 TRACE: create_agent_capabilities succeeded: {}", seed_url);
                            seed_url
                        }
                        Err(e) => {
                ...
                let final_seed_capability_url = if let Some(caps_manager) = &self.caps_manager {
                    ...
                    let caps_session_id = caps_manager.create_session(
                ...
                debug!("🔍 TRACE: Creating LoginResponse::success with seed capability: {}", final_seed_capability_url);
                let mut response = LoginResponse::success(&session, &config.server.base_url, &inventory, &final_seed_capability_url);
                ...
                let xml = XmlRpcResponseGenerator::generate_login_response(&response)?;
                info!("[LOGIN] XMLRPC response ready ({} bytes, {} folders, circuit={})",
                      xml.len(), inventory.inventory_skeleton.len(), session.circuit_code);

                if let Some(tracker) = &self.stage_tracker {
                    let session_id_str = session.session_id.to_string();
                    tracker.create_session(&session_id_str, session.circuit_code).await;
                    tracker.mark_passed(
                        &session_id_str,
                        crate::login_stage_tracker::LoginStage::XmlRpcSent,
                        Some(format!("circuit_code={} agent={}", session.circuit_code, session.agent_id))
                    ).await;
                }
            ...
            Err(e) => {
                warn!("Login failed for {} {}: {}", login_request.first, login_request.last, e);
    ...
    pub fn get_inventory_cache_stats(&self) -> Result<crate::inventory::InventoryCacheStats> {
        let inv_service = self.inventory_service.lock()
    ...
    pub fn clear_inventory_cache(&self, user_id: Uuid) -> Result<()> {
        let mut inv_service = self.inventory_service.lock()
            .map_err(|_| anyhow!("Failed to lock inventory service"))?;
    ...
    async fn query_inventory_from_database(
        ...
        debug!("[INVENTORY] Phase 121: Loading inventory from database for agent {}", agent_id);

        let db_folders = crate::inventory::folders::lookup_database_folder_uuids(db_pool, agent_id).await;

        if let Some(ref folders) = db_folders {
            info!("[INVENTORY] Found {} folder UUIDs in database", folders.len());
        } else {
            debug!("[INVENTORY] No folders in database, will use generated UUIDs");
        }

        let mut inventory = crate::inventory::UserInventory::create_with_db_folders(agent_id, db_folders.as_ref());
        ...
        for row in &all_db_folders {
            ...
            let folder = crate::inventory::InventoryFolder {
                id: folder_id,
                parent_id: if parent_id.is_nil() { None } else { Some(parent_id) },
                owner_id: agent_id,
                name,
        ...
        if added_folders > 0 {
            info!("[INVENTORY] Added {} user-created folders to login skeleton", added_folders);
        }

        let cof_id = inventory.folders.values()
        ...
        if let Some(cof_id) = cof_id {
            ...
            if !cof_items.is_empty() {
                ...
                info!("[INVENTORY] Loaded {} COF items from database (replaced hardcoded defaults)", cof_items.len());
            }
        }

        let bodypart_id = inventory.folders.values()
            .find(|f| f.folder_type == crate::inventory::InventoryFolderType::BodyPart)
            .map(|f| f.id);
        let clothing_id = inventory.folders.values()
        ...
        for folder_id in [bodypart_id, clothing_id].iter().flatten() {
            ...
            if !db_items.is_empty() {
                inventory.items.retain(|_, item| item.folder_id != *folder_id);
                for row in &db_items {
                    if let Ok(item) = Self::inventory_item_from_row(row, agent_id) {
                        inventory.add_item(item);
                    }
                }
                info!("[INVENTORY] Loaded {} items from {} folder", db_items.len(),
                    if *folder_id == bodypart_id.unwrap_or(Uuid::nil()) { "Body Parts" } else { "Clothing" });
            }
        }

        info!("[INVENTORY] Final login inventory: {} folders, {} items", inventory.folders.len(), inventory.items.len());

        Ok(inventory.to_login_response())
    }

    fn inventory_item_from_row(row: &sqlx::postgres::PgRow, agent_id: Uuid) -> Result<crate::inventory::InventoryItem> {
        use sqlx::Row;
        ...
        let creator_id: String = row.try_get("creatorid").unwrap_or_else(|_| agent_id.to_string());
        let flags: i32 = row.try_get("flags").unwrap_or(0);
        let base_perms: i32 = row.try_get("inventorybasepermissions").unwrap_or(0x7FFFFFFF);
        let current_perms: i32 = row.try_get("inventorycurrentpermissions").unwrap_or(0x7FFFFFFF);
        ...
        let next_perms: i32 = row.try_get("inventorynextpermissions").unwrap_or(0x7FFFFFFF);
        ...
        let mut item = crate::inventory::InventoryItem::new_with_id(
            id, asset_id, folder_id, agent_id, creator_uuid,
            name, description, inv_asset_type, flags as u32,
        );
...
pub async fn handle_grid_info_json() -> Response<Body> {
    ...
    let json = format!(
        ...
        grid_name.replace('"', "\\\""), grid_nick.replace('"', "\\\""),
        home_uri, home_uri, home_uri, home_uri, home_uri, home_uri, home_uri,
        gatekeeper_uri, gatekeeper_uri
    );
    ...
    if let Some(ref tracker) = login_service.readiness_tracker {
        if !tracker.is_login_ready() {
            let pending: Vec<&str> = tracker.status_breakdown().iter()
                ...
                .map(|(name, _)| *name)
                .collect();
            warn!("[LOGIN] Rejected login — server not ready. Pending: {:?}", pending);
            let xml = format!(
                r#"<?xml version="1.0"?>
<methodResponse><params><param><value><struct>
  ...
  <member><name>message</name><value><string>Region is starting up ({} services pending). Please wait 30 seconds and try again.</string></value></member>
    ...
    if !is_valid_content_type {
        warn!("Unexpected content type for login request: {} (body starts with: {})",
              content_type,
    ...
    if let Some(method_name) = extract_xmlrpc_method_name(&body) {
        ...
        if gk_methods.contains(&method_name.as_str()) {
            if let Some(ref gk) = login_service.gatekeeper_service {
                info!("[HG GK] XmlRpc method '{}' received at root path — dispatching to gatekeeper", method_name);
                ...
                warn!("[HG GK] XmlRpc method '{}' received but no gatekeeper service configured", method_name);
            }
        }

        let uas_methods = [
            "verify_agent", "verify_client", "get_home_region", "get_server_urls",
            "logout_agent", "get_uui", "get_uuid", "status_notification",
            "get_online_friends", "agent_is_coming_home", "get_user_info",
        ...
        if uas_methods.contains(&method_name.as_str()) {
            if let Some(ref uas) = login_service.uas_service {
                ...
                warn!("[HG UAS] XmlRpc method '{}' received but no UAS service configured", method_name);
            }
        }
        ...
        if method_name == "get_grid_info" {
            ...
            grid_info.insert("gridname".to_string(),
                crate::services::robust::xmlrpc::XmlRpcValue::String(grid_name));
            grid_info.insert("gridnick".to_string(),
                crate::services::robust::xmlrpc::XmlRpcValue::String(grid_nick));
            grid_info.insert("login".to_string(),
                crate::services::robust::xmlrpc::XmlRpcValue::String(format!("{}/", home_uri)));
            grid_info.insert("welcome".to_string(),
                crate::services::robust::xmlrpc::XmlRpcValue::String(format!("{}/welcome", home_uri)));
            grid_info.insert("economy".to_string(),
                crate::services::robust::xmlrpc::XmlRpcValue::String(format!("{}/", home_uri)));
            grid_info.insert("about".to_string(),
                crate::services::robust::xmlrpc::XmlRpcValue::String(format!("{}/welcome", home_uri)));
            grid_info.insert("register".to_string(),
                crate::services::robust::xmlrpc::XmlRpcValue::String(format!("{}/", home_uri)));
            grid_info.insert("help".to_string(),
                crate::services::robust::xmlrpc::XmlRpcValue::String(format!("{}/", home_uri)));
            grid_info.insert("password".to_string(),
                crate::services::robust::xmlrpc::XmlRpcValue::String(format!("{}/", home_uri)));
            grid_info.insert("gatekeeper".to_string(),
                crate::services::robust::xmlrpc::XmlRpcValue::String(format!("{}/", gatekeeper_uri)));
            grid_info.insert("uas".to_string(),
                crate::services::robust::xmlrpc::XmlRpcValue::String(format!("{}/", gatekeeper_uri)));
            grid_info.insert("platform".to_string(),
                crate::services::robust::xmlrpc::XmlRpcValue::String("OpenSim".to_string()));
            grid_info.insert("version".to_string(),
        ...
        if method_name != "login_to_simulator" {
            warn!("[HG] Unrecognized XMLRPC method '{}' at root path (body_len={}, body_preview={})",
                method_name, body.len(), &body[..std::cmp::min(body.len(), 500)]);
        }
    }
...
async fn handle_gatekeeper_xmlrpc_at_root(
    ...
    let param = params.first().cloned().unwrap_or(XmlRpcValue::Struct(std::collections::HashMap::new()));
    ...
    match method {
        ...
        "get_region" => {
            ...
            match gk.get_hyperlinkregion(region_id, agent_id, agent_home).await {
                Ok(Some(info)) => {
...
async fn handle_uas_xmlrpc_at_root(
    ...
    match method {
        "verify_agent" => {
            ...
            info!("[HG UAS] verify_agent: session={}, token={}", session_id, token);
        ...
        "get_user_info" => {
            ...
            match uas.get_user_info(user_id).await {
                Ok(info) => {
                    let mut result = std::collections::HashMap::new();
                    result.insert("result".to_string(), XmlRpcValue::String("success".to_string()));
                    for (k, v) in info {
                        result.insert(k, XmlRpcValue::String(v));
                    }
                    build_xmlrpc_response(&XmlRpcValue::Struct(result))
                ...
                Err(e) => {
                    ...
                    let mut result = std::collections::HashMap::new();
                    result.insert("result".to_string(), XmlRpcValue::String("failure".to_string()));
                    build_xmlrpc_response(&XmlRpcValue::Struct(result))
                }
            }
        }
        ...
        "avatar_properties_request" => {
            let avatar_id = param.get_str("avatar_id")
                .and_then(|s| uuid::Uuid::parse_str(s).ok())
                .unwrap_or_default();
            info!("[HG UAS] avatar_properties_request: avatar_id={}", avatar_id);
...
pub async fn handle_splash_image() -> Response<Body> {
    let png_data: &[u8] = &[
        ...
        0x9C, 0x00, 0x00, 0x02, 0x00, 0x49, 0x44, 0x41, 0x54, 0x78, 0x9C, 0xED, 0xD2, 0x31, 0x0E, 0x00,
        0x20, 0x08, 0x03, 0xD0, 0xF2, 0xFF, 0x3F, 0x8D, 0x0B, 0x0B, 0xA2, 0x24, 0x4E, 0x12, 0x77, 0x9B,
        0x60, 0x90, 0xB6, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        ...
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xF8, 0x77, 0x01, 0xE9, 0x09, 0x7F, 0x31, 0x3B,
        0x6C, 0x74, 0xBC, 0x00, 0x00, 0x00, 0x00, 0x49, 0x45, 0x4E, 0x44, 0xAE, 0x42, 0x60, 0x82,
    ];
...
pub async fn handle_currency_php(
    ...
    let agent_id = extract_xmlrpc_string(&body, "agentId")
    ...
    let xml = match method {
        "getCurrencyQuote" => {
            ...
            info!("[CURRENCY] getCurrencyQuote: agent={:?} balance={} currencyBuy={}", agent_id, balance, currency_buy);
...
</methodResponse>"#, currency_buy)
        }
        "buyCurrency" => {
            let currency_buy = extract_xmlrpc_int(&body, "currencyBuy").unwrap_or(0);
            info!("[CURRENCY] buyCurrency: agent={:?} amount={}", agent_id, currency_buy);
  ...
  <params>
    <param><value><struct>
      <member><name>success</name><value><boolean>1</boolean></value></member>
        ...
        "preflightBuyLandPrep" => {
            info!("[CURRENCY] preflightBuyLandPrep: agent={:?} balance={}", agent_id, balance);
            format!(r#"<?xml version="1.0"?>
<methodResponse>
  <params>
    <param><value><struct>
...
pub async fn handle_landtool_php(
...
mod tests {
    ...
    async fn test_login_service_creation() {
        ...
        let db_manager = Arc::new(DatabaseManager::new("postgresql://opensim:password123@localhost/opensim_pg").await.unwrap());
        let user_database = db_manager.user_accounts();
        let avatar_service = Arc::new(AvatarService::new(db_manager.clone()));

        let login_service = LoginService::new(config_manager, session_manager, user_database, avatar_service);
