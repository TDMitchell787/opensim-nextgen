...
use super::migration_manager::{MigrationManager, DatabaseType};
...
impl DatabaseConnection {
    ...
    pub async fn migrate(&self) -> Result<()> {
        ...
        for (i, migration) in migrations.iter().enumerate() {
            debug!("Running migration {}: {}", i + 1, migration.lines().next().unwrap_or(""));
            ...
            match sqlx::query(migration).execute(&self.pool).await {
                ...
                Err(e) => {
                    ...
                    if migration_manager.should_skip_migration_error(&error_str) {
                        debug!("Migration {} skipped (already applied or compatibility): {}", i + 1, e);
                    } else {
