...
impl MetricsRegistry {
    ...
    pub async fn register_counter(
        ...
        self.register_metric(name, help, MetricType::Counter, labels)
            .await
    }
    ...
    pub async fn register_gauge(
        ...
        self.register_metric(name, help, MetricType::Gauge, labels)
            .await
    }
...
impl Timer {
    ...
    pub async fn stop(self) -> Result<Duration> {
        ...
        if let Ok(Some(metric)) = self.registry.get_metric(&self.metric_name).await {
            if metric.metric_type == MetricType::Histogram {
                self.registry
                    .observe_histogram(&self.metric_name, duration_ms)
                    .await?;
            } else if metric.metric_type == MetricType::Summary {
                self.registry
                    .observe_summary(&self.metric_name, duration_ms)
                    .await?;
            }
        }
...
impl PerformanceCollector {
    ...
    async fn register_system_metrics(&self) -> Result<()> {
        ...
        self.registry
            .register_gauge(
                "opensim_active_avatars",
                "Number of active avatars",
                labels.clone(),
            )
            .await?;
        self.registry
            .register_gauge(
                "opensim_active_connections",
        ...
        self.registry
            .register_counter(
                "opensim_events_processed_total",
    ...
    async fn collect_system_metrics(registry: &Arc<MetricsRegistry>) -> Result<()> {
        ...
        registry
            .set_gauge("system_cpu_usage_percent", cpu_usage)
            .await?;
        ...
        registry
            .set_gauge("system_memory_total_bytes", total_memory)
            .await?;
        registry
            .set_gauge("system_memory_used_bytes", used_memory)
            .await?;
        registry
            .set_gauge("system_memory_available_bytes", available_memory)
            .await?;

        // Simulated process metrics
        let process_memory = 100_000_000.0 + rand::random::<f64>() * 50_000_000.0; // 100-150MB
        registry
            .set_gauge("process_memory_rss_bytes", process_memory)
            .await?;
...
mod tests {
    ...
    async fn test_metrics_registry() -> Result<()> {
        ...
        // Test counter
        registry
            .register_counter("test_counter", "Test counter", labels.clone())
            .await?;
        ...
        // Test gauge
        registry
            .register_gauge("test_gauge", "Test gauge", labels.clone())
            .await?;
    ...
    async fn test_histogram() -> Result<()> {
        ...
        registry
            .register_histogram("test_histogram", "Test histogram", labels)
            .await?;
    ...
    async fn test_timer() -> Result<()> {
        ...
        registry
            .register_histogram("test_timer", "Test timer", labels)
            .await?;
    ...
    async fn test_prometheus_export() -> Result<()> {
        ...
        registry
            .register_counter("test_counter", "Test counter", labels)
            .await?;
