...
impl LoadBalancer {
    ...
    pub async fn register_server(&self, server: ServerInstance) -> Result<()> {
        info!("Registering server: {} at {}:{}", server.id, server.address, server.port);
        
        let mut servers = self.servers.write().await;
        servers.insert(server.id.clone(), server);
        
        info!("Server registered successfully. Total servers: {}", servers.len());
        Ok(())
    }
    ...
    pub async fn update_server_metrics(&self, server_id: &str, metrics: ServerMetrics) -> Result<()> {
    ...
    pub async fn select_server_for_region(&self, region_id: Uuid) -> Result<LoadBalancingDecision> {
        ...
        self.region_assignments.write().await.insert(region_id, assignment);
        
        info!("Selected server {} for region {} using strategy {:?}", 
    ...
    pub async fn rebalance_regions(&self) -> Result<Vec<RegionAssignment>> {
        ...
        info!("Region rebalancing completed with {} actions", rebalance_actions.len());
    ...
    pub async fn get_statistics(&self) -> Result<LoadBalancingStatistics> {
        ...
        let healthy_servers = servers.values()
            .filter(|s| s.metrics.health_status == ServerHealth::Healthy)
            .count();
        
        let total_regions = assignments.len();
        let total_connections: u32 = servers.values()
        ...
        let average_memory: f32 = if total_servers > 0 {
            servers.values().map(|s| s.metrics.memory_usage).sum::<f32>() / total_servers as f32
        } else {
            0.0
        };
    ...
    async fn select_round_robin(&self, servers: &[&ServerInstance]) -> Result<LoadBalancingDecision> {
    ...
    async fn select_least_connections(&self, servers: &[&ServerInstance]) -> Result<LoadBalancingDecision> {
        ...
        factors.insert("connections".to_string(), selected_server.metrics.active_connections as f32);
    ...
    async fn select_weighted_round_robin(&self, servers: &[&ServerInstance]) -> Result<LoadBalancingDecision> {
    ...
    async fn select_least_resource_usage(&self, servers: &[&ServerInstance]) -> Result<LoadBalancingDecision> {
        let selected_server = servers
            .iter()
            .min_by(|a, b| {
        ...
        factors.insert("memory_usage".to_string(), selected_server.metrics.memory_usage);
    ...
    async fn select_geographic(&self, servers: &[&ServerInstance], region_id: Uuid) -> Result<LoadBalancingDecision> {
    ...
    async fn select_custom(&self, servers: &[&ServerInstance], algorithm: &str) -> Result<LoadBalancingDecision> {
        match algorithm {
            ...
            _ => {
                warn!("Unknown custom algorithm '{}', falling back to least resource usage", algorithm);
    ...
    fn calculate_server_load(&self, server: &ServerInstance) -> f32 {
        ...
        let connection_load = server.metrics.active_connections as f32 / server.max_connections as f32;
    ...
    async fn migrate_regions_from_server(&self, server_id: &str) -> Result<()> {
        ...
        for assignment in regions_to_migrate {
            ...
            if let Ok(decision) = self.select_server_for_region(assignment.region_id).await {
                ...
                self.region_assignments.write().await.insert(assignment.region_id, new_assignment);
            }
        }
            ...
            match self.ping_server(server).await {
                ...
                Err(e) => {
                    ...
                    if age > chrono::Duration::minutes(5) {
                        ...
                        error!("Server {} marked as offline due to prolonged health check failures", server.id);
                    }
                }
            }
        }
    ...
    async fn get_region_location(&self, region_id: Uuid) -> String {
        ...
        match self.region_manager.get_region_location(crate::region::RegionId(region_id.as_u128() as u64)).await {
    ...
    fn calculate_geographic_proximity(&self, region_location: &str, server: &ServerInstance) -> f32 {
        match (&server.geographic_location, region_location) {
            (Some(server_location), region_loc) if !region_loc.is_empty() => {
                ...
                if server_location == region_loc {
                    1.0 // Perfect match
                } else if server_location.contains(region_loc) || region_loc.contains(server_location) {
    ...
    async fn select_hybrid_algorithm(&self, servers: &[&ServerInstance]) -> Result<LoadBalancingDecision> {
        ...
        factors.insert("server_load".to_string(), self.calculate_server_load(selected_server));
    ...
    async fn select_latency_optimized(&self, servers: &[&ServerInstance]) -> Result<LoadBalancingDecision> {
        let selected_server = servers
            .iter()
            .filter(|s| s.metrics.health_status == ServerHealth::Healthy)
    ...
    async fn select_capacity_aware(&self, servers: &[&ServerInstance]) -> Result<LoadBalancingDecision> {
        let selected_server = servers
            .iter()
            .filter(|s| s.metrics.health_status == ServerHealth::Healthy)
            .max_by(|a, b| {
                ...
                capacity_a.partial_cmp(&capacity_b).unwrap_or(std::cmp::Ordering::Equal)
            })
            .ok_or_else(|| anyhow!("No healthy servers available"))?;

        let available_capacity = (selected_server.max_connections - selected_server.metrics.active_connections) as f32 
        ...
        factors.insert("active_connections".to_string(), selected_server.metrics.active_connections as f32);
        factors.insert("active_regions".to_string(), selected_server.metrics.active_regions as f32);
        ...
        match client.get(&health_url).send().await {
            Ok(response) => {
                if response.status().is_success() {
                    ...
                    warn!("Server {} returned non-success status: {}", server.id, response.status());
