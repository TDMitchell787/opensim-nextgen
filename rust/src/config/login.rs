...
impl LoginConfigManager {
    ...
    pub fn load_from_file(file_path: &str) -> Result<Self> {
        ...
        let config: LoginConfig = if file_path.ends_with(".toml") {
            ...
            return Err(anyhow!("Unsupported config file format. Use .toml or .json"));
        };
    ...
    pub fn load_from_env() -> Result<Self> {
        ...
        if let Ok(udp_port) = std::env::var("OPENSIM_LOGIN_UDP_PORT")
            .or_else(|_| std::env::var("OPENSIM_LOGIN_PORT"))
        {
            let port: u16 = udp_port.parse()
            ...
            info!("Login port set to {} — base_url updated to {}", port, config.server.base_url);
        }
        ...
        if let Ok(bypass_auth) = std::env::var("OPENSIM_LOGIN_BYPASS_AUTH") {
            config.auth.bypass_auth_for_testing = bypass_auth.parse()
                .unwrap_or(false);
        }
        
        // Session configuration  
        if let Ok(timeout) = std::env::var("OPENSIM_LOGIN_SESSION_TIMEOUT") {
            config.session.timeout_minutes = timeout.parse()
    ...
    pub fn get_summary(&self) -> HashMap<String, String> {
        ...
        summary.insert("bind_address".to_string(), self.config.server.bind_address.clone());
        summary.insert("udp_port".to_string(), self.config.server.udp_port.to_string());
        ...
        summary.insert("max_sessions".to_string(), self.config.server.max_sessions.to_string());
        summary.insert("session_timeout".to_string(), self.config.session.timeout_minutes.to_string());
        summary.insert("grid_name".to_string(), self.config.ui.grid_name.clone());
        summary.insert("inventory_enabled".to_string(), self.config.inventory.enabled.to_string());
        summary.insert("capabilities_enabled".to_string(), self.config.capabilities.enabled.to_string());
        summary.insert("auth_bypass".to_string(), self.config.auth.bypass_auth_for_testing.to_string());
...
mod tests {
    ...
    fn test_config_validation() {
        ...
        let invalid_manager = LoginConfigManager { config: invalid_config };
    ...
    fn test_config_file_operations() {
        ...
        let loaded_manager = LoginConfigManager::load_from_file(json_path.to_str().unwrap()).unwrap();
        assert_eq!(manager.config.server.udp_port, loaded_manager.config.server.udp_port);
        ...
        let loaded_manager = LoginConfigManager::load_from_file(toml_path.to_str().unwrap()).unwrap();
        assert_eq!(manager.config.server.udp_port, loaded_manager.config.server.udp_port);
    }
