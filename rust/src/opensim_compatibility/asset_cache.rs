...
impl AssetCacheManager {
    ...
    pub fn new(config: AssetCacheConfig) -> Result<Self> {
        ...
        tracing::info!("Initialized asset cache with {} buckets at: {}", 
                      config.cache_buckets, config.cache_directory.display());
    ...
    pub async fn store_asset(&mut self, asset_id: &str, asset_type: i32, data: Vec<u8>) -> Result<()> {
    ...
    fn evict_lru_memory_cache(&mut self) {
        if let Some((lru_key, _)) = self.memory_cache.iter()
    ...
    pub async fn cleanup_cache(&mut self) -> Result<()> {
        ...
        tracing::info!("Cache cleanup completed: {} files removed, {} bytes freed", 
