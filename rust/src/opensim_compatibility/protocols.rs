...
use super::message_templates::{MessageTemplate as NewMessageTemplate, MessageTemplateManager};
...
impl LLUDPClientStack {
    ...
    pub async fn start_with_circuit_codes(
        &mut self,
        circuit_codes: crate::login_session::CircuitCodeRegistry,
    ) -> Result<()> {
    ...
    async fn validate_message_with_template(
        &self,
        message_number: u32,
        data: &[u8],
    ) -> Result<String> {
        match self
            .message_template_manager
            .get_template_by_number(message_number)
        {
            Ok(template) => {
                // Validate message structure
                if let Err(e) = self
                    .message_template_manager
                    .validate_message(&template.name, data)
                {
    ...
    fn is_critical_login_message(&self, message_name: &str) -> bool {
        self.message_template_manager
            .is_critical_message(message_name)
    }
    ...
    async fn handle_message(
        ...
        match message_num {
            ...
            15 => {
                Self::handle_request_multiple_objects(clients, socket, data, addr, sequence).await
            }
            ...
            29 => {
                Self::handle_avatar_properties_request(clients, socket, data, addr, sequence).await
            }
            30 => {
                Self::handle_avatar_interests_request(clients, socket, data, addr, sequence).await
            }
            ...
            33 => {
                Self::handle_avatar_classifieds_request(clients, socket, data, addr, sequence).await
            }
    ...
    async fn handle_use_circuit_code(
        ...
        tracing::info!(
            "UseCircuitCode received from {} with circuit code: {}",
            addr,
            circuit_code
        );
        ...
        if let Some(registry) = circuit_codes {
            ...
            match registry.validate_circuit_code(circuit_code).await {
                Some(login_session) => {
                    tracing::info!(
                        "Valid circuit code {} for user {} {}",
                        circuit_code,
                        login_session.first_name,
                        login_session.last_name
                    );
                    ...
                    tracing::info!(
                        "LLUDP client authenticated: {} {} (circuit: {})",
                        login_session.first_name,
                        login_session.last_name,
                        circuit_code
                    );
                ...
                None => {
                    ...
                    if let Some(session) = recent_session {
                        tracing::info!(
                            "Accepting circuit code {} for recent login session {} {}",
                            circuit_code,
                            session.first_name,
                            session.last_name
                        );
                        ...
                        clients.write().await.insert(addr, client);
                        tracing::info!(
                            "LLUDP client authenticated with fallback: {} {} (circuit: {})",
                            session.first_name,
                            session.last_name,
                            circuit_code
                        );
                        ...
                        // Send essential login packets immediately
                        if let Err(e) =
                            Self::send_login_sequence(&*socket, addr, circuit_code).await
                        {
                            tracing::error!("Failed to send login sequence: {}", e);
                        }
                        ...
                        tracing::error!(
                            "No recent login sessions found for circuit code {} from {}",
                            circuit_code,
                            addr
                        );
        ...
        clients.write().await.insert(addr, client);
        tracing::warn!(
            "LLUDP client connected without validation: {} (circuit: {})",
            addr,
            circuit_code
        );
    ...
    async fn handle_complete_agent_movement(
        ...
        let circuit_code = {
            ...
            if let Some(client) = clients_guard.get_mut(&addr) {
                ...
                tracing::info!(
                    "Agent movement completed for client: {} - SENDING FINAL LOGIN CONFIRMATION",
                    addr
                );
                client.circuit_code
            } else {
        ...
        let movement_complete_response =
            Self::create_agent_movement_complete_response_packet(circuit_code)?;
        socket.send_to(&movement_complete_response, addr).await?;
        tracing::info!(
            "Sent AgentMovementCompleteResponse to {} - LOGIN SHOULD BE COMPLETE!",
            addr
        );
    ...
    async fn handle_complete_ping_check(
        ...
        if let Some(client) = clients_guard.get_mut(&addr) {
            ...
            client.last_packet_time = std::time::Instant::now();
            tracing::info!(
                "Received CompletePingCheck from {}, ping/pong established",
                addr
            );
        }
        Ok(())
    ...
    async fn handle_region_handshake_reply(
        ...
        let circuit_code = {
            ...
            if let Some(client) = clients_guard.get_mut(&addr) {
                ...
                client.last_packet_time = std::time::Instant::now();
                tracing::info!(
                    "Received RegionHandshakeReply from {}, handshake confirmed!",
                    addr
                );
                client.circuit_code
            } else {
        ...
        tracing::info!(
            "Sent SimStats packet to {} after handshake confirmation",
            addr
        );
        ...
        tracing::info!(
            "Sent UUIDNameReply packet to {} after handshake confirmation",
            addr
        );
    ...
    async fn handle_agent_throttle(
        ...
        if let Some(client) = clients_guard.get_mut(&addr) {
            ...
            client.last_packet_time = std::time::Instant::now();
            tracing::info!(
                "Received AgentThrottle from {}, bandwidth settings updated",
                addr
            );
        }
        Ok(())
    ...
    async fn handle_agent_cached_texture(
        ...
        let circuit_code = {
            ...
            if let Some(client) = clients_guard.get_mut(&addr) {
                ...
                client.last_packet_time = std::time::Instant::now();
                tracing::info!(
                    "Received AgentCachedTexture from {}, responding with texture info",
                    addr
                );
                client.circuit_code
            } else {
    ...
    async fn handle_agent_wearables_request(
        ...
        let circuit_code = {
            ...
            if let Some(client) = clients_guard.get_mut(&addr) {
                ...
                client.last_packet_time = std::time::Instant::now();
                tracing::info!(
                    "Received AgentWearablesRequest from {}, sending basic wearables",
                    addr
                );
                client.circuit_code
            } else {
    ...
    async fn handle_request_multiple_objects(
        ...
        let circuit_code = {
            ...
            if let Some(client) = clients_guard.get_mut(&addr) {
                ...
                client.last_packet_time = std::time::Instant::now();
                tracing::info!(
                    "Received RequestMultipleObjects from {}, sending basic objects",
                    addr
                );
                client.circuit_code
            } else {
    ...
    async fn send_login_sequence(
        ...
        tracing::info!(
            "Sent RegionHandshake packet to {} (circuit: {})",
            addr,
            circuit_code
        );
        ...
        tracing::info!(
            "Sent EnableSimulator packet to {} (circuit: {})",
            addr,
            circuit_code
        );
        ...
        tracing::info!(
            "Sent TeleportFinish packet to {} (circuit: {})",
            addr,
            circuit_code
        );
        ...
        tracing::info!(
            "Sent StartPingCheck packet to {} (circuit: {})",
            addr,
            circuit_code
        );
        ...
        tracing::info!(
            "Sent LayerData packet to {} (circuit: {})",
            addr,
            circuit_code
        );
        ...
        tracing::info!(
            "Sent RegionHandshakeReply packet to {} (circuit: {})",
            addr,
            circuit_code
        );
        ...
        socket.send_to(&sim_stats, addr).await?;
        tracing::info!(
            "Sent SimStats packet to {} (circuit: {})",
            addr,
            circuit_code
        );
        ...
        socket.send_to(&uuid_name_reply, addr).await?;
        tracing::info!(
            "Sent UUIDNameReply packet to {} (circuit: {})",
            addr,
            circuit_code
        );
