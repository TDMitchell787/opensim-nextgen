...
impl DatabaseCompatibilityManager {
    ...
    pub async fn detect_schema(&mut self) -> Result<()> {
        ...
        let version = self.detect_database_version().await.unwrap_or_else(|| {
            match schema_type {
        ...
        if let Some(schema) = &self.detected_schema {
            tracing::info!("Detected database schema: {:?} (version: {})", 
                          schema.schema_type, schema.version);
            
            if schema.needs_migration {
                tracing::warn!("Database migration required. Missing tables: {:?}", 
    ...
    fn determine_schema_type(&self, tables: &[String]) -> SchemaType {
        ...
        let opensim_next_count = OPENSIM_NEXT_TABLES.iter()
        ...
        let robust_count = ROBUST_TABLES.iter()
            .filter(|&&table| table_set.contains(&table.to_string()))
            .count();
        ...
        let opensim_count = OPENSIM_CORE_TABLES.iter()
            .filter(|&&table| table_set.contains(&table.to_string()))
            .count();
    ...
    fn find_missing_tables(&self, existing_tables: &[String], schema_type: &SchemaType) -> Vec<String> {
    ...
    pub async fn migrate_to_opensim_next(&mut self) -> Result<()> {
        if let Some(schema) = &self.detected_schema {
            tracing::info!("Starting migration from {:?} to OpenSimNext", schema.schema_type);
    ...
    async fn migrate_user_data(&self) -> Result<()> {
        ...
        let legacy_exists = sqlx::query("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'users')")
            .fetch_one(&self.pool)
            .await?;
    ...
    async fn migrate_region_data(&self) -> Result<()> {
        ...
        let legacy_exists = sqlx::query("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'regions')")
            .fetch_one(&self.pool)
            .await?;
    ...
    async fn migrate_asset_data(&self) -> Result<()> {
        ...
        let assets_exist = sqlx::query("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'assets')")
            .fetch_one(&self.pool)
            .await?;
    ...
    async fn migrate_robust_assets(&self) -> Result<()> {
        ...
        let robust_assets_exist = sqlx::query("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'Assets')")
            .fetch_one(&self.pool)
            .await?;
    ...
    async fn migrate_robust_regions(&self) -> Result<()> {
        ...
        let robust_regions_exist = sqlx::query("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'Regions')")
            .fetch_one(&self.pool)
            .await?;
