...
impl MapTileManager {
    ...
    pub async fn initialize(&mut self) -> Result<()> {
        ...
        tracing::info!(
            "Map tile manager initialized with {} cached tiles",
            self.tile_cache.len()
        );
        Ok(())
    }
        ...
        for dir in &dirs {
            if !dir.exists() {
                fs::create_dir_all(dir).map_err(|e| {
                    anyhow!(format!(
                        "Failed to create maptiles directory {}: {}",
                        dir.display(),
                        e
                    ))
                })?;
    ...
    async fn load_existing_tiles(&mut self) -> Result<()> {
        ...
        let entries = fs::read_dir(&regions_dir)
            .map_err(|e| anyhow!(format!("Failed to read regions directory: {}", e)))?;

        for entry in entries {
            let entry =
                entry.map_err(|e| anyhow!(format!("Failed to read directory entry: {}", e)))?;

            let path = entry.path();
            if path.is_file() {
                if let Some(tile) = self.parse_tile_from_path(&path).await? {
                    let cache_key =
                        self.make_cache_key(&tile.region_id, tile.x, tile.y, tile.zoom_level);
    ...
    async fn parse_tile_from_path(&self, path: &Path) -> Result<Option<MapTile>> {
        let file_name = path
            .file_stem()
            .and_then(|s| s.to_str())
            .ok_or_else(|| anyhow!(format!("Invalid tile filename: {}", path.display())))?;
        ...
        let region_id = Uuid::parse_str(parts[0])
            .map_err(|_| anyhow!(format!("Invalid region ID in filename: {}", parts[0])))?;

        let x: u32 = parts[1]
            .parse()
            .map_err(|_| anyhow!(format!("Invalid X coordinate in filename: {}", parts[1])))?;

        let y: u32 = parts[2]
            .parse()
            .map_err(|_| anyhow!(format!("Invalid Y coordinate in filename: {}", parts[2])))?;

        let zoom_level: u8 = parts[3]
            .parse()
            .map_err(|_| anyhow!(format!("Invalid zoom level in filename: {}", parts[3])))?;
        ...
        let metadata = fs::metadata(path)
            .map_err(|e| anyhow!(format!("Failed to get file metadata: {}", e)))?;
    ...
    pub async fn generate_region_tile(&mut self, region_info: &RegionInfo) -> Result<MapTile> {
        ...
        for zoom_level in zoom_levels {
            if zoom_level > 0 {
                self.generate_tile(region_info, tile_x, tile_y, zoom_level)
                    .await?;
            }
        }
    ...
    async fn generate_tile(
        ...
        fs::write(&file_path, &tile_data).map_err(|e| {
            anyhow!(format!(
        ...
        tracing::debug!(
            "Generated tile for region {} at {},{} zoom {}",
            region_info.name,
            x,
            y,
            zoom_level
    ...
    async fn render_tile(
        ...
        self.encode_image_data(image_data, tile_size, tile_size)
            .await
    }
    ...
    async fn encode_image_data(
        ...
        let img: RgbImage = ImageBuffer::from_raw(width, height, rgb_data)
            .ok_or_else(|| anyhow!("Failed to create image buffer from RGB data".to_string()))?;
        ...
        match self.config.format {
            TileFormat::Jpeg => {
                dynamic_img
                    ...
                    .map_err(|e| anyhow!(format!("Failed to encode JPEG: {}", e)))?;
            }
            TileFormat::Png => {
                dynamic_img
                    .write_to(&mut output, ImageFormat::Png)
                    .map_err(|e| anyhow!(format!("Failed to encode PNG: {}", e)))?;
            }
            TileFormat::WebP => {
                dynamic_img
                    .write_to(&mut output, ImageFormat::WebP)
                    .map_err(|e| anyhow!(format!("Failed to encode WebP: {}", e)))?;
            }
        }
        ...
        if let Some(tiles) = self.region_tiles.remove(region_id) {
            for tile in tiles {
                ...
                if tile.file_path.exists() {
                    fs::remove_file(&tile.file_path).map_err(|e| {
                        anyhow!(format!(
                            "Failed to delete tile file {}: {}",
                            tile.file_path.display(),
                            e
                        ))
                    })?;
                }
    ...
    pub async fn get_tile_data(&self, tile: &MapTile) -> Result<Vec<u8>> {
        fs::read(&tile.file_path).map_err(|e| {
            anyhow!(format!(
                "Failed to read tile file {}: {}",
                tile.file_path.display(),
                e
            ))
        })
    ...
    pub fn get_statistics(&self) -> HashMap<String, u64> {
        ...
        stats.insert(
            "regions_with_tiles".to_string(),
            self.region_tiles.len() as u64,
        );
    ...
    pub async fn generate_world_tile(
        &mut self,
        regions: &[RegionInfo],
        world_x: u32,
        world_y: u32,
        ...
        let tile_data = self
            .render_world_tile(regions, world_x, world_y, zoom_level)
            .await?;
        ...
        fs::write(&file_path, &tile_data)
            .map_err(|e| anyhow!(format!("Failed to write world tile: {}", e)))?;
    ...
    async fn render_world_tile(
        ...
        for py in 0..tile_size {
            for px in 0..tile_size {
                let world_coord_x =
                    (world_x * WORLD_TILE_SIZE) + (px * WORLD_TILE_SIZE / tile_size);
                let world_coord_y =
                    (world_y * WORLD_TILE_SIZE) + (py * WORLD_TILE_SIZE / tile_size);
                ...
                for region in regions {
                    ...
                    {
                        let height = self.get_terrain_height(
                            region,
                            world_coord_x as f32,
                            world_coord_y as f32,
                        );
