...
impl IniConfigParser {
    ...
    fn load_config_file(&mut self, config_path: &Path) -> Result<()> {
        ...
        if self.include_stack.contains(&config_path_str) {
            return Err(anyhow!(
                format!("Circular include detected: {}", config_path_str)
            ));
        }
        ...
        for (line_num, line) in content.lines().enumerate() {
            ...
            if let Some(eq_pos) = line.find('=') {
                if current_section.is_empty() {
                    return Err(anyhow!(
                        format!("Key-value pair outside section at line {}: {}", line_num + 1, line)
                    ));
                }
                ...
                self.sections.get_mut(&current_section)
                    .unwrap()
                    .insert(key, value);
            }
        }
    ...
    pub fn load_region_configs(&self, regions_path: &Path) -> Result<Vec<RegionConfig>> {
        ...
        for (section_name, section_data) in &region_parser.sections {
            ...
            let region = RegionConfig {
                region_uuid: section_data.get("RegionUUID")
                    .unwrap_or(&"00000000-0000-0000-0000-000000000000".to_string())
                    .clone(),
                region_name: section_data.get("RegionName")
                    .unwrap_or(section_name)
                    .clone(),
                location_x: section_data.get("Location")
                    .and_then(|loc| loc.split(',').next())
                    .and_then(|x| x.parse().ok())
                    .unwrap_or(1000),
                location_y: section_data.get("Location")
                    .and_then(|loc| loc.split(',').nth(1))
                    .and_then(|y| y.parse().ok())
                    .unwrap_or(1000),
                internal_address: section_data.get("InternalAddress")
                    .unwrap_or(&"0.0.0.0".to_string())
                    .clone(),
                internal_port: section_data.get("InternalPort")
                    .and_then(|p| p.parse().ok())
                    .unwrap_or(9000),
                external_host_name: section_data.get("ExternalHostName")
                    .unwrap_or(&"127.0.0.1".to_string())
                    .clone(),
                physics_engine: section_data.get("PhysicsEngine")
                    .unwrap_or(&"POS".to_string())
                    .clone(),
                max_prims: section_data.get("MaxPrims")
                    .and_then(|p| p.parse().ok())
                    .unwrap_or(100000),
                max_agents: section_data.get("MaxAgents")
    ...
    pub fn get_network_config(&self) -> (String, u16) {
        let address = self.get_value("Network", "ExternalHostNameForLSL")
            .or_else(|| self.get_value("Startup", "ExternalHostNameForLSL"))
            .unwrap_or_else(|| "127.0.0.1".to_string());
            
        let port = self.get_value("Network", "http_listener_port")
