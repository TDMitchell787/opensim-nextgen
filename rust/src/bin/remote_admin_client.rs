...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::CreateUser {
            ...
            match client
                .create_user(&firstname, &lastname, &password, &email)
                .await
            {
                Ok(result) => {
        ...
        Commands::GetAgents { region_uuid } => match client.get_agents(region_uuid).await {
            Ok(agents) => {
        ...
        Commands::Restart { region_uuid } => match client.restart_region(region_uuid).await {
            Ok(result) => {
        ...
        Commands::QueryRegions { region_uuid } => match client.query_regions(region_uuid).await {
            Ok(result) => {
        ...
        Commands::Console { command } => match client.console_command(&command).await {
            Ok(result) => {
        ...
        Commands::Status => match client.get_status().await {
            Ok(status) => {
