...
impl RemoteAdminTester {
    ...
    async fn test_authentication(&mut self) {
        ...
        match self
            .execute_command("admin_region_query", HashMap::new())
            .await
        {
            Ok(result) => {
                if result.success {
    ...
    async fn test_agent_management(&mut self) {
        ...
        let start = Instant::now();
        match self
            .execute_command("admin_get_agents", HashMap::new())
            .await
        {
            Ok(result) => {
                if result.success {
    ...
    async fn test_region_management(&mut self) {
            ...
            .await
        {
            Ok(result) => {
                if result.success {
                    let region_count = if let Some(regions) =
                        result.data.get("regions").and_then(|v| v.as_array())
                    {
                        regions.len()
    ...
    async fn test_error_handling(&mut self) {
        ...
        let start = Instant::now();
        match self
            .execute_command("admin_unknown_command", HashMap::new())
            .await
        {
            Ok(result) => {
                if result.success {
