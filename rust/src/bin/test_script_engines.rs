...
impl ScriptEngineTestSuite {
    ...
    async fn test_configuration_file(&mut self) {
        ...
        match fs::read_to_string(&self.config_file) {
            Ok(content) => {
                ...
                let engines_found = sections.keys()
    ...
    async fn test_engine_directories(&mut self) {
        ...
        for engine in &engines {
            ...
            let config_files = vec!["template.ini".to_string(), "config.ini".to_string(), format!("{}.ini", engine.to_lowercase())];
        ...
        if total_score == max_score {
            ...
            println!("  ⚠️  Engine directories: INCOMPLETE ({:.1}%)", success_rate);
        }
    ...
    async fn test_engine_manager(&mut self) {
        ...
        let has_source = details.get("source_exists").unwrap().as_bool().unwrap_or(false);
        if has_source {
            self.results.push(TestResult::success(
                "Engine Manager",
        ...
        for engine in &engines {
            if let Some(engine_config) = sections.get(*engine) {
                ...
                validation_results.insert(
                    format!("{}_issues", engine), 
                    json!(engine_issues)
                );
                issues.extend(engine_issues.into_iter().map(|issue| format!("{}: {}", engine, issue)));
            } else {
    ...
    async fn test_performance(&mut self) {
        ...
        if self.config_file.exists() {
            ...
            println!("      {} iterations in {:.2}ms", iterations, parse_time.as_millis());
            println!("      Average: {:.1}μs per parse", avg_parse_time.as_micros());
        }
    ...
    fn print_summary(&self) {
        ...
        println!("\n⏱️  Average test duration: {:.3}ms", avg_duration.as_millis());
...
async fn main() -> Result<()> {
    ...
    let mut test_suite = ScriptEngineTestSuite::new(
        cli.engines_dir,
        cli.config_file,
        cli.verbose,
    );
