...
impl DarkServicesClient {
    ...
    async fn request_transfer(&self, request: DarkTransferRequest) -> Result<DarkTransferResponse> {
        ...
        if self.verbose {
            println!(
                "Transfer response: {}",
                serde_json::to_string_pretty(&transfer_response)?
            );
        }
    ...
    async fn set_access_policy(&self, asset_id: Uuid, policy: AccessPolicy) -> Result<()> {
        ...
        if !response.status().is_success() {
            return Err(anyhow!(
                "Set policy request failed: HTTP {}",
                response.status()
            ));
        }
    ...
    async fn get_access_policy(&self, asset_id: Uuid) -> Result<AccessPolicy> {
        ...
        if !response.status().is_success() {
            return Err(anyhow!(
                "Get policy request failed: HTTP {}",
                response.status()
            ));
        }
...
async fn main() -> Result<()> {
    ...
    let client = DarkServicesClient::new(cli.server, cli.token, cli.verbose, cli.insecure)?;
    ...
    match cli.command {
        ...
        Commands::Download {
            ...
            client
                .download_chunks(session_id, &output_dir, verify)
                .await?;
        }
