...
impl Default for ScriptEngineConfig {
    fn default() -> Self {
        ...
        engines.insert("Native".to_string(), ScriptEngine {
        ...
        engines.insert("YEngine".to_string(), ScriptEngine {
        ...
        engines.insert("XEngine".to_string(), ScriptEngine {
...
impl ScriptEngineManager {
        ...
        for line in content.lines() {
            ...
            if let Some((key, value)) = line.split_once('=') {
                ...
                if current_section == "ScriptEngines" {
                    config.global_settings.insert(key.to_string(), value.to_string());
    ...
    fn list_engines(&self) {
        ...
        for engine in self.config.engines.values() {
            let status = if engine.enabled { "🟢 ENABLED" } else { "🔴 DISABLED" };
    ...
    fn show_status(&self) {
        ...
        println!("  Default engine: {}", self.config.global_settings.get("DefaultEngine").unwrap_or(&"None".to_string()));
        ...
        for engine in self.config.engines.values() {
            ...
            let status_text = if engine.enabled { "ENABLED" } else { "DISABLED" };
    ...
    fn validate_setup(&self) -> Result<()> {
        ...
        if !self.engines_dir.exists() {
            issues.push(format!("❌ ScriptEngines directory missing: {}", self.engines_dir.display()));
        } else {
        ...
        if !self.config_file.exists() {
            issues.push(format!("❌ Configuration file missing: {}", self.config_file.display()));
        } else {
        ...
        for engine in self.config.engines.values() {
            ...
            if !engine_dir.exists() {
                issues.push(format!("⚠️  Engine directory missing: {}", engine_dir.display()));
            }
        }
    ...
    fn create_template(&self, engine_name: &str) -> Result<()> {
        ...
        let template_file = self.engines_dir.join(format!("{}/template.ini", engine_name));
        fs::create_dir_all(template_file.parent().unwrap())?;
        fs::write(&template_file, template_content)?;
        
        println!("✅ Created template for {}: {}", engine_name, template_file.display());
        Ok(())
    }
}
