...
extern "C" {
    ...
    pub fn physics_create_mesh_shape(indices_count: i32, indices: *const i32, vertices_count: i32, vertices: *const f32) -> *mut std::ffi::c_void;
    pub fn physics_create_body_from_shape(shape: *mut std::ffi::c_void, id: u32, pos_x: f32, pos_y: f32, pos_z: f32, rot_x: f32, rot_y: f32, rot_z: f32, rot_w: f32) -> *mut std::ffi::c_void;
    ...
    pub fn physics_raycast(
        ...
        out_hit: *mut bool, out_id: *mut u32, out_fraction: *mut f32,
    ...
    pub fn physics_world_add_box(
        region_hi: u64, region_lo: u64, local_id: u32,
    ...
    pub fn physics_world_remove_body(
        region_hi: u64, region_lo: u64, local_id: u32,
    );
    ...
    pub fn physics_world_set_terrain(
        region_hi: u64, region_lo: u64,
        heights: *const f32, width: u32, depth: u32,
    ) -> bool;
    pub fn physics_world_collision_step(
        region_hi: u64, region_lo: u64,
    ) -> u32;
    pub fn physics_world_get_contact(
        region_hi: u64, region_lo: u64, index: u32,
        out_id_a: *mut u32, out_id_b: *mut u32,
        out_nx: *mut f32, out_ny: *mut f32, out_nz: *mut f32,
        out_depth: *mut f32,
        out_px: *mut f32, out_py: *mut f32, out_pz: *mut f32,
    ) -> bool;
    pub fn physics_world_raycast_down(
        ...
        max_dist: f32,
        out_hit: *mut bool, out_body_id: *mut u32,
        out_px: *mut f32, out_py: *mut f32, out_pz: *mut f32,
        out_nx: *mut f32, out_ny: *mut f32, out_nz: *mut f32,
    );
}
...
impl Physics {
    ...
    pub fn with_engine(engine_type: PhysicsEngineType) -> Result<Self, FFIError> {
        ...
        unsafe {
            if !physics_create_engine(engine_type as u8) {
                warn!("Failed to create {:?} engine, falling back to ODE", engine_type);
                physics_memory_init(1024 * 1024 * 10);
            }
        }

        info!("Physics system initialized successfully");
        Ok(Self { ffi_manager, engine_type })
    }
    ...
    pub fn create_terrain(&mut self, heights: &[f32], w: u32, h: u32) -> Result<(), FFIError> {
        ...
        if ok { Ok(()) } else { Err(FFIError::PhysicsError) }
    }
    ...
    pub fn step(&mut self, delta_time: f32) -> Result<(), FFIError> {
        ...
        if error_count > 0 {
            ...
            warn!("Physics step completed with {} errors: {}", error_count, error_msg);
        }
...
pub fn uuid_to_hi_lo(uuid: uuid::Uuid) -> (u64, u64) {
    ...
    let hi = u64::from_be_bytes([bytes[0], bytes[1], bytes[2], bytes[3], bytes[4], bytes[5], bytes[6], bytes[7]]);
...
impl CollisionWorld {
    ...
    pub fn add_box(
        region_uuid: uuid::Uuid, local_id: u32,
        pos: [f32; 3], rot: [f32; 4], half_extents: [f32; 3], flags: u32,
    ) -> bool {
        let (hi, lo) = uuid_to_hi_lo(region_uuid);
        unsafe {
    ...
    pub fn add_sphere(
        region_uuid: uuid::Uuid, local_id: u32,
        pos: [f32; 3], radius: f32, flags: u32,
    ) -> bool {
        let (hi, lo) = uuid_to_hi_lo(region_uuid);
    ...
    pub fn add_avatar(
        region_uuid: uuid::Uuid, local_id: u32,
        pos: [f32; 3], radius: f32, height: f32,
    ) -> bool {
        let (hi, lo) = uuid_to_hi_lo(region_uuid);
        unsafe { physics_world_add_avatar(hi, lo, local_id, pos[0], pos[1], pos[2], radius, height) }
    }
    ...
    pub fn update_transform(
        region_uuid: uuid::Uuid, local_id: u32, pos: [f32; 3], rot: [f32; 4],
    ) {
        let (hi, lo) = uuid_to_hi_lo(region_uuid);
        unsafe {
    ...
    pub fn get_contact(region_uuid: uuid::Uuid, index: u32) -> Option<ContactResult> {
        ...
        if ok {
            Some(ContactResult { body_id_a: id_a, body_id_b: id_b, normal: [nx, ny, nz], depth, point: [px, py, pz] })
        } else {
            None
        }
    }

    pub fn raycast_down(region_uuid: uuid::Uuid, origin: [f32; 3], max_dist: f32) -> Option<RayDownResult> {
        let (hi, lo) = uuid_to_hi_lo(region_uuid);
        let mut hit = false;
        ...
        if hit {
            Some(RayDownResult { body_id, point: [px, py, pz], normal: [nx, ny, nz] })
        } else {
            None
        }
    }
...
mod tests {
    ...
    fn test_ffi_error_conversion() {
        assert!(matches!(FFIError::from(FFI_ERROR_INVALID_HANDLE), FFIError::InvalidHandle));
        assert!(matches!(FFIError::from(FFI_ERROR_ALLOCATION_FAILED), FFIError::AllocationFailed));
    ...
    fn test_physics_body_creation() {
        ...
        if let Ok(mut body) = body {
            ...
            let new_vel = Vec3 { x: 1.0, y: 2.0, z: 3.0 };
