...
impl OSSLFunctions {
    ...
    fn os_set_terrain_texture_height(
        &self,
        args: &[LSLValue],
        _context: &ScriptContext,
    ) -> Result<LSLValue> {
    ...
    fn os_draw_rectangle(&self, args: &[LSLValue], _context: &ScriptContext) -> Result<LSLValue> {
        ...
        Ok(LSLValue::String(format!(
            "{}Rectangle {},{};",
            draw_list, width, height
        )))
    }

    fn os_draw_filled_rectangle(
        &self,
        args: &[LSLValue],
        _context: &ScriptContext,
    ) -> Result<LSLValue> {
    ...
    fn os_set_dynamic_texture_data(
        &self,
        args: &[LSLValue],
        _context: &ScriptContext,
    ) -> Result<LSLValue> {
        if args.len() < 4 {
            return Err(anyhow!(
                "osSetDynamicTextureData expects at least 4 arguments"
            ));
        }
        Ok(LSLValue::String(Uuid::new_v4().to_string()))
    }

    fn os_get_draw_string_size(
        &self,
        args: &[LSLValue],
        _context: &ScriptContext,
    ) -> Result<LSLValue> {
    ...
    fn os_get_number_of_notecard_lines(
        &self,
        args: &[LSLValue],
        _context: &ScriptContext,
    ) -> Result<LSLValue> {
    ...
    fn os_region_restart(&self, args: &[LSLValue], _context: &ScriptContext) -> Result<LSLValue> {
        ...
        warn!(
            "osRegionRestart requested with {}s delay",
            args[0].to_float()
        );
        Ok(LSLValue::Integer(0))
    }

    fn os_set_projection_params(
        &self,
        args: &[LSLValue],
        _context: &ScriptContext,
    ) -> Result<LSLValue> {
    ...
    fn os_get_number_of_attachments(
        &self,
        args: &[LSLValue],
        _context: &ScriptContext,
    ) -> Result<LSLValue> {
    ...
    fn os_force_detach_from_avatar(
        &self,
        _args: &[LSLValue],
        _context: &ScriptContext,
    ) -> Result<LSLValue> {
...
mod tests {
    ...
    fn test_threat_level_enforcement() {
        ...
        let result = ossl.execute_function(
            "osNpcCreate",
            &[
    ...
    fn test_region_info_functions() {
        ...
        let result = ossl
            .execute_function("osGetRegionSize", &[], &mut ctx)
            .unwrap();
        ...
        let result = ossl
            .execute_function("osGetScriptEngineName", &[], &mut ctx)
            .unwrap();
        if let LSLValue::String(s) = result {
            assert_eq!(s, "YEngine");
        } else {
