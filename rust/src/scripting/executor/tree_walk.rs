...
use super::{
    CompiledScript, EventHandler, ExecutionResult, ScriptExecutor, ScriptInstance, StateDefinition,
    UserFunction,
};
...
pub static LSL_CONSTANTS: std::sync::LazyLock<HashMap<String, LSLValue>> =
...
impl ScriptExecutor for TreeWalkExecutor {
    ...
    fn execute_event(
        ...
        match result {
            ...
            Ok(ControlFlow::StateChange(name)) => Ok(ExecutionResult::StateChange(name)),
            Ok(ControlFlow::Jump(_)) => Ok(ExecutionResult::Complete(LSLValue::Integer(0))),
...
impl ExecContext {
    ...
    fn execute_node(&mut self, node: &ASTNode) -> Result<ControlFlow> {
        ...
        match node {
            ...
            ASTNode::VectorLiteral { x, y, z } => {
                ...
                Ok(ControlFlow::Normal(LSLValue::Vector(LSLVector {
                    x: xv,
                    y: yv,
                    z: zv,
                })))
            ...
            ASTNode::RotationLiteral { x, y, z, s } => {
                ...
                Ok(ControlFlow::Normal(LSLValue::Rotation(LSLRotation {
                    x: xv,
                    y: yv,
                    z: zv,
                    s: sv,
            ...
            ASTNode::StateChange(name) => Ok(ControlFlow::StateChange(name.clone())),

            ASTNode::Label(_) => Ok(ControlFlow::Normal(LSLValue::Integer(0))),

            ASTNode::Jump(label) => Ok(ControlFlow::Jump(label.clone())),
    ...
    fn call_builtin(&mut self, name: &str, args: &[LSLValue]) -> Result<LSLValue> {
        ...
        match name {
            ...
            "llSay" => {
                ...
                self.actions.push(ScriptAction::Say {
                    channel: ch,
                    message: msg,
            ...
            "llShout" => {
                let ch = args.first().map(|a| a.to_integer()).unwrap_or(0);
                let msg = args.get(1).map(|a| a.to_string()).unwrap_or_default();
                self.actions.push(ScriptAction::Shout {
                    channel: ch,
                    message: msg,
                    object_id: self.ctx.object_id,
                });
            ...
            "llWhisper" => {
                let ch = args.first().map(|a| a.to_integer()).unwrap_or(0);
                let msg = args.get(1).map(|a| a.to_string()).unwrap_or_default();
                self.actions.push(ScriptAction::Whisper {
                    channel: ch,
                    message: msg,
                    object_id: self.ctx.object_id,
                });
            ...
            "llOwnerSay" => {
                ...
                self.actions.push(ScriptAction::OwnerSay {
                    message: msg,
                    object_id: self.ctx.object_id,
            ...
            "llRegionSay" => {
                ...
                self.actions.push(ScriptAction::RegionSay {
                    channel: ch,
                    message: msg,
                    object_id: self.ctx.object_id,
                });
            ...
            "llRegionSayTo" => {
                ...
                self.actions.push(ScriptAction::RegionSayTo {
                    target_id: target,
                    channel: ch,
                    message: msg,
                    object_id: self.ctx.object_id,
            ...
            "llListen" => {
                ...
                self.actions.push(ScriptAction::Listen {
                    channel: ch,
                    name: nm,
                    id,
                    msg,
            ...
            "llListenControl" => Ok(LSLValue::Integer(0)),
            ...
            "llDetectedPos" => {
                ...
                Ok(LSLValue::Vector(
                    self.ctx
                        ...
                        .get(idx)
                        .map(|d| d.position)
                        .unwrap_or(LSLVector::zero()),
                ))
            }
            "llDetectedRot" => {
                let idx = args.first().map(|a| a.to_integer()).unwrap_or(0) as usize;
                Ok(LSLValue::Rotation(
                    self.ctx
                        .detect_params
                        .get(idx)
                        .map(|d| d.rotation)
                        .unwrap_or(LSLRotation::identity()),
                ))
            }
            "llDetectedVel" => {
                let idx = args.first().map(|a| a.to_integer()).unwrap_or(0) as usize;
            ...
            | "llDetectedTouchPos" => Ok(LSLValue::Vector(LSLVector::zero())),
            "llDetectedTouchUV" | "llDetectedTouchST" => Ok(LSLValue::Vector(LSLVector::zero())),
            ...
            "llMessageLinked" => {
                ...
                self.actions.push(ScriptAction::MessageLinked {
                    link_num: link,
                    num,
                    str_val,
                    id,
            ...
            "llGetSubString" => {
                ...
                if s_idx <= e_idx {
                    Ok(LSLValue::String(
                        s.chars().skip(s_idx).take(e_idx - s_idx + 1).collect(),
                    ))
                } else {
            ...
            "llDeleteSubString" => {
                ...
                if s_idx <= e_idx {
                    ...
                    Ok(LSLValue::String(
                        s.chars().skip(e_idx + 1).take(s_idx - e_idx - 1).collect(),
                    ))
                }
            }
            "llSubStringIndex" => {
                ...
                Ok(LSLValue::Integer(
                    s.find(&pat).map(|i| i as i32).unwrap_or(-1),
                ))
            }
            "llToUpper" => {
                let s = args.first().map(|a| a.to_string()).unwrap_or_default();
            ...
            "llGetNotecardLine" | "llGetNumberOfNotecardLines" => Ok(LSLValue::Key(Uuid::nil())),
            ...
            "llList2String" => {
                ...
                let i = if idx < 0 {
                    (list.len() as i32 + idx).max(0) as usize
                } else {
                    idx as usize
                };
                Ok(LSLValue::String(
                    list.get(i).map(|v| v.to_string()).unwrap_or_default(),
                ))
            }
            "llList2Integer" => {
                let list = args.first().map(|a| a.to_list()).unwrap_or_default();
                ...
                Ok(LSLValue::Integer(
                    list.get(i).map(|v| v.to_integer()).unwrap_or(0),
                ))
            }
            "llList2Float" => {
                let list = args.first().map(|a| a.to_list()).unwrap_or_default();
                ...
                Ok(LSLValue::Float(
                    list.get(i).map(|v| v.to_float()).unwrap_or(0.0),
                ))
            }
            "llList2Key" => {
                let list = args.first().map(|a| a.to_list()).unwrap_or_default();
                ...
                Ok(LSLValue::Key(
                    list.get(i).map(|v| v.to_key()).unwrap_or(Uuid::nil()),
                ))
            }
            "llList2Vector" => {
                let list = args.first().map(|a| a.to_list()).unwrap_or_default();
            ...
            "llListSort" => {
                ...
                let mut chunks: Vec<Vec<LSLValue>> =
                    list.chunks(stride).map(|c| c.to_vec()).collect();
                chunks.sort_by(|a, b| {
                    let cmp = a
                        .first()
                        .map(|v| v.to_string())
            ...
            "llListReplaceList" => {
                ...
                let e = if end < 0 {
                    ...
                    (end as usize).min(if len > 0 { len as usize - 1 } else { 0 })
                };
                let mut r = dst[..s].to_vec();
                r.extend(src);
                if e + 1 < dst.len() {
                    r.extend_from_slice(&dst[e + 1..]);
                }
                Ok(LSLValue::List(r))
            }
            "llListFindList" => {
                ...
                while !remaining.is_empty() {
                    ...
                    if earliest_pos == remaining.len() {
                        if !remaining.is_empty() {
                            result.push(LSLValue::String(remaining.to_string()));
                        }
                        break;
                    }
                    if earliest_pos > 0 {
            ...
            "llCSV2List" => {
                let s = args.first().map(|a| a.to_string()).unwrap_or_default();
                let items: Vec<LSLValue> = s
                    .split(',')
                    .map(|p| LSLValue::String(p.trim().to_string()))
                    .collect();
            ...
            "llRotBetween" => Ok(LSLValue::Rotation(LSLRotation::identity())),
            "llAngleBetween" => {
                let a = args
                    .first()
                    .map(|v| v.to_rotation())
            ...
            "llAxisAngle2Rot" => {
                let axis = args
                    .first()
                    .map(|a| a.to_vector())
                    .unwrap_or(LSLVector::new(0.0, 0.0, 1.0));
                ...
                if mag < 1e-6 {
                    ...
                    let half = angle / 2.0;
                    let s = half.sin() / mag;
                    Ok(LSLValue::Rotation(LSLRotation {
                        x: axis.x * s,
                        y: axis.y * s,
                        z: axis.z * s,
                        s: half.cos(),
            ...
            "llGetTimestamp" => {
                ...
                let s = secs % 60;
                Ok(LSLValue::String(format!(
                    "2026-01-01T{:02}:{:02}:{:02}.000000Z",
                    hours, mins, s
                )))
            }
            "llGenerateKey" => Ok(LSLValue::Key(Uuid::new_v4())),
            "llSHA1String" => {
                let s = args.first().map(|a| a.to_string()).unwrap_or_default();
                Ok(LSLValue::String(format!(
                    "{:x}",
            ...
            "llGetListEntryType" => Ok(LSLValue::Integer(0)),
            ...
            "llRezObject" | "llRezAtRoot" => {
                ...
                let pos = args
                    .get(1)
                    .map(|a| a.to_vector())
                    .unwrap_or(super::super::LSLVector::zero());
                let vel = args
                    .get(2)
                    .map(|a| a.to_vector())
                    .unwrap_or(super::super::LSLVector::zero());
                let rot = args
                    .get(3)
                    .map(|a| a.to_rotation())
                    .unwrap_or(super::super::LSLRotation::identity());
            ...
            "llRemoveInventory" => {
                let item_name = args.first().map(|a| a.to_string()).unwrap_or_default();
                self.actions.push(ScriptAction::RemoveInventory {
                    object_id: self.ctx.object_id,
                    script_id: self.script_id,
                    item_name,
                });
            ...
            "llRequestAgentData" => {
                ...
                self.actions.push(ScriptAction::RequestUserData {
                    script_id: self.script_id,
                    agent_id: agent,
                    query_id: qid.clone(),
                    data_type: data,
            ...
            "llSetAlpha" => {
                ...
                self.actions.push(ScriptAction::SetAlpha {
                    object_id: self.ctx.object_id,
                    alpha,
                    face,
                });
            ...
            "llSetSitText" => {
                ...
                self.actions.push(ScriptAction::SetSitText {
                    object_id: self.ctx.object_id,
                    text,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llSetTouchText" => {
                let text = args.first().map(|a| a.to_string()).unwrap_or_default();
                self.actions.push(ScriptAction::SetTouchText {
                    object_id: self.ctx.object_id,
                    text,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llAvatarOnSitTarget" => Ok(LSLValue::Key(self.ctx.sitting_avatar_id)),
            "llUnSit" => {
                let target = args
                    .first()
                    .map(|a| a.to_key())
                    .unwrap_or(self.ctx.sitting_avatar_id);
                if !target.is_nil() {
            ...
            "llSetCameraParams" => {
                ...
                while i + 1 < list.len() {
                    ...
                    match ptype {
                        1 | 13 | 17 => {
                            let vec = list.get(i + 1).map(|v| v.to_vector()).unwrap_or(
                                crate::scripting::lsl_types::LSLVector::new(0.0, 0.0, 0.0),
                            );
                ...
                if !params.is_empty() {
                    self.actions.push(ScriptAction::SetCameraParams {
            ...
            | "llSetLinkPrimitiveParamsFast" => {
                let (link_num, rules_list) = if name == "llSetPrimitiveParams" {
                    (
                        self.ctx.link_num,
                        args.first().map(|a| a.to_list()).unwrap_or_default(),
                    )
                } else {
                ...
                if !rules.is_empty() {
                    self.actions.push(ScriptAction::SetLinkPrimParams {
                        object_id: self.ctx.object_id,
                        link_num,
                        rules,
                    });
            ...
            "llSetColor" => {
                let color = args
                    .first()
                    .map(|a| a.to_vector())
                    .unwrap_or(LSLVector::new(1.0, 1.0, 1.0));
                let face = args.get(1).map(|a| a.to_integer()).unwrap_or(-1);
                self.actions.push(ScriptAction::SetColor {
                    object_id: self.ctx.object_id,
            ...
            "llSetTexture" => {
                ...
                let face = args.get(1).map(|a| a.to_integer()).unwrap_or(-1);
                self.actions.push(ScriptAction::SetTexture {
                    object_id: self.ctx.object_id,
                    texture_id: tex,
                    face,
                });
            ...
            "llSetTextureAnim" => {
                ...
                self.actions.push(ScriptAction::SetTextureAnim {
                    object_id: self.ctx.object_id,
                    mode,
                    face,
                    size_x: sx,
            ...
            "llSetLinkColor" => {
                ...
                self.actions.push(ScriptAction::SetLinkColor {
                    object_id: self.ctx.object_id,
                    link_num: link,
                    color: [color.x, color.y, color.z],
                    face,
            ...
            "llSetLinkAlpha" => {
                ...
                let face = args.get(2).map(|a| a.to_integer()).unwrap_or(-1);
                self.actions.push(ScriptAction::SetLinkAlpha {
                    object_id: self.ctx.object_id,
                    link_num: link,
                    alpha,
                    face,
            ...
            "llSetLinkTexture" => {
                ...
                let face = args.get(2).map(|a| a.to_integer()).unwrap_or(-1);
                self.actions.push(ScriptAction::SetLinkTexture {
                    object_id: self.ctx.object_id,
                    link_num: link,
                    face,
                    texture_id: tex,
            ...
            "llSetObjectName" => {
                let name = args.first().map(|a| a.to_string()).unwrap_or_default();
                self.actions.push(ScriptAction::SetObjectName {
                    object_id: self.ctx.object_id,
                    name,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llSetObjectDesc" => {
                ...
                self.actions.push(ScriptAction::SetObjectDesc {
                    object_id: self.ctx.object_id,
                    desc,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llTargetOmega" => {
                ...
                self.actions.push(ScriptAction::TargetOmega {
                    object_id: self.ctx.object_id,
            ...
            "llSetCameraEyeOffset" => {
                let v = args
                    ...
                    .map(|a| a.to_vector())
                    .unwrap_or(LSLVector::zero());
                self.actions.push(ScriptAction::SetCameraEyeOffset {
                    object_id: self.ctx.object_id,
                    offset: [v.x, v.y, v.z],
                });
                Ok(LSLValue::Integer(0))
            ...
            "llSetCameraAtOffset" => {
                let v = args
                    ...
                    .map(|a| a.to_vector())
                    .unwrap_or(LSLVector::zero());
                self.actions.push(ScriptAction::SetCameraAtOffset {
                    object_id: self.ctx.object_id,
                    offset: [v.x, v.y, v.z],
                });
                Ok(LSLValue::Integer(0))
                ...
                for param in &param_list {
                    ...
                    if code == 7 {
                        ...
                        if link == 0 || link == self.ctx.link_num {
                            ...
                            self.ctx.link_scales.iter().find(|(num, _)| *num == link)
                        {
            ...
            "llParticleSystem" => {
                ...
                self.actions.push(ScriptAction::ParticleSystem {
                    object_id: self.ctx.object_id,
                    rules: ps_bytes,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llSetStatus" => {
                ...
                self.actions.push(ScriptAction::SetStatus {
                    object_id: self.ctx.object_id,
                    status,
                    value,
                });
            ...
            "llSetVehicleType" => {
                ...
                self.actions.push(ScriptAction::SetVehicleType {
                    object_id: self.ctx.object_id,
                    vehicle_type: vtype,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llSetVehicleFloatParam" => {
                ...
                self.actions.push(ScriptAction::SetVehicleFloatParam {
                    object_id: self.ctx.object_id,
                    param_id: param,
                    value: val,
                });
            ...
            "llSetVehicleVectorParam" => {
                ...
                let v = args
                    ...
                    .map(|a| a.to_vector())
                    .unwrap_or(LSLVector::zero());
                self.actions.push(ScriptAction::SetVehicleVectorParam {
                    object_id: self.ctx.object_id,
                    param_id: param,
                    value: [v.x, v.y, v.z],
                });
            ...
            "llSetVehicleRotationParam" => {
                ...
                let r = args
                    ...
                    .map(|a| a.to_rotation())
                    .unwrap_or(LSLRotation::identity());
                self.actions.push(ScriptAction::SetVehicleRotationParam {
                    object_id: self.ctx.object_id,
                    param_id: param,
                    value: [r.x, r.y, r.z, r.s],
                });
                Ok(LSLValue::Integer(0))
            }
            "llSetVehicleFlags" => {
                let f = args.first().map(|a| a.to_integer()).unwrap_or(0);
                self.actions.push(ScriptAction::SetVehicleFlags {
                    object_id: self.ctx.object_id,
                    flags: f,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llRemoveVehicleFlags" => {
                let f = args.first().map(|a| a.to_integer()).unwrap_or(0);
                self.actions.push(ScriptAction::RemoveVehicleFlags {
                    object_id: self.ctx.object_id,
                    flags: f,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llSetBuoyancy" => {
                ...
                self.actions.push(ScriptAction::SetBuoyancy {
                    object_id: self.ctx.object_id,
                    buoyancy: b,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llSetForce" => {
                ...
                self.actions.push(ScriptAction::SetForce {
                    object_id: self.ctx.object_id,
                    force: [v.x, v.y, v.z],
                    local,
                });
            ...
            "llSetTorque" => {
                let v = args
                    ...
                    .unwrap_or(LSLVector::zero());
                let local = args.get(1).map(|a| a.to_integer() != 0).unwrap_or(false);
                self.actions.push(ScriptAction::SetTorque {
                    object_id: self.ctx.object_id,
                    torque: [v.x, v.y, v.z],
                    local,
                });
            ...
            "llSetVelocity" => {
                let v = args
                    ...
                    .unwrap_or(LSLVector::zero());
                let local = args.get(1).map(|a| a.to_integer() != 0).unwrap_or(false);
                self.actions.push(ScriptAction::SetVelocity {
                    object_id: self.ctx.object_id,
            ...
            "llSetAngularVelocity" => {
                let v = args
                    ...
                    .unwrap_or(LSLVector::zero());
                let local = args.get(1).map(|a| a.to_integer() != 0).unwrap_or(false);
                self.actions.push(ScriptAction::SetAngularVelocity {
                    object_id: self.ctx.object_id,
                    velocity: [v.x, v.y, v.z],
                    local,
                });
            ...
            "llMoveToTarget" => {
                ...
                self.actions.push(ScriptAction::MoveToTarget {
                    object_id: self.ctx.object_id,
            ...
            "llStopMoveToTarget" => {
                self.actions.push(ScriptAction::StopMoveToTarget {
                    object_id: self.ctx.object_id,
                });
                Ok(LSLValue::Integer(0))
            }
            "llLookAt" => {
                ...
                self.actions.push(ScriptAction::LookAt {
                    object_id: self.ctx.object_id,
            ...
            "llStopLookAt" => {
                self.actions.push(ScriptAction::StopLookAt {
                    object_id: self.ctx.object_id,
                });
                Ok(LSLValue::Integer(0))
            }
            "llRotLookAt" => {
                ...
                let str_val = args.get(1).map(|a| a.to_float()).unwrap_or(1.0);
                let damp = args.get(2).map(|a| a.to_float()).unwrap_or(0.2);
                self.actions.push(ScriptAction::RotLookAt {
                    object_id: self.ctx.object_id,
                    rotation: [rot.x, rot.y, rot.z, rot.s],
                    strength: str_val,
                    damping: damp,
            ...
            "llSetPhysicsMaterial" => {
                ...
                self.actions.push(ScriptAction::SetPhysicsMaterial {
                    object_id: self.ctx.object_id,
                    gravity: grav,
                    restitution: rest,
                    friction: fric,
            ...
            "llSetHoverHeight" => {
                ...
                self.actions.push(ScriptAction::SetHoverHeight {
                    object_id: self.ctx.object_id,
                    height: h,
                    water,
                    tau,
            ...
            "llStopHover" => {
                self.actions.push(ScriptAction::StopHover {
                    object_id: self.ctx.object_id,
                });
                Ok(LSLValue::Integer(0))
            }
            "llPushObject" => {
                ...
                self.actions.push(ScriptAction::PushObject {
                    target_id: target,
                    impulse: [imp.x, imp.y, imp.z],
                    angular_impulse: [ang.x, ang.y, ang.z],
                    local,
                });
                Ok(LSLValue::Integer(0))
            }
            "llPassTouches" => {
                ...
                self.actions.push(ScriptAction::SetPassTouches {
                    object_id: self.ctx.object_id,
                    pass,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llPassCollisions" => {
                let pass = args.first().map(|a| a.to_integer() != 0).unwrap_or(false);
                self.actions.push(ScriptAction::SetPassCollisions {
                    object_id: self.ctx.object_id,
                    pass,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llSetForceAndTorque" => {
                ...
                self.actions.push(ScriptAction::SetForceAndTorque {
                    object_id: self.ctx.object_id,
            ...
            "llApplyRotationalImpulse" => {
                let imp = args
                    ...
                    .unwrap_or(LSLVector::zero());
                let local = args.get(1).map(|a| a.to_integer() != 0).unwrap_or(false);
                self.actions.push(ScriptAction::ApplyRotationalImpulse {
                    object_id: self.ctx.object_id,
                    impulse: [imp.x, imp.y, imp.z],
                    local,
                });
            ...
            "llGroundRepel" => {
                ...
                let water = args.get(1).map(|a| a.to_integer()).unwrap_or(0);
                let tau = args.get(2).map(|a| a.to_float()).unwrap_or(1.0);
                self.actions.push(ScriptAction::GroundRepel {
                    object_id: self.ctx.object_id,
                    height: h,
                    water,
                    tau,
            ...
            "llTriggerSound" => {
                ...
                self.actions.push(ScriptAction::TriggerSound {
                    object_id: self.ctx.object_id,
                    sound_id: sound,
            ...
            "llPlaySound" => {
                let sound = args.first().map(|a| a.to_key()).unwrap_or(Uuid::nil());
                let vol = args.get(1).map(|a| a.to_float()).unwrap_or(1.0);
                self.actions.push(ScriptAction::PlaySound {
                    object_id: self.ctx.object_id,
                    sound_id: sound,
                    volume: vol.clamp(0.0, 1.0),
                });
            ...
            "llLoopSound" => {
                let sound = args.first().map(|a| a.to_key()).unwrap_or(Uuid::nil());
                let vol = args.get(1).map(|a| a.to_float()).unwrap_or(1.0);
                self.actions.push(ScriptAction::LoopSound {
                    object_id: self.ctx.object_id,
                    sound_id: sound,
                    volume: vol.clamp(0.0, 1.0),
                });
            ...
            "llLoopSoundMaster" => {
                let sound = args.first().map(|a| a.to_key()).unwrap_or(Uuid::nil());
                let vol = args.get(1).map(|a| a.to_float()).unwrap_or(1.0);
                self.actions.push(ScriptAction::LoopSoundMaster {
                    object_id: self.ctx.object_id,
                    sound_id: sound,
                    volume: vol.clamp(0.0, 1.0),
                });
            ...
            "llLoopSoundSlave" => {
                let sound = args.first().map(|a| a.to_key()).unwrap_or(Uuid::nil());
                let vol = args.get(1).map(|a| a.to_float()).unwrap_or(1.0);
                self.actions.push(ScriptAction::LoopSoundSlave {
                    object_id: self.ctx.object_id,
                    sound_id: sound,
                    volume: vol.clamp(0.0, 1.0),
                });
            ...
            "llPlaySoundSlave" => {
                let sound = args.first().map(|a| a.to_key()).unwrap_or(Uuid::nil());
                let vol = args.get(1).map(|a| a.to_float()).unwrap_or(1.0);
                self.actions.push(ScriptAction::PlaySoundSlave {
                    object_id: self.ctx.object_id,
                    sound_id: sound,
                    volume: vol.clamp(0.0, 1.0),
                });
            ...
            "llStopSound" => {
                self.actions.push(ScriptAction::StopSound {
                    object_id: self.ctx.object_id,
                });
                Ok(LSLValue::Integer(0))
            }
            "llPreloadSound" => {
                let sound = args.first().map(|a| a.to_key()).unwrap_or(Uuid::nil());
                self.actions.push(ScriptAction::PreloadSound {
                    object_id: self.ctx.object_id,
                    sound_id: sound,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llTriggerSoundLimited" => {
                ...
                let tne = args
                    .get(2)
                    .map(|a| a.to_vector())
                    .unwrap_or(LSLVector::new(256.0, 256.0, 256.0));
                let bsw = args
                    .get(3)
                    .map(|a| a.to_vector())
                    .unwrap_or(LSLVector::zero());
                self.actions.push(ScriptAction::TriggerSoundLimited {
                    object_id: self.ctx.object_id,
                    sound_id: sound,
                    volume: vol.clamp(0.0, 1.0),
                    top_ne: [tne.x, tne.y, tne.z],
                    bot_sw: [bsw.x, bsw.y, bsw.z],
                });
                Ok(LSLValue::Integer(0))
            }
            "llAdjustSoundVolume" => {
                let vol = args.first().map(|a| a.to_float()).unwrap_or(1.0);
                self.actions.push(ScriptAction::AdjustSoundVolume {
                    object_id: self.ctx.object_id,
                    volume: vol.clamp(0.0, 1.0),
                });
                Ok(LSLValue::Integer(0))
            ...
            "llSetSoundQueueing" => {
                ...
                self.actions.push(ScriptAction::SetSoundQueueing {
                    object_id: self.ctx.object_id,
                    queueing: q,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llSetSoundRadius" => {
                ...
                self.actions.push(ScriptAction::SetSoundRadius {
                    object_id: self.ctx.object_id,
                    radius: r,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llCollisionSound" => {
                let sound = args.first().map(|a| a.to_key()).unwrap_or(Uuid::nil());
                let vol = args.get(1).map(|a| a.to_float()).unwrap_or(1.0);
                self.actions.push(ScriptAction::SetCollisionSound {
                    object_id: self.ctx.object_id,
                    sound_id: sound,
                    volume: vol.clamp(0.0, 1.0),
                });
            ...
            "llGetLinkName" => {
                ...
                if link == 0 || link == self.ctx.link_num {
                    ...
                    self.ctx.link_names.iter().find(|(num, _)| *num == link)
                {
            ...
            "llStartAnimation" => {
                ...
                if self.ctx.granted_perms & 0x10 != 0 {
                    let resolved = self
                        ...
                        .unwrap_or(anim);
                    self.actions.push(ScriptAction::StartAnimation {
                        avatar_id: self.ctx.perm_granter,
                        anim_name: resolved,
                    });
                }
            ...
            "llStopAnimation" => {
                ...
                if self.ctx.granted_perms & 0x10 != 0 {
                    let resolved = self
                        ...
                        .map(|item| item.asset_id.to_string())
                        .unwrap_or(anim);
                    self.actions.push(ScriptAction::StopAnimation {
                        avatar_id: self.ctx.perm_granter,
                        anim_name: resolved,
                    });
                }
            ...
            "llGetAnimation" => Ok(LSLValue::String("Standing".into())),
            "llGetAnimationList" => Ok(LSLValue::List(vec![])),
            "llSetAnimationOverride" => {
                ...
                if self.ctx.granted_perms & 0x8000 != 0 {
                    self.actions.push(ScriptAction::SetAnimationOverride {
                        avatar_id: self.ctx.perm_granter,
                        anim_state,
                        anim_name,
                    });
            ...
            "llResetAnimationOverride" => {
                let anim_state = args.first().map(|a| a.to_string()).unwrap_or_default();
                if self.ctx.granted_perms & 0x8000 != 0 {
                    self.actions.push(ScriptAction::ResetAnimationOverride {
                        avatar_id: self.ctx.perm_granter,
                        anim_state,
                    });
                }
            ...
            "llHTTPRequest" => {
                ...
                self.actions.push(ScriptAction::HttpRequest {
                    object_id: self.ctx.object_id,
                    script_id: self.script_id,
                    url,
                    params,
            ...
            "llHTTPResponse" => {
                ...
                let body = args.get(2).map(|a| a.to_string()).unwrap_or_default();
                self.actions.push(ScriptAction::HTTPInResponse {
                    request_id,
                    status,
                    body,
            ...
            "llSetContentType" => Ok(LSLValue::Integer(0)),
            "llGetHTTPHeader" => Ok(LSLValue::String(String::new())),
            "llRequestURL" => {
                let request_id = Uuid::new_v4();
                self.actions.push(ScriptAction::RequestURL {
                    object_id: self.ctx.object_id,
                    script_id: self.script_id,
                    request_id,
                    secure: false,
            ...
            | "llExecCharacterCmd" | "llNavigateTo" | "llWanderWithin" | "llPatrolPoints"
            | "llPursue" | "llEvade" | "llFleeFrom" => Ok(LSLValue::Integer(0)),
            ...
            "llClearCameraParams" => {
                if self.ctx.granted_perms & 0x800 != 0 {
                    self.actions.push(ScriptAction::ClearCameraParams {
                        avatar_id: self.ctx.perm_granter,
                    });
                }
                Ok(LSLValue::Integer(0))
            ...
            "llLoadURL" => {
                ...
                self.actions.push(ScriptAction::LoadURL {
                    avatar_id: avatar,
                    message: msg,
                    url,
                    object_name: self.ctx.object_name.clone(),
            ...
            "llMapDestination" => {
                ...
                let pos = args
                    .get(1)
                    .map(|a| a.to_vector())
                    .unwrap_or(LSLVector::new(128.0, 128.0, 0.0));
                let look = args
                    .get(2)
                    .map(|a| a.to_vector())
                    .unwrap_or(LSLVector::zero());
                self.actions.push(ScriptAction::MapDestination {
                    avatar_id: self.ctx.perm_granter,
                    sim_name: sim,
                    position: [pos.x, pos.y, pos.z],
            ...
            "llScaleByFactor" => {
                ...
                if factor > 0.0 {
                    self.actions.push(ScriptAction::ScaleByFactor {
                        object_id: self.ctx.object_id,
                        factor,
                    });
                }
            ...
            "llEmail" => {
                ...
                let body = args.get(2).map(|a| a.to_string()).unwrap_or_default();
                self.actions.push(ScriptAction::Email {
                    object_id: self.ctx.object_id,
                    address: addr,
                    subject: subj,
                    message: body,
            ...
            "llTeleportAgent" => {
                ...
                let pos = args
                    .get(2)
                    .map(|a| a.to_vector())
                    .unwrap_or(LSLVector::new(128.0, 128.0, 21.0));
                let look = args
                    .get(3)
                    .map(|a| a.to_vector())
                    .unwrap_or(LSLVector::new(0.0, 1.0, 0.0));
                self.actions.push(ScriptAction::TeleportAgent {
                    agent_id: agent,
                    landmark: lm,
                    position: [pos.x, pos.y, pos.z],
                    look_at: [look.x, look.y, look.z],
            ...
            "llTeleportAgentHome" => {
                let agent = args.first().map(|a| a.to_key()).unwrap_or(Uuid::nil());
                self.actions
                    .push(ScriptAction::TeleportAgentHome { avatar_id: agent });
                Ok(LSLValue::Integer(0))
            }
            "llEjectFromLand" => {
                let agent = args.first().map(|a| a.to_key()).unwrap_or(Uuid::nil());
                self.actions.push(ScriptAction::EjectFromLand {
                    object_id: self.ctx.object_id,
                    agent_id: agent,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llCastRay" => {
                ...
                while i + 1 < opts.len() {
                    match opts[i].to_integer() {
                        0 => {
                            reject = opts[i + 1].to_integer();
                            i += 2;
                        }
                        2 => {
                            max_hits = opts[i + 1].to_integer();
                            i += 2;
                        }
                        _ => {
                            i += 2;
                ...
                self.actions.push(ScriptAction::CastRay {
                    object_id: self.ctx.object_id,
            ...
            "llResetOtherScript" => {
                ...
                self.actions.push(ScriptAction::ResetOtherScript {
                    object_id: self.ctx.object_id,
                    script_name: sn,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llSetScriptState" => {
                ...
                self.actions.push(ScriptAction::SetScriptState {
                    object_id: self.ctx.object_id,
                    script_name: sn,
                    running,
                });
            ...
            "llSetPayPrice" => {
                ...
                self.actions.push(ScriptAction::SetPayPrice {
                    object_id: self.ctx.object_id,
                    prices,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llCollisionSprite" => {
                self.actions.push(ScriptAction::CollisionSprite {
                    object_id: self.ctx.object_id,
                });
                Ok(LSLValue::Integer(0))
            }
            "llAttachToAvatar" => {
                ...
                self.actions.push(ScriptAction::AttachToAvatar {
                    object_id: self.ctx.object_id,
                    attach_point: point,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llDetachFromAvatar" => {
                self.actions.push(ScriptAction::DetachFromAvatar {
                    object_id: self.ctx.object_id,
                });
                Ok(LSLValue::Integer(0))
            }
            "llGiveMoney" => {
                ...
                self.actions.push(ScriptAction::GiveMoney {
                    owner_id: self.ctx.owner_id,
                    destination_id: dest,
                    amount,
                    object_id: self.ctx.object_id,
            ...
            "llSetParcelMusicURL" => {
                let url = args.first().map(|a| a.to_string()).unwrap_or_default();
                self.actions.push(ScriptAction::SetParcelMusicURL {
                    object_id: self.ctx.object_id,
                    url,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llAddToLandBanList" => {
                ...
                self.actions.push(ScriptAction::AddToLandBanList {
                    object_id: self.ctx.object_id,
                    agent_id: agent,
                    hours,
                    is_ban: true,
            ...
            "llRemoveFromLandBanList" => {
                let agent = args.first().map(|a| a.to_key()).unwrap_or(Uuid::nil());
                self.actions.push(ScriptAction::RemoveFromLandBanList {
                    object_id: self.ctx.object_id,
                    agent_id: agent,
                    is_ban: true,
                });
            ...
            "llResetLandBanList" => {
                self.actions.push(ScriptAction::ResetLandBanList {
                    object_id: self.ctx.object_id,
                    is_ban: true,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llManageEstateAccess" => {
                ...
                self.actions.push(ScriptAction::ManageEstateAccess {
                    object_id: self.ctx.object_id,
                    action,
                    agent_id: agent,
                });
            ...
            "llSetObjectPermMask" => {
                ...
                self.actions.push(ScriptAction::SetObjectPermMask {
                    object_id: self.ctx.object_id,
                    mask_type,
                    mask_value,
                });
            ...
            "llSetInventoryPermMask" => {
                ...
                self.actions.push(ScriptAction::SetInventoryPermMask {
                    object_id: self.ctx.object_id,
                    item_name: item,
                    mask_type,
                    mask_value,
            ...
            "llCreateLink" => {
                ...
                self.actions.push(ScriptAction::CreateLink {
                    object_id: self.ctx.object_id,
                    target_id: target,
                    parent,
                });
            ...
            "llBreakLink" => {
                let link = args.first().map(|a| a.to_integer()).unwrap_or(0);
                self.actions.push(ScriptAction::BreakLink {
                    object_id: self.ctx.object_id,
                    link_num: link,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llBreakAllLinks" => {
                self.actions.push(ScriptAction::BreakAllLinks {
                    object_id: self.ctx.object_id,
                });
                Ok(LSLValue::Integer(0))
            }
            "llReturnObjectsByID" => {
                ...
                self.actions.push(ScriptAction::ReturnObjectsByID {
                    object_id: self.ctx.object_id,
                    ids,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llReturnObjectsByOwner" => {
                ...
                self.actions.push(ScriptAction::ReturnObjectsByOwner {
                    object_id: self.ctx.object_id,
                    owner_id: owner,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llSetPrimMediaParams" => {
                ...
                let mut i = 0;
                while i + 1 < pl.len() {
                    params.push((pl[i].to_integer(), pl[i + 1].to_string()));
                    i += 2;
                }
                self.actions.push(ScriptAction::SetPrimMediaParams {
                    object_id: self.ctx.object_id,
                    face,
                    params,
                });
            ...
            "llClearPrimMedia" => {
                let face = args.first().map(|a| a.to_integer()).unwrap_or(0);
                self.actions.push(ScriptAction::ClearPrimMedia {
                    object_id: self.ctx.object_id,
                    face,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llCollisionFilter" => {
                ...
                self.actions.push(ScriptAction::CollisionFilter {
                    object_id: self.ctx.object_id,
                    name,
                    id,
                    accept,
            ...
            "llSetLinkSitFlags" => {
                ...
                self.actions.push(ScriptAction::SetLinkSitFlags {
                    object_id: self.ctx.object_id,
                    flags,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llLinksetDataReset" => {
                self.actions.push(ScriptAction::LinksetDataReset {
                    object_id: self.ctx.object_id,
                });
                Ok(LSLValue::Integer(0))
            }
            "llDerezObject" => {
                ...
                self.actions.push(ScriptAction::DerezObject {
                    object_id: self.ctx.object_id,
                    target_id: id,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llGetAgentList" => {
                ...
                self.actions.push(ScriptAction::GetAgentList {
                    object_id: self.ctx.object_id,
                    scope,
                });
            ...
            "llMakeExplosion" | "llMakeFire" | "llMakeFountain" | "llMakeSmoke" => {
                ...
                let rules = vec![
                    LSLValue::Integer(0),
                    LSLValue::Integer(flags as i32),
                    LSLValue::Integer(1),
                    LSLValue::Integer(particles.min(100)),
                    LSLValue::Integer(2),
                    LSLValue::Float(if name == "llMakeFire" { 3.0 } else { 1.0 }),
                    LSLValue::Integer(3),
                    LSLValue::Float(0.5),
                    LSLValue::Integer(5),
                    LSLValue::Float(scale),
                    LSLValue::Integer(7),
                    LSLValue::Float(if name == "llMakeExplosion" { 2.0 } else { 0.5 }),
                ];
                let ps_bytes = Self::encode_particle_system_inline(&rules);
                self.actions.push(ScriptAction::ParticleSystem {
                    object_id: self.ctx.object_id,
            ...
            | "llSendRemoteData"
            | "llRemoteDataReply"
            | "llRefreshPrimURL" => Ok(LSLValue::Integer(0)),
            "llListenControl" => {
                let handle = args.first().map(|a| a.to_integer()).unwrap_or(0);
                let active = args.get(1).map(|a| a.to_integer() != 0).unwrap_or(true);
                self.actions.push(ScriptAction::ListenControl {
                    object_id: self.ctx.object_id,
                    handle,
                    active,
                });
            ...
            "llSetLinkTextureAnim" => {
                ...
                self.actions.push(ScriptAction::SetLinkTextureAnim {
                    object_id: self.ctx.object_id,
                    link_num: link,
                    mode,
                    face,
            ...
            "llParcelMediaCommandList" => {
                ...
                self.actions.push(ScriptAction::ParcelMediaCommandList {
                    object_id: self.ctx.object_id,
                    commands: cmds,
                });
                Ok(LSLValue::Integer(0))
            ...
            "osNpcCreate" => {
                ...
                self.actions.push(ScriptAction::NpcCreate {
                    first_name: first,
                    last_name: last,
            ...
            "osNpcSay" => {
                ...
                let ch = args.get(1).map(|a| a.to_integer()).unwrap_or(0);
                let msg = args.get(2).map(|a| a.to_string()).unwrap_or_default();
                self.actions.push(ScriptAction::NpcSay {
                    npc_id,
                    channel: ch,
                    message: msg,
                });
            ...
            "osNpcMoveTo" | "osNpcMoveToTarget" => {
                ...
                self.actions.push(ScriptAction::NpcMoveTo {
                    npc_id,
                    position: [pos.x as f32, pos.y as f32, pos.z as f32],
                    options: opts,
                });
            ...
            "osNpcSetRot" => {
                ...
                let rot = args
                    ...
                    .map(|a| a.to_rotation())
                    .unwrap_or(LSLRotation::identity());
                self.actions.push(ScriptAction::NpcSetRot {
                    npc_id,
            ...
            "osNpcPlayAnimation" => {
                ...
                self.actions.push(ScriptAction::NpcPlayAnimation {
                    npc_id,
                    anim_name: anim,
                });
                Ok(LSLValue::Integer(0))
            ...
            "osNpcStopAnimation" => {
                let npc_id = args.first().map(|a| a.to_key()).unwrap_or(Uuid::nil());
                let anim = args.get(1).map(|a| a.to_string()).unwrap_or_default();
                self.actions.push(ScriptAction::NpcStopAnimation {
                    npc_id,
                    anim_name: anim,
                });
                Ok(LSLValue::Integer(0))
            ...
            "osNpcShout" => {
                ...
                let ch = args.get(1).map(|a| a.to_integer()).unwrap_or(0);
                let msg = args.get(2).map(|a| a.to_string()).unwrap_or_default();
                self.actions.push(ScriptAction::NpcShout {
                    npc_id,
                    channel: ch,
                    message: msg,
                });
            ...
            "osNpcWhisper" => {
                ...
                let ch = args.get(1).map(|a| a.to_integer()).unwrap_or(0);
                let msg = args.get(2).map(|a| a.to_string()).unwrap_or_default();
                self.actions.push(ScriptAction::NpcWhisper {
                    npc_id,
                    channel: ch,
                    message: msg,
                });
            ...
            "osNpcTouch" => {
                ...
                self.actions.push(ScriptAction::NpcTouch {
                    npc_id,
                    target_id: target,
                    link_num: link,
                });
            ...
            "osNpcSit" => {
                let npc_id = args.first().map(|a| a.to_key()).unwrap_or(Uuid::nil());
                let target = args.get(1).map(|a| a.to_key()).unwrap_or(Uuid::nil());
                self.actions.push(ScriptAction::NpcSit {
                    npc_id,
                    target_id: target,
                });
                Ok(LSLValue::Integer(0))
            ...
            "osNpcLoadAppearance" => {
                ...
                self.actions.push(ScriptAction::NpcLoadAppearance {
                    npc_id,
                    notecard: nc,
                });
                Ok(LSLValue::Integer(0))
            ...
            "osNpcSaveAppearance" => {
                let npc_id = args.first().map(|a| a.to_key()).unwrap_or(Uuid::nil());
                let nc = args.get(1).map(|a| a.to_string()).unwrap_or_default();
                self.actions.push(ScriptAction::NpcSaveAppearance {
                    npc_id,
                    notecard: nc,
                });
            ...
            "osNpcGetPos" => Ok(LSLValue::Vector(LSLVector::zero())),
            "osNpcGetRot" => Ok(LSLValue::Rotation(LSLRotation::identity())),
            ...
            "osCreateStatue" => {
                ...
                let name = args
                    .get(4)
                    .map(|a| a.to_string())
                    .unwrap_or_else(|| format!("Statue_{}", anim_name));
            ...
            "osCreateSnapshotStatue" => {
                ...
                let name = args
                    .get(3)
                    .map(|a| a.to_string())
                    .unwrap_or_else(|| format!("Snapshot_{}", anim_name));
            ...
            "osSetTerrainHeight" => {
                ...
                self.actions
                    .push(ScriptAction::SetTerrainHeight { x, y, height: h });
                Ok(LSLValue::Integer(0))
            }
            "osSetTerrainTexture" => {
                let corner = args.first().map(|a| a.to_integer()).unwrap_or(0);
                let tex = args.get(1).map(|a| a.to_key()).unwrap_or(Uuid::nil());
                self.actions.push(ScriptAction::SetTerrainTexture {
                    corner,
                    texture_id: tex,
                });
                Ok(LSLValue::Integer(0))
            ...
            "osSetTerrainTextureHeight" => {
                ...
                self.actions.push(ScriptAction::SetTerrainTextureHeight {
                    corner,
                    low: lo,
                    high: hi,
                });
            ...
            "osSetRegionWaterHeight" => {
                ...
                self.actions
                    .push(ScriptAction::SetRegionWaterHeight { height: h });
                Ok(LSLValue::Integer(0))
            }
            "osSetSunParam" => {
                ...
                self.actions
                    .push(ScriptAction::SetSunParam { param, value: val });
                Ok(LSLValue::Integer(0))
            }
            "osSetWindParam" => {
                let param = args.first().map(|a| a.to_string()).unwrap_or_default();
                let val = args.get(1).map(|a| a.to_float()).unwrap_or(0.0);
                self.actions
                    .push(ScriptAction::SetWindParam { param, value: val });
                Ok(LSLValue::Integer(0))
            }
            "osGetTerrainHeight" => {
            ...
            "osKickAvatar" => {
                let avatar = args.first().map(|a| a.to_key()).unwrap_or(Uuid::nil());
                let msg = args.get(1).map(|a| a.to_string()).unwrap_or_default();
                self.actions.push(ScriptAction::KickAvatar {
                    avatar_id: avatar,
                    message: msg,
                });
                Ok(LSLValue::Integer(0))
            ...
            "osTeleportAgent" => {
                ...
                self.actions.push(ScriptAction::TeleportAgent {
                    ...
                    landmark: sim,
                    position: [pos.x as f32, pos.y as f32, pos.z as f32],
                    look_at: [lookat.x as f32, lookat.y as f32, lookat.z as f32],
                });
                Ok(LSLValue::Integer(0))
            }
            "osSetSpeed" => {
                ...
                self.actions.push(ScriptAction::SetSpeed {
                    avatar_id: avatar,
                    speed,
                });
                Ok(LSLValue::Integer(0))
            ...
            "osForceAttachToAvatar" => {
                let point = args.first().map(|a| a.to_integer()).unwrap_or(0);
                self.actions.push(ScriptAction::ForceAttachToAvatar {
                    object_id: self.ctx.object_id,
                    avatar_id: self.ctx.owner_id,
                    attach_point: point,
                });
                Ok(LSLValue::Integer(0))
            }
            "osForceDetachFromAvatar" => {
                self.actions.push(ScriptAction::ForceDetachFromAvatar {
                    object_id: self.ctx.object_id,
                    avatar_id: self.ctx.owner_id,
                });
                Ok(LSLValue::Integer(0))
            ...
            "osForceOtherSit" => {
                let avatar = args.first().map(|a| a.to_key()).unwrap_or(Uuid::nil());
                let target = args.get(1).map(|a| a.to_key()).unwrap_or(Uuid::nil());
                self.actions.push(ScriptAction::ForceOtherSit {
                    avatar_id: avatar,
                    target_id: target,
                });
                Ok(LSLValue::Integer(0))
            ...
            "osRegionNotice" => {
                let msg = args.first().map(|a| a.to_string()).unwrap_or_default();
                self.actions
                    .push(ScriptAction::RegionNotice { message: msg });
                Ok(LSLValue::Integer(0))
            }
            "osRegionRestart" => {
                let secs = args.first().map(|a| a.to_integer()).unwrap_or(120);
                self.actions.push(ScriptAction::RegionRestart {
                    seconds: secs,
                    message: "Region restart requested by script".into(),
                });
                Ok(LSLValue::Integer(0))
            }
            "osAgentSaveAppearance" => {
                let avatar = args.first().map(|a| a.to_key()).unwrap_or(Uuid::nil());
                let nc = args.get(1).map(|a| a.to_string()).unwrap_or_default();
                self.actions.push(ScriptAction::AgentSaveAppearance {
                    avatar_id: avatar,
                    notecard: nc,
                });
                Ok(LSLValue::Key(Uuid::new_v4()))
            ...
            "osForceAttachToOtherAvatarFromInventory" => {
                ...
                self.actions.push(ScriptAction::ForceAttachToOtherAvatar {
                    object_id: self.ctx.object_id,
                    avatar_id: target,
                    attach_point: point,
                });
            ...
            "osAvatarPlayAnimation" => {
                let avatar = args.first().map(|a| a.to_key()).unwrap_or(Uuid::nil());
                let anim = args.get(1).map(|a| a.to_string()).unwrap_or_default();
                self.actions.push(ScriptAction::AvatarPlayAnimation {
                    avatar_id: avatar,
                    anim_name: anim,
                });
                Ok(LSLValue::Integer(0))
            ...
            "osAvatarStopAnimation" => {
                let avatar = args.first().map(|a| a.to_key()).unwrap_or(Uuid::nil());
                let anim = args.get(1).map(|a| a.to_string()).unwrap_or_default();
                self.actions.push(ScriptAction::AvatarStopAnimation {
                    avatar_id: avatar,
                    anim_name: anim,
                });
                Ok(LSLValue::Integer(0))
            ...
            "osMessageObject" => {
                let target = args.first().map(|a| a.to_key()).unwrap_or(Uuid::nil());
                let msg = args.get(1).map(|a| a.to_string()).unwrap_or_default();
                self.actions.push(ScriptAction::MessageObject {
                    target_id: target,
                    message: msg,
                });
                Ok(LSLValue::Integer(0))
            }
            "osGetAvatarList" => Ok(LSLValue::List(vec![])),
            "osGetNumberOfNotecardLines" => Ok(LSLValue::Integer(0)),

            // ==================== OSSL: Object Animations ====================
            "llStartObjectAnimation" => {
                let anim = args.first().map(|a| a.to_string()).unwrap_or_default();
                self.actions.push(ScriptAction::StartObjectAnimation {
                    object_id: self.ctx.object_id,
                    anim_name: anim,
                });
                Ok(LSLValue::Integer(0))
            ...
            "llStopObjectAnimation" => {
                let anim = args.first().map(|a| a.to_string()).unwrap_or_default();
                self.actions.push(ScriptAction::StopObjectAnimation {
                    object_id: self.ctx.object_id,
                    anim_name: anim,
                });
                Ok(LSLValue::Integer(0))
            ...
            "osSetProjectionParams" => {
                ...
                self.actions.push(ScriptAction::SetProjectionParams {
                    object_id: self.ctx.object_id,
                    enabled,
                    texture_id: tex,
                    fov,
            ...
            "llModifyLand" => {
                ...
                self.actions.push(ScriptAction::ModifyLand {
                    object_id: self.ctx.object_id,
                    action,
                    brush_size,
                });
            ...
            "llSetLinkCamera" => {
                ...
                let at_v = args
                    ...
                    .map(|a| a.to_vector())
                    .unwrap_or(LSLVector::zero());
                self.actions.push(ScriptAction::SetLinkCamera {
                    object_id: self.ctx.object_id,
                    link_num: link,
                    eye: [eye_v.x as f32, eye_v.y as f32, eye_v.z as f32],
                    at: [at_v.x as f32, at_v.y as f32, at_v.z as f32],
                });
                Ok(LSLValue::Integer(0))
            }
            "osSetParcelDetails" => {
                ...
                self.actions.push(ScriptAction::SetParcelDetails {
                    position: [pos.x as f32, pos.y as f32, pos.z as f32],
                    params,
                });
                Ok(LSLValue::Integer(0))
            ...
            | "osGetGridCustom" => Ok(LSLValue::String(String::new())),
            ...
            "osGetPhysicsEngineType" | "osGetPhysicsEngineName" => {
                Ok(LSLValue::String("ODE".into()))
            }
            ...
            "osMakeNotecard" => {
                ...
                self.actions.push(ScriptAction::MakeNotecard {
                    name: nc_name,
                    contents,
                    object_id: self.ctx.object_id,
                });
    ...
    fn encode_particle_system_inline(rules: &[LSLValue]) -> Vec<u8> {
        ...
        let mut i = 0;
        while i + 1 < rules.len() {
            let c = rules[i].to_integer();
            i += 1;
            match c {
                0 => {
                    pdf = rules[i].to_integer() as u32;
                    i += 1;
                }
                1 => {
                ...
                9 => {
                    pat = rules[i].to_integer() as u8;
                    i += 1;
                }
                12 => {
                    tex = Uuid::parse_str(&rules[i].to_string()).unwrap_or(Uuid::nil());
                    i += 1;
                }
                13 => {
                    br = rules[i].to_float();
                ...
                15 => {
                    bpc = rules[i].to_integer().clamp(0, 255) as u8;
                    i += 1;
                }
                16 => {
                    brad = rules[i].to_float();
                    i += 1;
                }
                17 => {
                    bsn = rules[i].to_float();
                    i += 1;
                }
                18 => {
                    bsx = rules[i].to_float();
                    i += 1;
                }
                19 => {
                    ma = rules[i].to_float();
                ...
                24 => {
                    bs = rules[i].to_integer() as u8;
                    hb = true;
                    i += 1;
                }
                25 => {
                    bd = rules[i].to_integer() as u8;
                    hb = true;
                    i += 1;
                }
                26 => {
        ...
        b.push(bpc);
        for v in &av {
            b.extend_from_slice(&pfs16(*v, 8, 7).to_le_bytes());
        }
        for v in &ac {
            b.extend_from_slice(&pfs16(*v, 8, 7).to_le_bytes());
        }
        ...
        for c in &sc {
            b.push((*c * 255.0).clamp(0.0, 255.0) as u8);
        }
        for c in &ec {
            b.push((*c * 255.0).clamp(0.0, 255.0) as u8);
        }
        b.push(pfu8(ss[0], 3, 5));
        b.push(pfu8(ss[1], 3, 5));
        b.push(pfu8(es[0], 3, 5));
        b.push(pfu8(es[1], 3, 5));
        if hg {
            b.push((sg * 255.0).clamp(0.0, 255.0) as u8);
            b.push((eg * 255.0).clamp(0.0, 255.0) as u8);
        }
        if hb {
            b.push(bs);
            b.push(bd);
        ...
        match op {
            Token::Plus => match (left, right) {
                (LSLValue::Integer(a), LSLValue::Integer(b)) => {
                    Ok(LSLValue::Integer(a.wrapping_add(*b)))
                }
            ...
            Token::Minus => match (left, right) {
                (LSLValue::Integer(a), LSLValue::Integer(b)) => {
                    Ok(LSLValue::Integer(a.wrapping_sub(*b)))
                }
                (LSLValue::Float(a), LSLValue::Float(b)) => Ok(LSLValue::Float(a - b)),
                (LSLValue::Integer(a), LSLValue::Float(b)) => Ok(LSLValue::Float(*a as f32 - b)),
                (LSLValue::Float(a), LSLValue::Integer(b)) => Ok(LSLValue::Float(a - *b as f32)),
            ...
            Token::Multiply => match (left, right) {
                (LSLValue::Integer(a), LSLValue::Integer(b)) => {
                    Ok(LSLValue::Integer(a.wrapping_mul(*b)))
                }
                (LSLValue::Float(a), LSLValue::Float(b)) => Ok(LSLValue::Float(a * b)),
                (LSLValue::Integer(a), LSLValue::Float(b)) => Ok(LSLValue::Float(*a as f32 * b)),
                (LSLValue::Float(a), LSLValue::Integer(b)) => Ok(LSLValue::Float(a * *b as f32)),
            ...
            Token::Less => Ok(LSLValue::Integer(if left.to_float() < right.to_float() {
                1
            } else {
                0
            })),
            Token::Greater => Ok(LSLValue::Integer(if left.to_float() > right.to_float() {
                1
            } else {
                0
            })),
            Token::LessEqual => Ok(LSLValue::Integer(if left.to_float() <= right.to_float() {
                1
            } else {
                0
            })),
            Token::GreaterEqual => Ok(LSLValue::Integer(if left.to_float() >= right.to_float() {
                1
            } else {
                0
            })),
    ...
    fn unary_op(&self, op: &Token, val: &LSLValue) -> Result<LSLValue> {
        match op {
            Token::Minus => match val {
                ...
                LSLValue::Vector(v) => Ok(LSLValue::Vector(LSLVector {
                    x: -v.x,
                    y: -v.y,
                    z: -v.z,
                })),
                LSLValue::Rotation(r) => Ok(LSLValue::Rotation(LSLRotation {
                    x: -r.x,
                    y: -r.y,
                    z: -r.z,
                    s: -r.s,
...
"#;
        ...
        assert!(compiled.states["default"]
            .events
            .contains_key("state_entry"));
        Ok(())
    }
