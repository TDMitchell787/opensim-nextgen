...
impl AssetStreamingManager {
    ...
    pub async fn start_stream(
        ...
        info!(
            "Started streaming session {} for asset {} (client: {}, priority: {:?})",
    ...
    pub async fn get_next_chunk(
        ...
        let (final_data, compression) =
            if matches!(session.quality.compression, CompressionType::Gzip) {
                (
                    self.compress_data(&chunk_data, CompressionType::Gzip)?,
        ...
        debug!(
            "Generated chunk {} for session {} ({} bytes)",
            chunk.chunk_id,
            session_id,
            chunk.data.len()
        );
    ...
    pub async fn stop_stream(&self, session_id: &Uuid) -> Result<()> {
        ...
        if let Some(session) = session {
            ...
            info!(
                "Stopped streaming session {} ({}% complete)",
                session_id,
    ...
    pub async fn update_stream_quality(&self, session_id: &Uuid, bandwidth_bps: u64) -> Result<()> {
        ...
        if let Some(session) = active_sessions.get_mut(session_id) {
            ...
            if self.config.enable_adaptive_bitrate {
                ...
                debug!(
                    "Updated stream quality for session {}: bandwidth={}bps, chunk_size={}",
    ...
    fn compress_data(&self, data: &Bytes, compression: CompressionType) -> Result<Bytes> {
        match compression {
            ...
            _ => {
                ...
                warn!(
                    "Unsupported compression type: {:?}, using no compression",
                    compression
                );
                Ok(data.clone())
            }
...
mod tests {
    ...
    async fn test_chunk_streaming() {
        ...
        // Get first chunk
        let chunk1 = manager
            .get_next_chunk(&session_id, &asset_data)
            .await
            .unwrap();
