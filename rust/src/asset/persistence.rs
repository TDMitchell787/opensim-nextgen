...
impl AssetPersistenceManager {
        ...
        let backup_backend = if config.enable_backup {
            match &config.backup_backend {
                ...
                Some(StorageBackendType::FileSystem) => {
                    Some(Box::new(FileSystemBackend::new("./assets_backup")?) as Box<dyn StorageBackend>)
                }
                _ => None,
            }
        } else {
    ...
    pub async fn store_asset(&self, asset_id: Uuid, asset_type: AssetType, data: AssetData) -> Result<()> {
        // Store in primary backend
        self.primary_backend.store_asset(&asset_id, asset_type.clone(), &data).await?;
        
        // Store in backup backend if enabled
        if let Some(backup_backend) = &self.backup_backend {
            if let Err(e) = backup_backend.store_asset(&asset_id, asset_type.clone(), &data).await {
                warn!("Failed to store asset {} in backup backend: {}", asset_id, e);
            }
        }
    ...
    pub async fn delete_asset(&self, asset_id: &Uuid) -> Result<bool> {
        ...
        if self.primary_backend.delete_asset(asset_id).await.unwrap_or(false) {
            deleted = true;
        }
    ...
    pub async fn asset_exists(&self, asset_id: &Uuid) -> Result<bool> {
        ...
        if self.primary_backend.asset_exists(asset_id).await.unwrap_or(false) {
            return Ok(true);
        }
    ...
    pub async fn perform_backup(&self) -> Result<()> {
        ...
        for asset_id in asset_ids {
            ...
            if let Ok(Some((asset_type, data))) = self.primary_backend.retrieve_asset(&asset_id).await {
                if let Err(e) = backup_backend.store_asset(&asset_id, asset_type, &data).await {
...
impl StorageBackend for FileSystemBackend {
    async fn store_asset(&self, asset_id: &Uuid, _asset_type: AssetType, data: &AssetData) -> Result<()> {
...
impl StorageBackend for DatabaseBackend {
    async fn store_asset(&self, _asset_id: &Uuid, _asset_type: AssetType, _data: &AssetData) -> Result<()> {
...
mod tests {
    ...
    async fn test_filesystem_backend() {
        ...
        // Test store
        backend.store_asset(&asset_id, AssetType::Texture, &asset_data).await.unwrap();
    ...
    async fn test_persistence_manager() {
        ...
        manager.store_asset(asset_id, AssetType::Texture, asset_data.clone()).await.unwrap();
