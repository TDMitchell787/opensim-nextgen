...
impl AssetCache {
    ...
    pub async fn new(config: CacheConfig) -> Result<Self> {
        let memory_cache = Arc::new(RwLock::new(LruCache::new(
            std::num::NonZeroUsize::new(config.memory_cache_size).unwrap(),
        )));
        ...
        let redis_client = if let Some(ref redis_url) = config.redis_url {
            match RedisClient::open(redis_url.as_str()) {
                ...
                Err(e) => {
                    warn!(
                        "Failed to create Redis client: {}, continuing without Redis cache",
                        e
                    );
                    None
                }
        ...
        if let Some(ref redis_client) = self.redis_client {
            match self.get_from_redis(redis_client, asset_id).await {
                Ok(Some(asset)) => {
                    ...
                    debug!(
                        "Asset {} found in Redis cache and promoted to memory",
                        asset_id
                    );
    ...
    pub async fn put(&self, asset_id: &str, data: Bytes, asset_type: AssetType) -> Result<()> {
        ...
        if self.config.enable_compression
            && cached_asset.data.len() >= self.config.compression_threshold
        {
        ...
        self.memory_cache
            .write()
            .put(asset_id.to_string(), cached_asset.clone());
    ...
    pub async fn set(&self, asset_id: &str, asset: Arc<crate::asset::Asset>) -> Result<()> {
        self.put(
            asset_id,
            (**asset.data.as_ref().unwrap()).clone(),
    ...
    fn compress_asset(&self, mut asset: CachedAsset) -> Result<CachedAsset> {
        ...
        if compressed_size < original_size {
            ...
            debug!(
                "Asset {} compressed from {} to {} bytes (ratio: {:.2})",
    ...
    async fn put_to_redis(&self, redis_client: &RedisClient, asset: &CachedAsset) -> Result<()> {
        ...
        let _: () = conn
            .set_ex(&key, data, self.config.redis_ttl_seconds)
            .await?;
        Ok(())
    }
