...
pub async fn handle_map_post(State(state): State<RobustState>, body: String) -> impl IntoResponse {
...
fn map_failure_xml(reason: &str) -> String {
    format!(
        r#"<Result><response type="Failure">true</response><message>{}</message></Result>"#,
        reason
    )
}
