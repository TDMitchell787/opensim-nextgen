...
pub async fn handle_mutelist(
    ...
    let xml = match method.to_lowercase().as_str() {
        "get" => {
            let agent_id = params.get("agentid").or_else(|| params.get("AGENTID"))
                    ...
                    Ok(mutes) => {
                        ...
                        for m in &mutes {
                            text.push_str(&format!("{} {} {}|{}\n",
                                m.mute_type, m.mute_id.trim(), m.mute_name.trim(), m.mute_flags));
                        }
        ...
        "update" => {
            let agent_id = params.get("agentid").or_else(|| params.get("AGENTID"))
                .cloned().unwrap_or_default();
            let mute_id = params.get("muteid").or_else(|| params.get("MUTEID"))
                .cloned().unwrap_or_default();
            ...
            if agent_id.is_empty() || mute_id.is_empty() {
                ...
                let mute = MuteData {
                    ...
                    mute_type: params.get("mutetype").and_then(|s| s.parse().ok()).unwrap_or(0),
                    mute_flags: params.get("muteflags").and_then(|s| s.parse().ok()).unwrap_or(0),
                    stamp: params.get("mutestamp").and_then(|s| s.parse().ok()).unwrap_or(now),
                };
