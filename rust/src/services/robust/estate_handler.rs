...
pub async fn handle_estates_get(
    ...
    match result {
        Ok(ids) if ids.is_empty() => (
            ...
            .into_response(),
        Ok(ids) => {
            let items: Vec<HashMap<String, String>> = ids
                .iter()
                .map(|id| {
...
pub async fn handle_estate_get(
    ...
    let result = if let Some(region_id) = params.get("region") {
        ...
        return (
            StatusCode::NOT_FOUND,
            [(header::CONTENT_TYPE, "text/xml; charset=utf-8")],
            failure_result("Missing region or eid parameter"),
        )
            .into_response();
    };
...
pub async fn handle_estate_regions(
    ...
    match svc.get_regions(eid).await {
        Ok(regions) => {
            let items: Vec<HashMap<String, String>> = regions
                .iter()
                .map(|r| {
                    let mut m = HashMap::new();
...
fn parse_estate_from_params(params: &HashMap<String, String>) -> EstateSettings {
    EstateSettings {
        ...
        estate_name: params
            .get("EstateName")
            .cloned()
            .unwrap_or_else(|| "My Estate".to_string()),
        estate_owner: params
            .get("EstateOwner")
            .cloned()
            .unwrap_or_else(|| "00000000-0000-0000-0000-000000000000".to_string()),
        parent_estate_id: params
            .get("ParentEstateID")
            .and_then(|s| s.parse().ok())
            .unwrap_or(0),
