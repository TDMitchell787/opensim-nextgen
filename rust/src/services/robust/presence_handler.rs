...
pub async fn handle_presence(State(state): State<RobustState>, body: String) -> impl IntoResponse {
...
async fn handle_login(state: &RobustState, params: &HashMap<String, String>) -> String {
    ...
    match state
        .presence_service
        .login_agent(user_id, session_id, secure_session_id, region_id)
        .await
    {
...
async fn handle_report(state: &RobustState, params: &HashMap<String, String>) -> String {
    ...
    match state
        .presence_service
        .report_agent(session_id, region_id)
        .await
    {
        Ok(true) => success_result(),
        Ok(false) => failure_result("Session not found"),
    ...
    match state.presence_service.get_agents(&user_ids).await {
        Ok(agents) => {
            let items: Vec<HashMap<String, String>> =
                agents.iter().map(|a| presence_to_fields(a)).collect();
...
fn presence_to_fields(info: &PresenceInfo) -> HashMap<String, String> {
    ...
    m.insert(
        "SecureSessionID".to_string(),
        info.secure_session_id.to_string(),
    );
    m.insert("RegionID".to_string(), info.region_id.to_string());
    m.insert(
        "Online".to_string(),
        if info.online { "True" } else { "False" }.to_string(),
    );
