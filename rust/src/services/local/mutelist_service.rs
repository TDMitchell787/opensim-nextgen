...
use crate::services::traits::{MuteListServiceTrait, MuteData};
...
impl MuteListServiceTrait for LocalMuteListService {
    ...
    async fn update_mute(&self, mute: &MuteData) -> Result<bool> {
        ...
        debug!("[MUTELIST] Updated mute for {} -> {}", mute.agent_id, mute.mute_name);
        Ok(true)
    }
    ...
    async fn remove_mute(&self, agent_id: &str, mute_id: &str, mute_name: &str) -> Result<bool> {
        ...
        debug!("[MUTELIST] Removed mute for {} -> {} ({})", agent_id, mute_name, result.rows_affected());
