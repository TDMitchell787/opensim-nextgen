...
use crate::services::traits::{FriendInfo, FriendsServiceTrait};
...
impl FriendsServiceTrait for LocalFriendsService {
    ...
    async fn delete_friend(&self, principal_id: &str, friend: &str) -> Result<bool> {
        ...
        let result =
            sqlx::query(r#"DELETE FROM friends WHERE "PrincipalID" = $1 AND "Friend" = $2"#)
                .bind(principal_id)
                .bind(friend)
                .execute(pool)
                .await?;
