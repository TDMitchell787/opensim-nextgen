...
impl HGFriendsServiceTrait for HGFriendsService {
    ...
    async fn new_friendship(&self, principal_id: Uuid, friend_id: &str, secret: &str, verified: bool) -> Result<bool> {
        ...
        let secret_to_use = if secret.is_empty() { Self::generate_secret() } else { secret.to_string() };
        ...
        info!("[HG-FRIENDS] New friendship: {} -> {} (verified={})", principal_id, friend_id, verified);
        Ok(true)
    }
    ...
    async fn delete_friendship(&self, principal_id: Uuid, friend_id: &str, secret: &str) -> Result<bool> {
        ...
        if let Some(ref stored) = stored_secret {
            if stored != secret {
                warn!("[HG-FRIENDS] Delete friendship secret mismatch for {} -> {}", principal_id, friend_id);
                return Ok(false);
            }
        }

        sqlx::query(
            "DELETE FROM friends WHERE \"PrincipalID\" = $1 AND \"Friend\" = $2"
        )
        .bind(principal_id.to_string())
        .bind(friend_id)
        .execute(self.db_pool.as_ref())
        ...
        info!("[HG-FRIENDS] Deleted friendship: {} <-> {}", principal_id, friend_id);
        Ok(true)
    }
    ...
    async fn status_notification(&self, friends: &[String], user_id: Uuid, online: bool) -> Result<Vec<String>> {
        ...
        for friend_id in friends {
            let _friend_uuid = if let Ok(u) = Uuid::parse_str(friend_id) { u } else { continue };

            let has_perm = self.get_friend_perms(user_id, friend_id).await.unwrap_or(-1);
            if has_perm < 0 {
                continue;
            }

            debug!("[HG-FRIENDS] Status notification: {} is {}", friend_id, if online { "online" } else { "offline" });
