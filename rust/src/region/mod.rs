...
use crate::region::avatar::{appearance::Appearance, Avatar};
...
impl RegionManager {
    ...
    pub fn new(physics_bridge: Arc<PhysicsBridge>, state_manager: Arc<StateManager>) -> Self {
        Self {
            physics_bridge,
            state_manager,
    ...
    pub async fn create_region(
        ...
        self.regions
            .write()
            .await
            .insert(region_id, scene_arc.clone());
    ...
    pub async fn get_region_info(
        &self,
        region_id: RegionId,
    ) -> Result<RegionInfoDetails, RegionError> {
        let regions = self.regions.read().await;
    ...
    pub async fn get_least_loaded_region(&self) -> Result<RegionInfoDetails, RegionError> {
        ...
        if let Some(region_id) = least_loaded_region {
            ...
            Err(RegionError::Internal(
                "Failed to find least loaded region".to_string(),
            ))
        }
    }
    ...
    pub async fn load_heightmap(
        &self,
        region_id: RegionId,
        file_path: &str,
    ) -> Result<(), RegionError> {
        tracing::info!(
            "Loading heightmap for region {:?} from {}",
            region_id,
            file_path
        );
    ...
    pub async fn save_heightmap(
        &self,
        region_id: RegionId,
        file_path: &str,
    ) -> Result<(), RegionError> {
        tracing::info!(
            "Saving heightmap for region {:?} to {}",
            region_id,
            file_path
        );
    ...
    pub async fn load_xml(&self, region_id: RegionId, file_path: &str) -> Result<(), RegionError> {
        tracing::info!(
            "Loading region data from XML file: {} for region {:?}",
            file_path,
            region_id
        );
    ...
    pub async fn save_xml(&self, region_id: RegionId, file_path: &str) -> Result<(), RegionError> {
        tracing::info!(
            "Saving region {:?} data to XML file: {}",
            region_id,
            file_path
        );
    ...
    pub async fn load_oar(&self, region_id: RegionId, file_path: &str) -> Result<(), RegionError> {
        tracing::info!(
            "Loading region from OAR file: {} for region {:?}",
            file_path,
            region_id
        );
