...
pub async fn handle_seed_capability(
    ...
    let agent_uuid = match Uuid::parse_str(&agent_id) {
        ...
        Err(e) => {
            warn!(
                "Invalid agent ID in seed capability request: {} - {}",
                agent_id, e
            );
    ...
    debug!(
        "Returning {} capabilities for agent {}",
...
pub async fn handle_seed_capability_with_uuid(
    ...
    debug!(
        "Handling seed capability request for agent: {} with UUID: {}",
        agent_id, cap_uuid
    );
...
impl SeedCapabilityService {
    ...
    pub fn new(base_url: String) -> Self {
        info!(
            "Initializing seed capability service with base URL: {}",
            base_url
        );
        Self {
            base_url,
    ...
    pub fn create_seed_capability(&mut self, agent_id: Uuid) -> String {
        ...
        debug!(
            "Created seed capability URL for agent {}: {}",
            agent_id, seed_url
        );
        seed_url
    }
...
mod tests {
    ...
    fn test_filter_capabilities() {
        ...
        agent_caps.add_capability(Capability::new(
            "Cap1".to_string(),
            "http://test.com/cap/1".to_string(),
        ));
        agent_caps.add_capability(Capability::new(
            "Cap2".to_string(),
            "http://test.com/cap/2".to_string(),
        ));
        agent_caps.add_capability(Capability::new(
            "Cap3".to_string(),
            "http://test.com/cap/3".to_string(),
        ));
