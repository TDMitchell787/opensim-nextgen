...
pub fn create_avatar_api_router(avatar_manager: Arc<AdvancedAvatarManager>) -> Router {
    ...
    Router::new()
        ...
        .route(
            "/avatars/:avatar_id/appearance",
            get(get_appearance_endpoint),
        )
        .route(
            "/avatars/:avatar_id/appearance",
            put(update_appearance_endpoint),
        )
        .route("/avatars/:avatar_id/wearables", post(add_wearable_endpoint))
        .route(
            "/avatars/:avatar_id/wearables/:wearable_type",
            delete(remove_wearable_endpoint),
        )
        .route(
            "/avatars/:avatar_id/attachments",
            post(add_attachment_endpoint),
        )
        .route(
            "/avatars/:avatar_id/attachments/:item_id",
            delete(remove_attachment_endpoint),
        )
        .route(
            "/avatars/:avatar_id/visual-params",
            put(update_visual_params_endpoint),
        )
        // Avatar behavior endpoints
        .route("/avatars/:avatar_id/behavior", get(get_behavior_endpoint))
        .route(
            "/avatars/:avatar_id/behavior",
            put(update_behavior_endpoint),
        )
        .route(
            "/avatars/:avatar_id/animations",
            post(start_animation_endpoint),
        )
        .route(
            "/avatars/:avatar_id/animations/:animation_id",
            delete(stop_animation_endpoint),
        )
        .route(
            "/avatars/:avatar_id/animations",
            get(get_active_animations_endpoint),
        )
        .route(
            "/avatars/:avatar_id/gestures/:gesture_id",
            post(trigger_gesture_endpoint),
        )
        .route(
            "/avatars/:avatar_id/expressions",
            put(update_expression_endpoint),
        )
        // Avatar social endpoints
        .route(
            "/avatars/:avatar_id/social",
            get(get_social_profile_endpoint),
        )
        .route(
            "/avatars/:avatar_id/social",
            put(update_social_profile_endpoint),
        )
        .route("/avatars/:avatar_id/friends", get(get_friends_endpoint))
        .route(
            "/avatars/:avatar_id/friends/:friend_id",
        ...
        .route(
            "/avatars/:avatar_id/achievements",
            get(get_achievements_endpoint),
        )
        .route(
            "/avatars/:avatar_id/achievements",
            post(add_achievement_endpoint),
        )
        ...
        .route(
            "/avatars/:avatar_id/position",
            put(update_position_endpoint),
        )
        .route(
            "/avatars/:avatar_id/statistics",
            get(get_statistics_endpoint),
        )
        // System endpoints
        .route("/avatars/active", get(get_active_avatars_endpoint))
        .route(
            "/avatars/region/:region_id",
            get(get_avatars_in_region_endpoint),
        )
...
async fn create_avatar_endpoint(
    ...
    match state
        .avatar_manager
        .create_avatar(request.user_id, request.name, request.initial_appearance)
        .await
    {
...
async fn delete_avatar_endpoint(
    ...
    match state.avatar_manager.delete_avatar(avatar_id).await {
        Ok(_) => Ok(Json(ApiResponse::success(
            "Avatar deleted successfully".to_string(),
        ))),
        Err(AvatarError::NotFound { .. }) => Err(StatusCode::NOT_FOUND),
        Err(e) => {
...
async fn update_appearance_endpoint(
    ...
    match state
        .avatar_manager
        .update_appearance(avatar_id, appearance)
        .await
    {
        Ok(_) => Ok(Json(ApiResponse::success(
            "Appearance updated successfully".to_string(),
        ))),
        Err(AvatarError::NotFound { .. }) => Err(StatusCode::NOT_FOUND),
        Err(e) => {
    ...
    match state.avatar_manager.get_avatar(avatar_id).await {
        Ok(mut avatar) => {
            ...
            match state
                .avatar_manager
                .appearance_engine
                .update_wearable(&mut avatar.appearance, wearable)
            {
                Ok(_) => {
                    // Save updated appearance
                    match state
                        .avatar_manager
                        .update_appearance(avatar_id, avatar.appearance)
                        .await
                    {
                        Ok(_) => Ok(Json(ApiResponse::success(
                            "Wearable added successfully".to_string(),
                        ))),
                        Err(e) => {
    ...
    match state.avatar_manager.get_avatar(avatar_id).await {
        Ok(mut avatar) => {
            match state
                .avatar_manager
                .appearance_engine
                .remove_wearable(&mut avatar.appearance, wearable_type)
            {
                Ok(removed) => {
                    if removed {
                        match state
                        ...
                        {
                            Ok(_) => Ok(Json(ApiResponse::success(
                                "Wearable removed successfully".to_string(),
                            ))),
                            Err(e) => {
...
async fn add_attachment_endpoint(
    ...
    Ok(Json(ApiResponse::success(
        "Attachment added successfully".to_string(),
    )))
}
...
async fn remove_attachment_endpoint(
    ...
    Ok(Json(ApiResponse::success(
        "Attachment removed successfully".to_string(),
    )))
}
...
async fn update_visual_params_endpoint(
    ...
    Ok(Json(ApiResponse::success(
        "Visual parameters updated successfully".to_string(),
    )))
}
...
async fn update_behavior_endpoint(
    ...
    Ok(Json(ApiResponse::success(
        "Behavior updated successfully".to_string(),
    )))
}
...
async fn start_animation_endpoint(
    ...
    Ok(Json(ApiResponse::success(
        "Animation started successfully".to_string(),
    )))
}
...
async fn stop_animation_endpoint(
    ...
    Ok(Json(ApiResponse::success(
        "Animation stopped successfully".to_string(),
    )))
}
...
async fn trigger_gesture_endpoint(
    ...
    Ok(Json(ApiResponse::success(
        "Gesture triggered successfully".to_string(),
    )))
}
...
async fn update_expression_endpoint(
    ...
    Ok(Json(ApiResponse::success(
        "Expression updated successfully".to_string(),
    )))
}
...
async fn update_social_profile_endpoint(
    ...
    Ok(Json(ApiResponse::success(
        "Social profile updated successfully".to_string(),
    )))
}
...
async fn add_friend_endpoint(
    ...
    Ok(Json(ApiResponse::success(
        "Friend added successfully".to_string(),
    )))
}
...
async fn remove_friend_endpoint(
    ...
    Ok(Json(ApiResponse::success(
        "Friend removed successfully".to_string(),
    )))
}
...
async fn add_achievement_endpoint(
    ...
    Ok(Json(ApiResponse::success(
        "Achievement added successfully".to_string(),
    )))
}
...
async fn login_avatar_endpoint(
    ...
    Ok(Json(ApiResponse::success(
        "Avatar logged in successfully".to_string(),
    )))
}
...
async fn logout_avatar_endpoint(
    ...
    Ok(Json(ApiResponse::success(
        "Avatar logged out successfully".to_string(),
    )))
}
...
async fn update_position_endpoint(
    ...
    Ok(Json(ApiResponse::success(
        "Position updated successfully".to_string(),
    )))
}
