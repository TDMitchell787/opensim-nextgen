...
impl DarkServicesManager {
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        info!("Starting dark services on {}:{}", self.config.bind_address, self.config.port);
    ...
    pub async fn request_transfer(&self, request: DarkTransferRequest) -> Result<DarkTransferResponse> {
        info!("Received dark transfer request for asset {}", request.asset_id);
        ...
        let response = DarkTransferResponse {
            ...
            transfer_url: Some(format!("dark://{}:{}/transfer/{}", 
        ...
        info!("Dark transfer session {} created for asset {}", 
    ...
    pub async fn execute_transfer(&self, session_id: Uuid) -> Result<Vec<AssetChunk>> {
        ...
        info!("Dark transfer completed for session {} ({} chunks)", 
              session_id, chunks.len());
    ...
    async fn validate_transfer_request(&self, request: &DarkTransferRequest) -> Result<()> {
        ...
        // Get asset info
        let asset_info = self.asset_manager.get_asset_info(&request.asset_id).await?
        ...
        if asset_info.original_size as u64 > self.config.max_asset_size {
            return Err(anyhow!("Asset too large: {} bytes (max: {} bytes)", 
                asset_info.original_size, self.config.max_asset_size));
        }
        
        // Check asset type (using content_type since AssetMetadata doesn't have asset_type)
        let asset_type_str = asset_info.content_type.split('/').next().unwrap_or(&asset_info.content_type);
        if !self.config.allowed_asset_types.iter().any(|t| asset_info.content_type.contains(t)) {
            return Err(anyhow!("Asset type not allowed for dark transfer: {}", 
                asset_info.content_type));
        }
        
        // Check security level
        if request.security_level > self.config.security_level {
            return Err(anyhow!("Requested security level {} exceeds maximum {}", 
                request.security_level, self.config.security_level));
        }
        ...
        if active_count >= self.config.max_concurrent_transfers as usize {
            return Err(anyhow!("Maximum concurrent transfers exceeded: {}", active_count));
        }
        ...
        if let Some(policy) = policies.get(&request.asset_id) {
            ...
            if !policy.allowed_requesters.is_empty() && 
               !policy.allowed_requesters.contains(&request.requester_id) {
                return Err(anyhow!("Requester not authorized for asset: {}", request.requester_id));
            }
            ...
            debug!("No access policy found for asset {}, using default", request.asset_id);
        }
    ...
    async fn create_transfer_session(&self, request: &DarkTransferRequest) -> Result<DarkTransferSession> {
        let asset_info = self.asset_manager.get_asset_info(&request.asset_id).await?
            .ok_or_else(|| anyhow!("Asset metadata not found: {}", request.asset_id))?;
        let asset_data = self.asset_manager.get_asset_data(&request.asset_id).await?
        ...
        // Get access policy
        let access_policy = self.get_access_policy(&request.asset_id).await
    ...
    async fn create_asset_chunks(&self, session: &DarkTransferSession, asset_data: Bytes) -> Result<Vec<AssetChunk>> {
        ...
        for (i, chunk_data) in asset_data.chunks(chunk_size).enumerate() {
            ...
            let is_encrypted = if !session.encryption_key.is_empty() {
                chunk_bytes = self.crypto_manager.encrypt_data(&chunk_bytes, &session.encryption_key)?;
                true
            } else {
                false
            };
    ...
    async fn process_transfer_request(&self, request: DarkTransferRequest) -> Result<()> {
        match self.request_transfer(request).await {
            Ok(response) => {
                info!("Transfer request processed successfully: {}", response.session_id);
                Ok(())
            }
            Err(e) => {
            ...
            loop {
                ...
                if !expired_sessions.is_empty() {
                    ...
                    for session_id in expired_sessions {
                        ...
                        metrics_lock.active_sessions = metrics_lock.active_sessions.saturating_sub(1);
