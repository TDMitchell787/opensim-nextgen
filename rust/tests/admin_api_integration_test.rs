...
async fn test_create_user_api() -> Result<()> {
    ...
    if response.status().is_success() {
        ...
        println!("Create user result: {}", serde_json::to_string_pretty(&result)?);
...
async fn test_list_users_api() -> Result<()> {
    ...
    if response.status().is_success() {
        let result: serde_json::Value = response.json().await?;
        println!("List users result: {}", serde_json::to_string_pretty(&result)?);
...
async fn test_reset_password_api() -> Result<()> {
    ...
    if response.status().is_success() {
        let result: serde_json::Value = response.json().await?;
        println!("Reset password result: {}", serde_json::to_string_pretty(&result)?);
...
async fn test_database_stats_api() -> Result<()> {
    ...
    if response.status().is_success() {
        let result: serde_json::Value = response.json().await?;
        println!("Database stats result: {}", serde_json::to_string_pretty(&result)?);
...
async fn test_admin_health_api() -> Result<()> {
    ...
    if response.status().is_success() {
        let result: serde_json::Value = response.json().await?;
        println!("Admin health result: {}", serde_json::to_string_pretty(&result)?);
...
async fn test_user_lifecycle() -> Result<()> {
    ...
    if create_response.status().is_success() {
        ...
        println!("User created: {}", serde_json::to_string_pretty(&create_result)?);
    }
    
    // 2. Show user account
    let show_response = config.client
        .get(&format!("{}/admin/users/account?firstname={}&lastname=User", 
    ...
    if show_response.status().is_success() {
        ...
        println!("User account: {}", serde_json::to_string_pretty(&show_result)?);
    }
    ...
    if update_response.status().is_success() {
        ...
        println!("User level updated: {}", serde_json::to_string_pretty(&update_result)?);
    }
    ...
    if delete_response.status().is_success() {
        ...
        println!("User deleted: {}", serde_json::to_string_pretty(&delete_result)?);
    }
...
async fn test_rate_limiting() -> Result<()> {
    ...
    for i in 0..70 { // More than typical rate limit
        let response = config.client
            .get(&format!("{}/admin/health", config.api_base_url))
            .header("X-API-Key", &config.api_key)
            .send()
...
async fn require_server_running() -> Result<bool> {
    ...
    match response {
        ...
        _ => {
            println!("⚠️  OpenSim Next server not running on {}", config.api_base_url);
