...
async fn test_user_social_profile_management() -> Result<()> {
    ...
    let profile = server.create_user_social_profile(user_id, display_name.clone()).await?;
    assert_eq!(profile.user_id, user_id);
    assert_eq!(profile.display_name, display_name);
    assert_eq!(profile.social_statistics.friend_count, 0);
    
    // Update online status
    server.update_user_online_status(user_id, OnlineStatus::Online).await?;
...
async fn test_friend_system_workflow() -> Result<()> {
    ...
    server.create_user_social_profile(user1_id, "User1".to_string()).await?;
    server.create_user_social_profile(user2_id, "User2".to_string()).await?;
    
    // Send friend request
    let friend_request = server.send_friend_request(
        user1_id,
        user2_id,
...
async fn test_messaging_system() -> Result<()> {
    ...
    server.create_user_social_profile(user1_id, "Sender".to_string()).await?;
    server.create_user_social_profile(user2_id, "Receiver".to_string()).await?;
    ...
    assert_eq!(message.content.text, Some("Hello, how are you?".to_string()));
...
async fn test_group_management() -> Result<()> {
    ...
    server.create_user_social_profile(owner_id, "GroupOwner".to_string()).await?;
...
async fn test_social_search_functionality() -> Result<()> {
    ...
    // Create social profile
    server.create_user_social_profile(user_id, "SearchUser".to_string()).await?;
    ...
    let search_results = server.social_manager()
        .search_social_content(user_id, search_criteria).await?;
...
async fn test_social_statistics_generation() -> Result<()> {
    ...
    // Create social profile
    server.create_user_social_profile(user_id, "StatsUser".to_string()).await?;
    
    // Get user social statistics
    let user_stats = server.social_manager()
        .get_user_social_statistics(user_id).await?;
    ...
    let system_stats = server.social_manager()
        .get_system_social_statistics().await?;
...
async fn test_comprehensive_social_workflow() -> Result<()> {
    ...
    server.create_user_social_profile(user1_id, "Alice".to_string()).await?;
    server.create_user_social_profile(user2_id, "Bob".to_string()).await?;
    server.create_user_social_profile(user3_id, "Charlie".to_string()).await?;
    
    // Update online status
    server.update_user_online_status(user1_id, OnlineStatus::Online).await?;
    server.update_user_online_status(user2_id, OnlineStatus::Online).await?;
    server.update_user_online_status(user3_id, OnlineStatus::Away).await?;
    
    // Send friend requests
    let _request1 = server.send_friend_request(user1_id, user2_id, Some("Hi Bob!".to_string())).await?;
    let _request2 = server.send_friend_request(user1_id, user3_id, Some("Hi Charlie!".to_string())).await?;
    ...
    let stats = server.social_manager().get_user_social_statistics(user1_id).await?;
