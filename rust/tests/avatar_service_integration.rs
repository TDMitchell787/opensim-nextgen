...
async fn test_avatar_service_create_default_appearance() -> anyhow::Result<()> {
    ...
    let avatar = avatar_service
        .get_or_create_default_appearance(don_mclean_uuid)
        .await?;
    ...
    assert!(
        avatar.data.contains_key("AvatarHeight"),
        "Should have AvatarHeight"
    );
    assert_eq!(
        avatar.data.get("AvatarHeight"),
        Some(&"1.771488".to_string())
    );

    assert!(avatar.data.contains_key("BodyItem"), "Should have BodyItem");
    assert!(
        avatar.data.contains_key("BodyAsset"),
        "Should have BodyAsset"
    );
    assert!(avatar.data.contains_key("SkinItem"), "Should have SkinItem");
    assert!(
        avatar.data.contains_key("SkinAsset"),
        "Should have SkinAsset"
    );
    assert!(avatar.data.contains_key("HairItem"), "Should have HairItem");
    assert!(
        avatar.data.contains_key("HairAsset"),
        "Should have HairAsset"
    );
    assert!(avatar.data.contains_key("EyesItem"), "Should have EyesItem");
    assert!(
        avatar.data.contains_key("EyesAsset"),
        "Should have EyesAsset"
    );

    assert!(
        avatar.data.contains_key("VisualParams"),
        "Should have VisualParams"
    );
    ...
    assert!(
        avatar.data.contains_key("Wearable 0:0"),
        "Should have Wearable 0:0 (Shape)"
    );
    assert!(
        avatar.data.contains_key("Wearable 1:0"),
        "Should have Wearable 1:0 (Skin)"
    );
    assert!(
        avatar.data.contains_key("Wearable 2:0"),
        "Should have Wearable 2:0 (Hair)"
    );
    assert!(
        avatar.data.contains_key("Wearable 3:0"),
        "Should have Wearable 3:0 (Eyes)"
    );
    ...
    assert_eq!(
        reloaded_avatar.data.get("AvatarHeight"),
        Some(&"1.771488".to_string())
    );
