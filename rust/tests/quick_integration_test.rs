...
use opensim_next::physics::{PhysicsEngineType, MultiPhysicsSystem};
...
async fn test_physics_engine_creation() {
    ...
    let result = physics_system.create_manager(
        "test-region".to_string(),
        PhysicsEngineType::ODE,
        config,
    ).await;
    
    assert!(result.is_ok(), "Failed to create physics manager: {:?}", result.err());
...
fn test_physics_types() {
    ...
    use opensim_next::physics::{Vec3, Quat, PhysicsEngineType};
