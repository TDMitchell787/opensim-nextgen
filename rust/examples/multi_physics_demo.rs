...
use opensim_next::physics::{
    config::PhysicsEngineConfig, MultiPhysicsSystem, PhysicsBodyData, PhysicsEngineType,
    PhysicsShape, Quat, Vec3,
};
...
async fn main() -> Result<()> {
    ...
    let avatar_manager = physics_system
        .create_manager(
    ...
    let vehicle_manager = physics_system
        .create_manager(
    ...
    let large_world_manager = physics_system
        .create_manager(
    ...
    let particle_manager = physics_system
        .create_manager(
    ...
    let test_manager = physics_system
        .create_manager(
    ...
    info!(
        "👤 Created avatar in Avatar Region (ODE): {:?}",
        avatar_handle
    );
    ...
    info!(
        "🚗 Created vehicle in Vehicle Region (Bullet): {:?}",
        vehicle_handle
    );
    ...
    info!(
        "💧 Created particle in Particle Region (POS): {:?}",
        particle_handle
    );
    ...
    for (region_id, stats) in all_stats {
        info!(
            "📈 {}: {} engine - {} active bodies, {:.2} ms step time, {:.1} FPS",
    ...
    match Arc::try_unwrap(avatar_manager_mut) {
        Ok(mut manager) => {
            ...
            manager
                .switch_engine(PhysicsEngineType::Basic, basic_config)
                .await?;
            ...
            info!(
                "👤 Avatar position after engine switch: ({:.2}, {:.2}, {:.2})",
