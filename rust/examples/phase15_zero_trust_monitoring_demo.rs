...
use opensim_next::{
    ziti::{
        config::ZitiConfig,
        monitoring::{ZitiAdvancedMonitoring, AlertSeverity},
    },
...
async fn main() -> PhysicsResult<()> {
    ...
    for i in 1..=5 {
        ...
        monitoring.record_connection(&connection_id, &service_name, &identity_name).await?;
        println!("   📡 Connection established: {} -> {}", identity_name, service_name);
    }
    ...
    for i in 1..=5 {
        ...
        monitoring.record_data_sent(&connection_id, (1024 * i) as usize, Some(latency)).await?;
        monitoring.record_data_received(&connection_id, (512 * i) as usize, Some(latency)).await?;
        
        println!("   📤 Data transfer: {} bytes (latency: {:?})", 1536 * i, latency);
    }
    ...
    let alert_id = monitoring.create_alert_rule(
    ...
    println!("📦 Data Transferred: {} bytes", analytics.total_data_transferred);
    ...
    if let Some(performance) = monitoring.get_performance_metrics().await? {
        ...
        println!("✅ Connection Success Rate: {:.1}%", performance.connection_success_rate);
        println!("🟢 Service Availability: {:.1}%", performance.service_availability);
        println!("💾 Resource Utilization: {:.1}%", performance.resource_utilization);
        println!();
    }
    ...
    for (conn_id, metrics) in connections {
        ...
        println!("   🛡️  Security Violations: {}", metrics.security_violations);
    ...
    println!("🔗 Active Connections: {}", final_analytics.active_connections);
    println!("📦 Total Data Transferred: {} bytes", final_analytics.total_data_transferred);
    println!("⚡ Performance Score: {:.1}%", final_analytics.performance_score);
    println!();
