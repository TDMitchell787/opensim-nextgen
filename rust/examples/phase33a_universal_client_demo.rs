...
use anyhow::Result;
use opensim_next::mobile::{MobilePlatform, DeviceInfo, ThermalState};
use opensim_next::pwa::{PlatformInfo, DeviceType, NotificationType, PushPayload, NotificationAction};
use opensim_next::sync::{ClientPlatform, DataType, SyncDirection, SyncCapabilities, ConflictStrategy};
use uuid::Uuid;
...
async fn demo_mobile_vr_applications(server: &OpenSimServer, user_id: Uuid) -> Result<()> {
    ...
    let mobile_session = server.create_mobile_session(
        user_id,
    ...
    let ios_session = server.create_mobile_session(
        user_id,
        MobilePlatform::iOS,
        ios_device_info
    ).await?;
...
async fn demo_progressive_web_app(server: &OpenSimServer, user_id: Uuid) -> Result<()> {
    ...
    let pwa_session = server.create_pwa_session(
        user_id,
    ...
    let push_payload = PushPayload {
        ...
        data: {
            ...
            data.insert("action".to_string(), serde_json::Value::String("open_world".to_string()));
    ...
    server.send_push_notification(user_id, NotificationType::SystemNotification, push_payload).await?;
    ...
    println!("✅ Service Worker script generated ({} bytes)", service_worker.len());
...
async fn demo_cross_platform_sync(server: &OpenSimServer, user_id: Uuid) -> Result<()> {
    ...
    let desktop_session = server.create_sync_session(
        user_id,
    ...
    let mobile_sync_session = server.create_sync_session(
        user_id,
    ...
    let sync_result = server.sync_data(
        desktop_session,
        DataType::Avatar,
    ...
    let mobile_sync_result = server.sync_data(
        mobile_sync_session,
        DataType::Avatar,
    ...
    server.resolve_sync_conflict(conflict_id, ConflictStrategy::UserChoice).await?;
...
async fn test_phase33a_integration() -> Result<()> {
    ...
    let sync_session = server.create_sync_session(
        user_id,
        ClientPlatform::Desktop,
