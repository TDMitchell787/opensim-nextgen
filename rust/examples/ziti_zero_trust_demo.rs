...
use opensim_next::ziti::{
    ZitiNetworkManager, 
    config::ZitiConfig,
    integration::{OpenSimZitiIntegration, OpenSimZitiConfig, OpenSimComponentType}
};
...
async fn demo_basic_ziti_manager() -> Result<()> {
    ...
    println!("   - Dark Services: {}", capabilities.supports_dark_services);
    println!("   - Policy Enforcement: {}", capabilities.supports_policy_enforcement);
    println!("   - Identity Verification: {}", capabilities.supports_identity_verification);
...
async fn demo_opensim_integration() -> Result<()> {
    ...
    println!("✅ Integration Status: {}", 
             if integration.is_enabled() { "Enabled" } else { "Disabled" });
...
async fn demo_secure_service_communication() -> Result<()> {
    ...
    for (component_type, service_name) in components {
        ...
        match integration.create_secure_connection(
            ...
            Ok(connection_id) => {
                ...
                match integration.send_secure_data(
                    &connection_id,
                    &demo_data,
                    component_type
                ).await {
...
async fn demo_zero_trust_policies() -> Result<()> {
    ...
    let test_cases = vec![
        ...
        ("region-server", "region-communication", "Region-to-region communication"),
    ...
    for i in 1..=5 {
        ...
        match network_manager.get_network_stats().await {
            Ok(stats) => {
                ...
                for (key, value) in stats.iter().take(5) { // Show first 5 stats
                    println!("      - {}: {}", key, value);
                }
            }
            Err(e) => {
