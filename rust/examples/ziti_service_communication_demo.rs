...
use opensim_next::ziti::{
    ...
    config::ZitiConfig,
    services::{
        ZitiServiceManager, ServiceCommunicationHandler, ServiceMessage, ServiceMessageType,
        ServiceConnectionEvent, ServiceHandlerCapabilities, ServiceConnectionType, MessagePriority,
        ServiceDiscoveryInfo, ServiceCommunicationStats
    }
};
...
impl ServiceCommunicationHandler for RegionServiceHandler {
    async fn handle_message(&self, message: ServiceMessage) -> PhysicsResult<Option<ServiceMessage>> {
        println!("🏗️  {} received {} message from {}: {} bytes", 
                self.service_name, 
        ...
        match message.message_type {
            ServiceMessageType::Request => {
                ...
                let response_payload = format!("Response from {} to request {}", 
            ...
            ServiceMessageType::Heartbeat => {
                println!("💓 {} received heartbeat from {}", self.service_name, message.source_service);
                Ok(None)
            }
    ...
    async fn handle_connection_event(&self, event: ServiceConnectionEvent) -> PhysicsResult<()> {
        match event {
            ServiceConnectionEvent::Connected { connection_id, service_id } => {
                println!("🔗 {} connected to service {} (connection: {})", 
                        self.service_name, service_id, connection_id);
            }
            ServiceConnectionEvent::Disconnected { connection_id, reason } => {
                println!("🔌 {} disconnected from connection {} (reason: {})", 
                        self.service_name, connection_id, reason);
            }
            ServiceConnectionEvent::Error { connection_id, error } => {
                println!("❌ {} connection error on {}: {}", 
                        self.service_name, connection_id, error);
            }
            ServiceConnectionEvent::DataReceived { connection_id, data_size } => {
                println!("📦 {} received {} bytes on connection {}", 
                        self.service_name, data_size, connection_id);
            }
            ServiceConnectionEvent::AuthenticationRequired { connection_id, challenge } => {
                println!("🔐 {} requires authentication for connection {} (challenge: {})", 
...
impl ServiceCommunicationHandler for AssetServiceHandler {
    async fn handle_message(&self, message: ServiceMessage) -> PhysicsResult<Option<ServiceMessage>> {
        println!("💾 {} received {} message from {}: {} bytes", 
                self.service_name, 
                format!("{:?}", message.message_type),
                message.source_service,
                message.payload.len());
        ...
        match message.message_type {
            ServiceMessageType::Request => {
                ...
                if request_data.contains("asset_id") {
                    ...
                    println!("📤 {} sending asset data: {} bytes", self.service_name, asset_data.len());
                    Ok(Some(response))
                } else {
                    Ok(None)
                }
...
async fn demo_service_manager_initialization() -> Result<()> {
    ...
    println!("📋 Service manager initialized with {} default services", services.len());
...
async fn demo_service_registration_discovery() -> Result<()> {
    ...
    service_manager.register_handler("region-server", region_handler).await?;
    service_manager.register_handler("asset-server", asset_handler).await?;
    ...
    let region_services = service_manager.discover_services(
        Some(opensim_next::ziti::ZitiServiceType::Region)
    ).await?;
...
async fn demo_secure_service_communication() -> Result<()> {
    ...
    let connection_types = vec![
        ("region-server", "asset-server", ServiceConnectionType::Direct),
        ("region-server", "auth-server", ServiceConnectionType::Proxied),
        ("asset-server", "inventory-server", ServiceConnectionType::P2P),
    ];
    
    let mut connection_ids = Vec::new();
    
    for (source, target, conn_type) in connection_types {
        match service_manager.connect_services(source, target, conn_type).await {
            Ok(connection_id) => {
                println!("   ✅ Connected {} to {} ({})", source, target, connection_id);
...
async fn demo_message_routing() -> Result<()> {
    ...
    let messages = vec![
        ("region-server", "asset-server", ServiceMessageType::Request, "asset_id=12345", MessagePriority::High),
        ("asset-server", "region-server", ServiceMessageType::Event, "Asset cache updated", MessagePriority::Normal),
        ("region-server", "auth-server", ServiceMessageType::Request, "authenticate user", MessagePriority::Critical),
        ("messaging-server", "region-server", ServiceMessageType::Heartbeat, "ping", MessagePriority::Low),
    ];
    
    for (source, target, msg_type, content, priority) in messages {
        let message_id = service_manager.send_service_message(
            source,
            target,
            msg_type,
        ...
        println!("   📬 Sent {} message {} from {} to {}: \"{}\"", 
                format!("{:?}", msg_type), message_id, source, target, content);
...
async fn demo_service_health_monitoring() -> Result<()> {
    ...
    for service in &hosted_services {
        ...
        println!("   {} {} ({:?}): {:?} - {} clients, {} bytes transferred", 
                status_emoji,
                service.service_id,
        ...
        println!("      Last heartbeat: {} seconds ago", 
                std::time::SystemTime::now()
                    .duration_since(std::time::UNIX_EPOCH)
                    .unwrap_or_default()
                    .as_secs() - service.last_heartbeat);
    }
...
async fn demo_service_statistics() -> Result<()> {
    ...
    for service in &services {
        let stats = service_manager.get_communication_stats(&service.service_id).await?;
