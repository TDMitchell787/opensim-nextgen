...
use opensim_next::grid::{
    BackupFrequency, BackupPolicy, EnterpriseGrid, FederationInfo, GridConfiguration,
    GridStatistics, GridStatus, GridType, InstanceSize, LoadBalancingStrategy, MonitoringLevel,
    RegionAllocationPolicy, ScaleDownPolicy, ScaleUpPolicy, ScalingLimits, ScalingMetric,
    ScalingOperationType, ScalingPolicy, SecurityLevel, SharedServiceType, TrustLevel,
    TrustPermission,
};
...
async fn main() -> Result<()> {
    ...
    demo_enterprise_grid_registration(
        &server,
        corporate_grid_id,
        educational_grid_id,
        healthcare_grid_id,
    ...
    demo_grid_federation_management(
        &server,
        corporate_grid_id,
        educational_grid_id,
        healthcare_grid_id,
    ...
    demo_multi_grid_load_balancing(
        &server,
        vec![corporate_grid_id, educational_grid_id, healthcare_grid_id],
    )
    .await?;
    ...
    demo_enterprise_grid_ecosystem(
        &server,
        corporate_grid_id,
        educational_grid_id,
        healthcare_grid_id,
...
async fn demo_enterprise_grid_registration(
    ...
    let corporate_grid = EnterpriseGrid {
        ...
        grid_description: "Enterprise virtual collaboration platform for global technology company"
            .to_string(),
    ...
    let educational_grid = EnterpriseGrid {
        ...
        grid_description:
            "Educational virtual world for distance learning and research collaboration".to_string(),
    ...
    let healthcare_grid = EnterpriseGrid {
        ...
        grid_description:
            "Healthcare virtual environment for medical training and patient consultation"
                .to_string(),
...
async fn demo_grid_federation_management(
    ...
    let federation_request = server
        .create_federation_request(
            ...
            Some(
                "Request for research collaboration between TechCorp and Global University"
                    .to_string(),
            ),
        )
        .await?;
    ...
    println!(
        "✅ Federation request created: {}",
        federation_request.request_id
    );
    ...
    for grid in &federated_grids {
        println!(
            ...
            grid.grid_name,
            grid.grid_type,
            grid.statistics.total_regions,
            grid.statistics.total_users
        );
    }

    println!("🔹 Checking trust relationship...");
    let trust_relationship = server
        .get_trust_relationship(corporate_grid_id, educational_grid_id)
        .await?;
    match trust_relationship {
        Some(relationship) => {
            println!(
                "✅ Trust relationship found: {:?} level",
    ...
    println!(
        "   • Active partnerships: {}",
        federation_stats.active_partnerships
    );
    println!(
        "   • Shared services: {}",
        federation_stats.total_shared_services
    );
    println!(
        "   • Cross-grid users: {}",
        federation_stats.cross_grid_users
    );
    println!(
        "   • Federation uptime: {:.2}%",
        federation_stats.federation_uptime_percentage
    );
